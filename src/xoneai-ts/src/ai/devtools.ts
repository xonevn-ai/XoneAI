...
export async function enableDevTools(config?: DevToolsConfig): Promise<void> {
  ...
  const shouldEnable = config?.enabled ??
    (process.env.NODE_ENV === 'development' || process.env.XONEAI_DEVTOOLS === 'true');
...
export async function autoEnableDevTools(): Promise<void> {
  ...
  const isExplicitlyEnabled = process.env.XONEAI_DEVTOOLS === 'true';
  const isExplicitlyDisabled = process.env.XONEAI_DEVTOOLS === 'false';

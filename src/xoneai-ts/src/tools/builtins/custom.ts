...
import type { ToolMetadata, XoneTool, ToolExecutionContext, ToolParameterSchema } from '../registry/types';
...
export function registerCustomTool<TInput = unknown, TOutput = unknown>(
  ...
  const tool: XoneTool<TInput, TOutput> = {
    name: config.name,
    description: config.description,
  ...
  registry.register(metadata, () => tool as XoneTool<unknown, unknown>);
...
export async function registerNpmTool(
  packageName: string,
  toolName?: string
): Promise<XoneTool<unknown, unknown>> {
  try {

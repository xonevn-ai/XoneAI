 ...
 */
export function codeMode(config?: CodeModeConfig): XoneTool<CodeModeInput, CodeModeResult> {
  const settings = {
    allowNetwork: false,
    timeoutMs: 30000,
    maxMemoryMb: 128,
 ...
 */
export function createCodeModeTool(config?: CodeModeConfig): XoneTool<unknown, unknown> {
  return codeMode(config) as XoneTool<unknown, unknown>;
}

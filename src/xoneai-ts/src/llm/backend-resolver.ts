 ...
 * - XONEAI_BACKEND: 'ai-sdk' | 'native' | 'auto' (default: 'auto')
 */
...
export function getPreferredBackend(): 'ai-sdk' | 'native' | 'auto' {
  const env = process.env.XONEAI_BACKEND?.toLowerCase();
...
export async function resolveBackend(
  ...
  if (preferredBackend === 'ai-sdk' || preferredBackend === 'auto') {
    ...
    if (aiSdkAvailable) {
      try {
        ...
        if (preferredBackend === 'ai-sdk') {
          ...
          throw new Error(
            ...
            `Set XONEAI_BACKEND=native to use native providers.`
          );
        }
      ...
      throw new Error(
        `AI SDK is not installed but XONEAI_BACKEND=ai-sdk is set. ` +

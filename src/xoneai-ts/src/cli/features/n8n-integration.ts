...
export class N8NIntegration {
  ...
  /**
   * Export XoneAI workflow to n8n format
   */
  exportWorkflow(
    name: string,
    steps: Array<{
  ...
  /**
   * Map XoneAI step type to n8n node type
   */
  ...
  private mapStepParameters(step: { type: string; config?: Record<string, any> }): Record<string, any> {
    switch (step.type) {
      case 'agent':
        return {
          method: 'POST',
          url: '={{ $env.XONEAI_API_URL }}/agent/execute',
          sendBody: true,
          bodyParameters: {
            parameters: [
...
return input;
  ...
  /**
   * Import n8n workflow and convert to XoneAI format
   */
  ...
  /**
   * Map n8n node type to XoneAI step type
   */

...
function getDbUrl(options: KnowledgeOptions): string | undefined {
  return options.db || process.env[ENV_VARS.XONEAI_DB];
}
...
async function showHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('  xoneai-ts knowledge add "Important fact" --db sqlite:./data.db');
    await pretty.dim('  xoneai-ts knowledge list --db sqlite:./data.db');
  }
}

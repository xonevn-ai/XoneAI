...
async function showHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('  xoneai-ts observability list');
    await pretty.dim('  xoneai-ts observability doctor langfuse');
    await pretty.dim('  xoneai-ts observability test memory');
  }
}

...
async function testCommand(args: string[], options: LLMOptions, outputFormat: string): Promise<void> {
  ...
  if (!providerId) {
    if (outputFormat === 'json') {
      ...
      await pretty.dim('  Example: xoneai-ts llm test openai');
    }
    process.exit(LLM_EXIT_CODES.INVALID_ARGS);
    return;
  }
...
async function runCommand(args: string[], options: LLMOptions, outputFormat: string): Promise<void> {
  ...
  if (!prompt) {
    if (outputFormat === 'json') {
      ...
      await pretty.dim('  Example: xoneai-ts llm run "Hello!" --model openai/gpt-4o-mini');
    }
    process.exit(LLM_EXIT_CODES.INVALID_ARGS);
    return;
  }
...
async function modelsCommand(args: string[], options: LLMOptions, outputFormat: string): Promise<void> {
  try {
    ...
    if (outputFormat === 'json') {
      ...
      await pretty.newline();
      await pretty.info('Usage: xoneai-ts llm run "prompt" --model provider/model');
    }
  } catch (error) {
...
async function helpCommand(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('  xoneai-ts llm providers');
    await pretty.dim('  xoneai-ts llm test openai');
    await pretty.dim('  xoneai-ts llm run "Hello" --model openai/gpt-4o-mini');
  }
}

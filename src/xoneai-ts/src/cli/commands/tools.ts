...
async function listTools(options: ToolsOptions, outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('\nUse "xoneai-ts tools info <id>" for details');
    await pretty.dim('Use "xoneai-ts tools doctor" to check dependencies');
  }
}
...
async function toolInfo(args: string[], options: ToolsOptions, outputFormat: string): Promise<void> {
  ...
  if (!metadata && !registeredTool) {
    if (outputFormat === 'json') {
      ...
      await pretty.dim('Use "xoneai-ts tools list" to see available tools');
    }
  ...
  if (outputFormat === 'json') {
    ...
    if (metadata) {
      ...
      await pretty.keyValue({
        ...
        'Docs': `https://docs.xone.ai/js/${metadata.docsSlug}`,
      });
...
async function toolAdd(args: string[], options: ToolsOptions, outputFormat: string): Promise<void> {
  ...
  if (!packageOrPath) {
    if (outputFormat === 'json') {
      ...
      await pretty.dim('Usage: xoneai-ts tools add <package-name>');
      await pretty.dim('       xoneai-ts tools add ./path/to/tool.js');
    }
    process.exit(EXIT_CODES.INVALID_ARGUMENTS);
  }

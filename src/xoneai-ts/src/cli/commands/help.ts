...
async function showGeneralHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.plain(`
XoneAI TypeScript CLI v${CLI_SPEC_VERSION}

Usage:
  xoneai-ts <command> [options]
...
Environment Variables:
  ...
  XONEAI_MODEL      Default model

Examples:
  xoneai-ts chat "Hello, how are you?"
  xoneai-ts chat "Write a poem" --stream
  xoneai-ts run "Analyze this data" --agent my-agent.yaml
  xoneai-ts providers
  xoneai-ts help chat
`);
  }
...
async function showCommandHelp(command: string, outputFormat: string): Promise<void> {
  ...
  if (!spec) {
    ...
    await pretty.info('Run "xoneai-ts help" to see available commands');
    return;
  }
...
Usage:
  xoneai-ts ${command}${args.length ? ' ' + args.map(a => a.required ? `<${a.name}>` : `[${a.name}]`).join(' ') : ''} [options]`);

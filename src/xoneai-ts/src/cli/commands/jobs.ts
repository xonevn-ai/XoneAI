...
function showHelp(isJson: boolean): void {
  const help = {
    ...
    examples: [
      'xoneai-ts jobs add my-job \'{"input": "data"}\'',
      'xoneai-ts jobs add task --priority high',
      'xoneai-ts jobs list --status pending',
      'xoneai-ts jobs get <job-id>',
      'xoneai-ts jobs cancel <job-id>',
      'xoneai-ts jobs stats',
      'xoneai-ts jobs cleanup --days 30'
    ]
  };

...
async function showHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('  xoneai-ts voice speak "Hello World" --voice nova');
    await pretty.dim('  xoneai-ts voice transcribe audio.mp3');
    await pretty.dim('  xoneai-ts voice voices');
  }
}

...
async function approveRequest(requestId: string, outputFormat: string): Promise<void> {
  if (!requestId) {
    throw new Error('Request ID required. Usage: xoneai approval approve <requestId>');
  }
...
async function denyRequest(requestId: string, outputFormat: string): Promise<void> {
  if (!requestId) {
    throw new Error('Request ID required. Usage: xoneai approval deny <requestId>');
  }
...
async function cancelRequest(requestId: string, outputFormat: string): Promise<void> {
  if (!requestId) {
    throw new Error('Request ID required. Usage: xoneai approval cancel <requestId>');
  }
...
async function addAutoApprove(args: string[], outputFormat: string): Promise<void> {
  ...
  if (!toolName) {
    throw new Error('Tool name required. Usage: xoneai approval auto-approve <toolName>');
  }
...
async function addAutoDeny(args: string[], outputFormat: string): Promise<void> {
  const toolName = args[0];
  if (!toolName) {
    throw new Error('Tool name required. Usage: xoneai approval auto-deny <toolName>');
  }
...
async function showHelp(outputFormat: string): Promise<void> {
  const help = {
    ...
    examples: [
      'xoneai approval status',
      'xoneai approval pending',
      'xoneai approval approve abc123',
      'xoneai approval auto-approve readFile',
      'xoneai approval interactive',
    ],
  };

...
const CACHE_DIR = path.join(os.homedir(), '.xoneai', 'cache');
...
async function showHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.dim('  xoneai-ts cache set mykey "Hello World"');
    await pretty.dim('  xoneai-ts cache get mykey');
    await pretty.dim('  xoneai-ts cache list');
  }
}

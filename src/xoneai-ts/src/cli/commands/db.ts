...
async function showHelp(outputFormat: string): Promise<void> {
  ...
  if (outputFormat === 'json') {
    ...
    await pretty.newline();
    await pretty.dim('Examples:');
    await pretty.dim('  xoneai-ts db connect sqlite:./data.db');
    await pretty.dim('  xoneai-ts db test');
    await pretty.dim('  xoneai-ts db test postgres://user:pass@host:5432/db');
  }
}

...
async function handleCreate(args: string[], options: Record<string, unknown>, isJson: boolean): Promise<void> {
  ...
  if (!name) {
    ...
    if (isJson) {
      ...
      console.log('Usage: xoneai-ts scheduler create <name> --cron "* * * * *"');
    }
    process.exit(1);
  }
...
function showHelp(isJson: boolean): void {
  const help = {
    ...
    examples: [
      'xoneai-ts scheduler create my-task --cron "*/5 * * * *"',
      'xoneai-ts scheduler create backup --interval 3600',
      'xoneai-ts scheduler list',
      'xoneai-ts scheduler run <task-id>',
      'xoneai-ts scheduler patterns'
    ]
  };

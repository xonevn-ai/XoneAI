...
export async function execute(args: string[], options: Record<string, unknown>): Promise<void> {
  ...
  const tui = createInteractiveTUI({
    ...
    historyFile,
    welcomeMessage: `XoneAI Interactive Mode
Model: ${model || 'default'}
...
Examples:
  xoneai-ts interactive
  xoneai-ts interactive --model gpt-4o
  xoneai-ts interactive --history ~/.xoneai_history
`);
}

...
export function loadEnvConfig(): EnvConfig {
  ...
  const model = process.env[ENV_VARS.XONEAI_MODEL];
  if (model) config.model = model;

  const profile = process.env[ENV_VARS.XONEAI_PROFILE];
  if (profile) config.profile = profile;

  const verbose = process.env[ENV_VARS.XONEAI_VERBOSE];
  if (verbose) config.verbose = verbose.toLowerCase() === 'true';

  const configPath = process.env[ENV_VARS.XONEAI_CONFIG];

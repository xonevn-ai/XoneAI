 ...
 * These tests require API keys and are gated by XONEAI_TEST_REAL_KEYS env var
 */

import { describe, it, expect, beforeAll, afterAll } from '@jest/globals';

// Skip all tests if real keys not enabled
const SKIP_REAL_TESTS = !process.env.XONEAI_TEST_REAL_KEYS;
...
describe('Real Provider Integration Tests', () => {
  beforeAll(() => {
    if (SKIP_REAL_TESTS) {
      console.log('Skipping real API tests. Set XONEAI_TEST_REAL_KEYS=1 to enable.');
    }
  });

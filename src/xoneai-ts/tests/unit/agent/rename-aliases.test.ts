 ...
 * - XoneAIAgents → AgentTeam (with Agents, XoneAIAgents as aliases)
...
describe('AgentTeam Rename', () => {
  ...
  describe('Silent Aliases', () => {
    it('should export XoneAIAgents as alias for AgentTeam', async () => {
      const { AgentTeam, XoneAIAgents } = await import('../../../src/agent/simple');
      expect(XoneAIAgents).toBe(AgentTeam);
    });
  ...
  describe('Main Package Exports', () => {
    ...
    it('should export all aliases from main index', async () => {
      ...
      expect(pkg.AgentTeam).toBe(pkg.XoneAIAgents);
    });
  });
});
...
describe('Backward Compatibility', () => {
  it('should allow instantiation with old name XoneAIAgents', async () => {
    const { XoneAIAgents, Agent } = await import('../../../src/agent/simple');
    const agent = new Agent({ instructions: 'Test agent' });
    const team = new XoneAIAgents([agent]);

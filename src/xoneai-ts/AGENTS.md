# AGENTS.md - XoneAI TypeScript SDK Comprehensive Guide

> **For AI Agents and Developers**: This document provides the complete context needed to work with the XoneAI TypeScript SDK, including design principles, architecture, repository structure, and implementation guidelines.

---

## 1. What is XoneAI TypeScript?

XoneAI TypeScript is a **high-performance, agentic AI framework** for Node.js/JavaScript, designed for building production-ready AI agents and multi-agent workflows.
...
├── src/
│   ├── xoneai-agents/          # Python SDK (xoneaiagents)
│   │   ├── xoneaiagents/       # Python package
│   │   ├── tests/                 # Unit & integration tests
│   │   └── examples/              # Package-level examples
│   │
│   ├── xoneai-ts/              # TypeScript SDK (this package)
│   │   ├── src/                   # Source code
│   │   ├── tests/                 # Unit & integration tests
│   │   ├── examples/              # TypeScript examples
│   │   └── package.json           # Package config
│   │
│   └── xoneai/                 # Wrapper (xoneai CLI)
│
...
```typescript
import { AgentTeam, Agent } from 'xoneai';
...
export type XoneAIAgents = AgentTeam;
```
...
```typescript
import { tool, FunctionTool } from 'xoneai';
...
XoneAI TypeScript provides first-class support for Vercel AI SDK:
...
```typescript
import { Agent, tools } from 'xoneai';
...
```typescript
import { Memory, FileMemory, AutoMemory } from 'xoneai';
...
```typescript
import { Agent, AgentCallbacks } from 'xoneai';
...
```typescript
import { WorkflowHooks, createLoggingWorkflowHooks } from 'xoneai';
...
```typescript
import { Task, Workflow, createContext } from 'xoneai';
...
```typescript
import { Repeat, repeat } from 'xoneai';
...
```typescript
import { MCPClient, createMCPClient } from 'xoneai';
...
```typescript
import { MCPServer, createMCPServer } from 'xoneai';
...
```typescript
import { createLangfuseAdapter, createLangSmithAdapter } from 'xoneai';
...
```typescript
import { createGuardrail, GuardrailAction } from 'xoneai';
...
```typescript
import { AudioAgent, createAudioAgent } from 'xoneai';
...
```bash
# Install globally
npm install -g xoneai

# Run agent
xoneai-ts agent run --prompt "Hello"

# Multi-agent
xoneai-ts agents run --config agents.yaml

# Start server
xoneai-ts serve --port 8000
```
...
```typescript
// Most common
import { Agent, AgentTeam, Task, tool, tools } from 'xoneai';

// Memory
import { Memory, FileMemory, AutoMemory, db } from 'xoneai';

// Hooks
import { HookManager, AgentCallbacks, WorkflowHooks } from 'xoneai';

// MCP
import { MCPClient, MCPServer, createMCPClient } from 'xoneai';

// Observability
import { createLangfuseAdapter, createLangSmithAdapter } from 'xoneai';

// Specialized agents
import { AudioAgent, VideoAgent, VisionAgent, CodeAgent } from 'xoneai';

// Workflows
import { Workflow, Loop, Repeat, createContext } from 'xoneai';

// RAG
import { RAG, createRAG, Knowledge } from 'xoneai';

// Eval
import { Evaluator, EvalSuite, createEvaluator } from 'xoneai';
```
...
// CLI: xoneai-ts memory clear --session <id>
```

| Feature | API | CLI Equivalent |
|---------|-----|----------------|
| Chat | `agent.chat(prompt)` | `xoneai-ts agent chat` |
| Multi-agent | `team.start(task)` | `xoneai-ts agents run` |
| Server | `createServer()` | `xoneai-ts serve` |
...
> Make XoneAI the **best TypeScript agent framework in the world**
...
*This document is the source of truth for the XoneAI TypeScript SDK architecture and design principles.*

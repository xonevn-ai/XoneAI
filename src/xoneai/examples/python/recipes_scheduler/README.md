...
```bash
# Basic scheduler
xoneai schedule start news-checker "Check AI news" --interval hourly

# With timeout and budget
xoneai schedule start reporter "Generate report" \
    --interval daily \
    --timeout 300 \
    --max-cost 1.00

# With max retries
xoneai schedule start checker "Check updates" \
    --interval hourly \
    --max-retries 5
```
...
```bash
# Schedule a recipe
xoneai schedule start news-monitor --recipe news-analyzer --interval hourly

# With all options
xoneai schedule start my-scheduler \
    --recipe my-recipe \
    --interval "*/6h" \
    --timeout 600 \
    --max-cost 2.00 \
...
xoneai schedule "Check news" --interval "*/30s"
```
...
# View logs
xoneai schedule logs news-checker --follow

# Show statistics
xoneai schedule stats news-checker

# Stop a scheduler
xoneai schedule stop news-checker

# Stop all schedulers
xoneai schedule stop-all

# Delete a scheduler
xoneai schedule delete news-checker
```

### Export scheduler config

```bash
xoneai schedule save news-checker --output news-checker.yaml
```

"""XoneAI Code - Optimized Chainlit Application
...
- Profiling hooks (optional via XONE_CODE_PROFILE=1)
"""
...
PROFILING_ENABLED = os.getenv("XONE_CODE_PROFILE", "").lower() in ("1", "true", "yes")
_profile_data = {}
...
def _get_xoneai_agent():
    """Lazy load XoneAI Agent for reuse."""
    if 'Agent' not in _cached_modules:
        try:
            _profile_start("import_xoneai_agent")
            from xoneaiagents import Agent
            _cached_modules['Agent'] = Agent
            _profile_end("import_xoneai_agent")
        except ImportError:
...
def _get_interactive_tools():
    ...
    if 'interactive_tools' not in _cached_modules:
        try:
            ...
            config.workspace = os.environ.get("XONEAI_CODE_REPO_PATH", os.getcwd())
...
async def claude_code_tool(query: str) -> str:
    ...
    try:
        repo_path = os.environ.get("XONEAI_CODE_REPO_PATH", ".")
...
async def start():
    ...
    claude_code_enabled = os.getenv("XONEAI_CLAUDECODE_ENABLED", "false").lower() == "true"
    ...
    await cl.Message(
        content=f"🚀 **XoneAI Code Assistant Ready**\n\n"
                f"**Model:** {model_name}\n"
...
"""
    ...
    # Try XoneAI Agent first (faster, with tool reuse)

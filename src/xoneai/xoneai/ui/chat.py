"""XoneAI Chat - Optimized Chainlit Application
...
- Session management
- Profiling hooks (optional via XONE_CHAT_PROFILE=1)
...
PROFILING_ENABLED = os.getenv("XONE_CHAT_PROFILE", "").lower() in ("1", "true", "yes")
_profile_data = {}
...
def load_custom_tools():
    ...
    tools_path = os.getenv("XONEAI_TOOLS_PATH")
    if tools_path:
        if not os.path.exists(tools_path):
            logger.warning(f"XONEAI_TOOLS_PATH set but path does not exist: {tools_path}")
            return custom_tools
    else:

...
from xoneaiagents import Agent, Task, Agents, register_display_callback
...
# Main XoneAI Runner
# -----------------------------------------------------------------------------
async def ui_run_xoneai(config, topic, tools_dict):
    logger.info("Starting ui_run_xoneai")
    agents_map = {}
    tasks = []
    tasks_dict = {}
    ...
    try:
        ...
        await cl.Message(content="Starting XoneAI agents execution...", author="System").send()
        ...
        await cl.Message(content="XoneAI agents execution completed.", author="System").send()
        await asyncio.sleep(1)
        queue_processor.cancel()
        return result

    except Exception as e:
        error_msg = f"Error in ui_run_xoneai: {str(e)}"
...
async def main(message: cl.Message):
    try:
        ...
        # Run XoneAI
        result = await ui_run_xoneai(config, message.content, tools_dict)
        ...
        error_msg = f"Error running XoneAI agents: {str(e)}"
        logger.error(error_msg, exc_info=True)
        await cl.Message(content=error_msg, author="System").send()

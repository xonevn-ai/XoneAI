"""XoneAI Bot UI - Chainlit chat interface with real-time streaming.

Provides a browser-based chat UI backed by XoneAI Agent with:
...
Usage:
    ...
    xoneai ui bot --agent agents.yaml --tools DuckDuckGoTool
"""
...
def _get_stream_event_type():
    if "StreamEventType" not in _cache:
        from xoneaiagents.streaming.events import StreamEventType
...
def _get_bot_handler():
    ...
    if "BotHandler" not in _cache:
        from xoneai.cli.features.bots_cli import BotHandler
...
def _get_bot_capabilities():
    if "BotCapabilities" not in _cache:
        from xoneai.cli.features.bots_cli import BotCapabilities
...
def _get_config():
    ...
    return {
        "model": os.getenv("XONEAI_BOT_MODEL", "gpt-4o-mini"),
        "agent_file": os.getenv("XONEAI_BOT_AGENT_FILE"),
        "memory": os.getenv("XONEAI_BOT_MEMORY", "").lower() in ("1", "true"),
        "web_search": os.getenv("XONEAI_BOT_WEB_SEARCH", "").lower() in ("1", "true"),
        "web_provider": os.getenv("XONEAI_BOT_WEB_PROVIDER", "duckduckgo"),
        "tools": [t.strip() for t in os.getenv("XONEAI_BOT_TOOLS", "").split(",") if t.strip()],
        "auto_approve": os.getenv("XONEAI_BOT_AUTO_APPROVE", "").lower() in ("1", "true"),
    }
...
def _create_agent(model: str = None, memory: bool = False, web_search: bool = False,
    ...
    """Create a XoneAI Agent with capabilities, reusing BotHandler patterns."""
...
def _fire_hook_safe(agent, hook_name: str, data: dict):
    ...
    try:
        ...
        if hook_name == "MESSAGE_RECEIVED":
            from xoneaiagents.hooks.events import MessageReceivedInput
            ...
            from xoneaiagents.hooks.events import MessageSendingInput
            ...
            from xoneaiagents.hooks.events import MessageSentInput

...
class ContextGatherer:
    def __init__(self, directory='.', output_file='context.txt',
        ...
        self.max_tokens = int(os.getenv("XONEAI_MAX_TOKENS", max_tokens))
    ...
    def get_ignore_patterns(self):
        ...
        the following order:
            ...
            3. XONEAI_IGNORE_FILES environment variable
            4. .gitignore
            5. Default patterns
        """
        ignore_patterns = []
        ...
        # 1. Load from .xoneignore
        load_from_file(os.path.join(self.directory, '.xoneignore'))
        ...
        ignore_files_env = os.getenv("XONEAI_IGNORE_FILES")
        if ignore_files_env:
    ...
    def get_include_paths(self):
        ...
        Loads include paths from:
            1. .xoneinclude (includes ONLY files/directories listed)
            2. .xonecontext (if .xoneinclude doesn't exist, this is used
        ...
        include_file = os.path.join(self.directory, '.xonecontext')
        ...
        # If .xonecontext doesn't exist, fall back to .xoneinclude
        # for including all relevant files
        if not include_paths: 
            include_file = os.path.join(self.directory, '.xoneinclude')
            if os.path.exists(include_file):
                with open(include_file, 'r') as f:
                    include_paths.extend(
                        line.strip() for line in f
    ...
    def gather_context(self):
        ...
        and include options from .xoneinclude and .xonecontext.
        """
        context = []
        total_files = 0
        processed_files = 0

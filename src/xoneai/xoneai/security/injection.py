"""
Prompt injection defense for XoneAI agents.

Implements a 6-check detection pipeline adapted from production
sovereign-agent security research. Works as a XoneAI hook
...
_TRUSTED_SOURCES = frozenset([
    "trusted_tool", "internal", "system", "xoneai_core",
])
...
def scan_text(text: str, source: str = "external") -> ScanResult:
    ...
    if triggered:
        logger.warning(
            "[xoneai.security] Injection detected | source=%s level=%s checks=%s preview=%r",
...
class InjectionDefense:
    """
    Injection defense that integrates with XoneAI's hook system.
    ...
    Example:
        ...
        >>> # Register with agent:
        >>> from xoneaiagents.hooks import add_hook
    ...
    def create_hook(self) -> Callable:
        """
        Create a BEFORE_TOOL hook function for use with XoneAI's hook system.
        ...
        def _injection_hook(data: Any):
            ...
            from xoneaiagents.hooks import HookResult
            ...
            for text in strings:
                ...
                if result.blocked:
                    logger.warning(
                        "[xoneai.security] Blocking tool=%s agent=%s checks=%s",
    ...
    def create_agent_hook(self) -> Callable:
        ...
        def _agent_injection_hook(data: Any):
            ...
            if result.blocked:
                logger.warning(
                    "[xoneai.security] Blocking agent prompt agent=%s checks=%s",
                    getattr(data, "agent_name", "?"),
                    result.checks_triggered,
                )
                return HookResult(

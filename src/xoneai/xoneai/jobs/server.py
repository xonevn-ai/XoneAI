"""
Server for XoneAI Async Jobs API.
...
def get_executor() -> JobExecutor:
    ...
    if _executor is None:
        _executor = JobExecutor(
            store=get_store(),
            max_concurrent=int(os.environ.get("XONEAI_MAX_CONCURRENT_JOBS", "10")),
            default_timeout=int(os.environ.get("XONEAI_JOB_TIMEOUT", "3600"))
        )
    return _executor
...
def start_server(
    ...
    logger.info(f"Starting XoneAI Jobs API on {host}:{port}")
    
    uvicorn.run(
        "xoneai.jobs.server:create_app",
        host=host,
        port=port,
        reload=reload,
        workers=workers,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="XoneAI Jobs API Server")

"""
Job Executor for XoneAI Async Jobs API.
...
class JobExecutor:
    ...
    async def _run_agent(self, job: Job) -> Any:
        ...
        This is the core execution logic that runs the XoneAI agent.
        ...
        # Imports are done lazily in _run_xoneai_agents and _run_legacy_xoneai
        
        # Determine agent configuration
        agent_file = job.agent_file or "agents.yaml"
        framework = job.framework or "xoneai"
        ...
        try:
            # Try xoneaiagents first (preferred)
            if framework == "xoneai":
                result = await self._run_xoneai_agents(job, agent_file)
            else:
                # Use legacy XoneAI for crewai/autogen
                result = await self._run_legacy_xoneai(job, agent_file, framework)
    ...
    async def _run_recipe(self, job: Job) -> Any:
        ...
        try:
            from xoneai.recipe.bridge import resolve, execute_resolved_recipe
        except ImportError:
            raise RuntimeError("Recipe execution requires xoneai.recipe module")
    ...
    async def _run_xoneai_agents(self, job: Job, agent_file: str) -> Any:
        """Run using xoneaiagents framework."""
        try:
            from xoneaiagents import Agent
        except ImportError:
            raise RuntimeError("xoneaiagents not installed")
    ...
    async def _run_legacy_xoneai(self, job: Job, agent_file: str, framework: str) -> Any:
        """Run using legacy XoneAI (crewai/autogen)."""
        try:
            from xoneai import XoneAI
        except ImportError:
            raise RuntimeError("xoneai not installed")
        
        # Update progress
        job.update_progress(percentage=20.0, step="Creating XoneAI instance")
        await self.store.save(job)
        await self._notify_progress(job)
        
        # Create XoneAI instance
        xoneai = XoneAI(
        ...
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, xoneai.run)

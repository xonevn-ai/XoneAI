...
agents = XoneAIAgentTeam(agents=[agent1, agent2], tasks=[task])
result = agents.start()
print(result)
...
'''
    ...
    def _generate_concept_doc(self, slug: FeatureSlug) -> str:
        ...
        return f'''---
title: "{name}"
description: "Understanding {slug_str} in XoneAI"
icon: "lightbulb"
---
...
'''
    ...
    def _generate_feature_doc(self, slug: FeatureSlug) -> str:
        ...
        return f'''---
title: "{name}"
description: "How to use {slug_str} in XoneAI"
icon: "code"
---
...
- [API Reference](/docs/sdk/xoneaiagents/{slug_str}/{slug_str})
'''
...
|----------|-------------|---------|
| `XONEAI_{slug_str.upper().replace("-", "_")}` | Configure {slug_str} | None |
...
```bash
xoneai "Hello" --{slug_str}
```

### With Other Flags

```bash
xoneai "Hello" --{slug_str} --verbose
```

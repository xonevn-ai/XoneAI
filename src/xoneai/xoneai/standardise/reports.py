...
class ReportGenerator:
    ...
    def _generate_text(self, report: StandardiseReport) -> str:
        ...
        # Header
        lines.append("")
        lines.append("🔍 XoneAI Standardisation Check")
        lines.append("══════════════════════════════════")
        lines.append("")
        ...
        lines.append("───────────────")
        lines.append("1. Run `xoneai standardise report --format markdown` for details")
        if report.duplicates:
            lines.append("2. Run `xoneai standardise fix --feature <slug>` to consolidate")
        if report.missing_artifacts:
            lines.append("3. Run `xoneai standardise init --feature <slug>` to create missing")
        lines.append("")
    ...
    def _generate_markdown(self, report: StandardiseReport) -> str:
        ...
        # Header
        lines.append("# XoneAI Standardisation Report")
        lines.append("")

...
class AIGenerator:
    """AI-powered content generator using xoneaiagents.
    ...
    def _get_safe_edit_pipeline(self):
        ...
        if self._safe_edit_pipeline is None:
            try:
                from xoneai.acp.safe_edit import SafeEditPipeline
        ...
        if self._agent is None:
            try:
                ...
                self._agent = Agent(
                    ...
                    goal="Generate high-quality documentation and examples for XoneAI features",
                ...
                raise RuntimeError("xoneaiagents not available. Install with: pip install xoneaiagents")
        return self._agent
    ...
    def _get_verifier(self):
        ...
        if self._verifier is None:
            try:
                ...
                raise RuntimeError("xoneaiagents not available")
...
"""
    ...
    def _build_basic_example_prompt(self, slug_str: str, name: str, 
        ...
        prompt_parts = [
            f"Generate a BASIC Python example for '{name}' in XoneAI.",
            "",
            "## CRITICAL RULES:",
            f"1. ONE-LINE docstring: `\"\"\"{name} - Basic Example\"\"\"`",
            "2. MUST use real imports: `from xoneaiagents import Agent`",
...
```''')
    ...
    def _build_advanced_example_prompt(self, slug_str: str, name: str,
        ...
        prompt_parts = [
            f"Generate a COMPREHENSIVE ADVANCED Python example for '{name}' in XoneAI.",
            "",
            "## CRITICAL RULES:",
            f"1. ONE-LINE docstring: `\"\"\"{name} - Advanced Example\"\"\"`",
            "2. MUST use real imports: `from xoneaiagents import Agent, Task, Agents`",
            "3. Use AgentTeam() NOT XoneAIAgentTeam() - Agents is the correct alias",
            "4. NO mock classes - use the REAL classes from xoneaiagents",
...
agents = AgentTeam(agents=[agent4], tasks=[task])
...
```''')
    ...
    def _build_docs_prompt(self, slug: FeatureSlug,
        ...
        prompt_parts = [
            f"Generate a {artifact_type.value} documentation page for the '{name}' feature in XoneAI.",
            "",
            "## Template to follow:",
            "```",
            template[:2000],
        ...
        prompt_parts.extend([
            ...
            "6. Ensure all imports are correct for xoneaiagents",

...
Based on analysis of existing XoneAI documentation best practices:
...
```python
from xoneaiagents import Agent, Task
...
FEATURE_TEMPLATE = '''---
title: "{name}"
description: "How to use {slug} in XoneAI"
icon: "code"
---
...
agents = AgentTeam(agents=[agent1, agent2], tasks=[task])
result = agents.start()
```
</CodeGroup>
...
- [API Reference](/docs/sdk/xoneaiagents/{slug}/{slug})
'''
...
|----------|-------------|---------|
| `XONEAI_{slug_upper}` | Configure {slug} | None |
...
```bash
xoneai {slug} --config config.yaml
```
...
<Accordion title="Command not found">
  Ensure XoneAI is installed: `pip install xoneai`
</Accordion>
...
```python
from xoneaiagents import {class_name}
...
class EnhancedTemplateGenerator:
    ...
    def generate(self, slug: FeatureSlug, artifact_type: ArtifactType) -> str:
        ...
        return template.format(
            ...
            description=f"{name} functionality for XoneAI agents",
        )
    ...
    def get_expected_path(self, slug: FeatureSlug, artifact_type: ArtifactType,
        ...
        if artifact_type == ArtifactType.EXAMPLE_BASIC:
            ...
            if docs_root:
                return docs_root / "sdk" / "xoneaiagents" / slug_str / f"{slug_str}.mdx"

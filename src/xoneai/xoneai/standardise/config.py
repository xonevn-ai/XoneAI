...
2. Config file (.xone/standardise.yaml)
...
class StandardiseConfig:
    ...
    def _detect_project_root(self) -> Optional[Path]:
        ...
        # Look for .xone directory or pyproject.toml
        for parent in [cwd] + list(cwd.parents):
            if (parent / ".xone").exists():
                return parent
    ...
    def _auto_detect_paths(self):
        ...
        # Docs root detection - default: ~/XoneAIDocs/docs
        if self.docs_root is None:
            # First check home directory for XoneAIDocs
            home_docs = Path.home() / "XoneAIDocs" / "docs"
            if home_docs.exists():
                self.docs_root = home_docs
            else:
                # Fallback to project-relative paths
                for candidate in ["docs", "XoneAIDocs/docs"]:
                    path = root / candidate
                    if path.exists():
                        self.docs_root = path
                        break
                # Check parent for XoneAIDocs
                parent_docs = root.parent / "XoneAIDocs"
                if parent_docs.exists() and self.docs_root is None:
                    self.docs_root = parent_docs / "docs"
        
        # Examples root detection - default: ~/xoneai-package/examples
        if self.examples_root is None:
            # First check home directory for xoneai-package/examples
            home_examples = Path.home() / "xoneai-package" / "examples"
        ...
        if self.sdk_root is None:
            for candidate in [
                "src/xoneai-agents/xoneaiagents",
                "xoneaiagents",
            ]:
                path = root / candidate
                if path.exists():
        ...
        if self.manifest_dir is None:
            self.manifest_dir = root / ".xone" / "features"
    ...
    def from_env(cls) -> "StandardiseConfig":
        ...
        return cls(
            project_root=Path(os.environ["XONE_PROJECT_ROOT"]) if os.environ.get("XONE_PROJECT_ROOT") else None,
            docs_root=Path(os.environ["XONE_DOCS_ROOT"]) if os.environ.get("XONE_DOCS_ROOT") else None,
            dry_run=os.environ.get("XONE_DRY_RUN", "true").lower() == "true",

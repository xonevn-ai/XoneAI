...
3. Config file (~/.xone/config.yaml)
4. Defaults (lowest)
"""
...
class ACPConfig:
    ...
    def from_env(cls) -> "ACPConfig":
        ...
        return cls(
            workspace=Path(os.environ.get("XONEAI_WORKSPACE", ".")).resolve(),
            agent=os.environ.get("XONEAI_AGENT", "default"),
            model=os.environ.get("XONEAI_MODEL") or os.environ.get("OPENAI_MODEL"),
            debug=os.environ.get("XONEAI_DEBUG", "").lower() in ("1", "true", "yes"),
            read_only=os.environ.get("XONEAI_READ_ONLY", "true").lower() in ("1", "true", "yes"),
    ...
    def from_file(cls, path: Optional[Path] = None) -> "ACPConfig":
        ...
        if path is None:
            path = Path.home() / ".xone" / "config.yaml"

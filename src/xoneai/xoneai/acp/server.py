"""
ACP Server implementation for XoneAI.

This module implements the Agent Client Protocol server that allows
IDEs/editors to communicate with XoneAI agents.
"""
...
def _setup_stderr_logging(debug: bool = False) -> None:
    ...
    root_logger = logging.getLogger("xoneai.acp")
...
class ACPServer:
    """
    ACP Server that exposes XoneAI agents to IDE clients.
    ...
    def _get_agent(self):
        ...
        try:
            ...
            logger.error("xoneaiagents not installed")
            raise RuntimeError("xoneaiagents package required")
...
async def _run_server(config: ACPConfig) -> None:
    ...
    try:
        ...
        logger.error(
            ...
            "Install with: pip install xoneai[acp]\n"
    ...
    logger.info("Starting XoneAI ACP server...")
    await run_agent(XoneACPAgent(), use_unstable_protocol=True)
...
def serve(
    ...
    **kwargs: Any,
    ...
    This is the main entry point for running XoneAI as an ACP agent.

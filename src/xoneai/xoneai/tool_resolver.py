...
2. xoneaiagents.tools.TOOL_MAPPINGS (built-in SDK tools)
3. xoneai-tools package (external tools, optional)
4. Tool registry (plugins via entry_points)

Usage:
    from xoneai.tool_resolver import ToolResolver, resolve_tool
...
class ToolResolver:
    ...
    to built-in tools from xoneaiagents.tools.
    ...
    def __init__(self, tools_py_path: Optional[str] = None):
        ...
        self._xoneai_tools_available: Optional[bool] = None
    ...
    def _resolve_from_xoneaiagents(self, name: str) -> Optional[Callable]:
        """Resolve tool from xoneaiagents.tools.TOOL_MAPPINGS.
        
        Uses lazy loading via __getattr__ in xoneaiagents.tools.
        ...
        """
        try:
            from xoneaiagents import tools as agent_tools
            ...
            if hasattr(agent_tools, 'TOOL_MAPPINGS'):
                if name in agent_tools.TOOL_MAPPINGS:
                    ...
                    if tool is not None:
                        logger.debug(f"Resolved '{name}' from xoneaiagents.tools")
                        return tool
            
            # Also try direct attribute access (for non-TOOL_MAPPINGS items)
            tool = getattr(agent_tools, name, None)
            if tool is not None and callable(tool):
                logger.debug(f"Resolved '{name}' from xoneaiagents.tools (direct)")
                return tool
                
        except ImportError:
            logger.debug("xoneaiagents not available")
        except AttributeError:
            pass
        except Exception as e:
            logger.debug(f"Error resolving '{name}' from xoneaiagents: {e}")
        
        return None
    
    def _resolve_from_xoneai_tools(self, name: str) -> Optional[Callable]:
        """Resolve tool from xoneai-tools package (external).
        ...
        if self._xoneai_tools_available is None:
            self._xoneai_tools_available = importlib.util.find_spec("xoneai_tools") is not None
        
        if not self._xoneai_tools_available:
            return None
        ...
        try:
            ...
            tool = getattr(xoneai_tools, name, None)
            if tool is not None:
                logger.debug(f"Resolved '{name}' from xoneai-tools")
                return tool
                
        except ImportError:
            self._xoneai_tools_available = False
        except AttributeError:
            pass
        except Exception as e:
            logger.debug(f"Error resolving '{name}' from xoneai-tools: {e}")
    ...
    def _resolve_from_registry(self, name: str) -> Optional[Callable]:
        ...
        """
        try:
            from xoneaiagents.tools.registry import get_tool
            tool = get_tool(name)
            if tool is not None:
    ...
    def resolve(self, name: str) -> Optional[Callable]:
        ...
        2. xoneaiagents.tools.TOOL_MAPPINGS (built-in)
        3. xoneai-tools package (external, optional)
        ...
        # 2. Check xoneaiagents.tools
        tool = self._resolve_from_xoneaiagents(name)
        if tool is not None:
            return tool
        
        # 3. Check xoneai-tools package
        tool = self._resolve_from_xoneai_tools(name)
        if tool is not None:
            return tool
    ...
    def list_available(self) -> Dict[str, str]:
        ...
        # 2. Add xoneaiagents.tools
        try:
            from xoneaiagents.tools import TOOL_MAPPINGS
            for name in TOOL_MAPPINGS.keys():
                if name not in available:
                    available[name] = "Built-in tool from xoneaiagents"
        except ImportError:
            pass
        
        # 3. Add xoneai-tools (if installed)
        if self._xoneai_tools_available is None:
            self._xoneai_tools_available = importlib.util.find_spec("xoneai_tools") is not None
        
        if self._xoneai_tools_available:
            try:
                from xoneai_tools import __all__ as xoneai_tools_all
                for name in xoneai_tools_all:
                    if name not in available:
                        available[name] = "External tool from xoneai-tools"

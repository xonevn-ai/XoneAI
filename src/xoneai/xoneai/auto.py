"""Auto-generation module for XoneAI agents and workflows.
...
_xoneai_available = None
_xoneai_tools_available = None
...
_xoneai_classes = None  # (XoneAgent, XoneTask, Agents)
_xoneai_tools = None  # dict of tool classes
_litellm = None
_openai_client = None
...
# --- XoneAI Agents lazy loading ---
def _check_xoneai_available() -> bool:
    """Check if xoneaiagents is available (cached)."""
    global _xoneai_available
    if _xoneai_available is None:
        try:
            import xoneaiagents  # noqa: F401
            _xoneai_available = True
        except ImportError:
            _xoneai_available = False
    return _xoneai_available


def _get_xoneai():
    """Lazy load xoneaiagents classes."""
    global _xoneai_classes
    if _xoneai_classes is None:
        from xoneaiagents import Agent as XoneAgent, Task as XoneTask, Agents
        _xoneai_classes = (XoneAgent, XoneTask, Agents)
    return _xoneai_classes


# --- XoneAI Tools lazy loading ---
def _check_xoneai_tools_available() -> bool:
    """Check if xoneai_tools is available (cached)."""
    global _xoneai_tools_available
    if _xoneai_tools_available is None:
        try:
            import xoneai_tools  # noqa: F401
            _xoneai_tools_available = True
        except ImportError:
            _xoneai_tools_available = False
    return _xoneai_tools_available


def _get_xoneai_tools():
    """Lazy load xoneai_tools classes."""
    global _xoneai_tools
    if _xoneai_tools is None:
        from xoneai_tools import (
...
# =============================================================================
# Available Tools List (shared between generators) - Legacy for xoneai_tools
# =============================================================================
AVAILABLE_TOOLS = [
    "CodeDocsSearchTool", "CSVSearchTool", "DirectorySearchTool", "DOCXSearchTool",
    "DirectoryReadTool", "FileReadTool", "TXTSearchTool", "JSONSearchTool",
...
# =============================================================================
# Enhanced Tool Discovery from xoneaiagents.tools
# =============================================================================

# Tool categories with their tools from xoneaiagents.tools
TOOL_CATEGORIES = {
    'web_search': [
...
class AutoGenerator(BaseAutoGenerator):
    ...
    def __init__(self, topic="Movie Story writing about AI", agent_file="test.yaml", 
        ...
        Args:
            ...
            framework: Framework to use (crewai, autogen, xoneai)
...
AutoGen is not installed. Please install with:
    pip install "xoneai[autogen]" for v0.2
    pip install "xoneai[autogen-v4]" for v0.4
""")
        elif framework == "xoneai" and not _check_xoneai_available():
            raise ImportError("""
Xoneai is not installed. Please install with:
    pip install xoneaiagents
""")

        # Only show tools message if using a framework and tools are needed
        if (framework in ["crewai", "autogen"]) and not _check_xoneai_tools_available():
...
Tools are not available for {framework}. To use tools, install:
    pip install "xoneai[{framework}]"
""")

        self.topic = topic
        self.agent_file = agent_file
        self.framework = framework or "xoneai"
...
class WorkflowAutoGenerator(BaseAutoGenerator):
    ...
    def __init__(self, topic: str = "Research and write about AI", 
        ...
        Args:
            ...
            config_list: Optional LLM configuration
            framework: Framework to use (xoneai, crewai, autogen)

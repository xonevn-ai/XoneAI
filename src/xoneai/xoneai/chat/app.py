"""
XoneAI Chat - Default Chainlit Application

This is the default chat application that runs when using `xoneai chat`.
It integrates with XoneAI agents for multi-agent conversations.
"""

import os
import chainlit as cl

# Check if we're in XoneAI Chat mode with registered agents
XONEAI_CHAT_MODE = os.environ.get("XONEAI_CHAT_MODE", "false").lower() == "true"


def get_agents():
    """Get registered agents from the chat module."""
    try:
        from xoneai.chat import get_registered_agents
...
async def on_chat_start():
    ...
    if agents:
        ...
        await cl.Message(
            content=f"Welcome to XoneAI Chat! Available agents: {', '.join(agent_names)}"
        ).send()
    else:
        await cl.Message(
            content="Welcome to XoneAI Chat! No agents configured. "

...
class DependencyChecker:
    ...
    Supports checking:
    - Tools (from built-in registry, xoneai-tools, custom dirs)
    ...
    TOOL_INSTALL_HINTS = {
        "youtube_tool": "pip install xoneai-tools[video]",
        "whisper_tool": "pip install xoneai-tools[audio]",
        "vision_tool": "pip install xoneai-tools[vision]",
    ...
    def check_tool(self, tool_name: str) -> Dict[str, Any]:
        ...
        # Check xoneai-tools
        source = self._check_xoneai_tools(tool_name)
        if source:
            status = DependencyStatus(
                name=tool_name,
                available=True,
        ...
        if not install_hint:
            install_hint = "Check xoneai-tools or define custom tool"
    ...
    def _check_xoneai_tools(self, tool_name: str) -> Optional[str]:
        """Check if tool exists in xoneai-tools package."""
        try:
            import xoneai_tools
            if hasattr(xoneai_tools, tool_name):
                return "xoneai-tools"
            # Check for get_tool function
            if hasattr(xoneai_tools, 'get_tool'):
                try:
                    tool = xoneai_tools.get_tool(tool_name)
                    if tool:
                        return "xoneai-tools"
                except Exception:
                    pass

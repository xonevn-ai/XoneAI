...
class TemplateResolver:
    ...
    - xone://local/./path
    - xone://package/name/template
    - xone://github/owner/repo/template@ref
    """
    
    # URI patterns
    GITHUB_PATTERN = re.compile(
        r'^(?:github:|xone://github/)([^/]+)/([^/]+)/([^@]+)(?:@(.+))?$'
    )
    PACKAGE_PATTERN = re.compile(
        r'^(?:package:|xone://package/)([^/]+)/(.+)$'
    )
    ...
    def resolve(cls, uri: str) -> ResolvedTemplate:
        ...
        if cls.HTTP_PATTERN.match(uri):
            # Extract actual URL
            url = uri
            if uri.startswith("xone://"):
                url = uri.replace("xone://", "")
            return ResolvedTemplate(
        ...
        if cls.LOCAL_PATTERN.match(uri) or os.path.exists(uri) or uri.startswith(("./", "../", "~/")):
            path = uri
            if uri.startswith("xone://local/"):
                path = uri.replace("xone://local/", "")
            # Expand user home

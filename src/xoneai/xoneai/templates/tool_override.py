"""
Tool Override Loader for XoneAI.
...
def create_tool_registry_with_overrides(
    ...
    5. Default custom dirs (~/.xone/tools, etc.)
    6. Package discovery (xoneai-tools if installed)
    7. Built-in tools
    ...
    # 6. Package discovery - try xoneai-tools if installed
    try:
        import xoneai_tools.tools as external_tools
        # Get all exported tools from xoneai_tools
    ...
    for tool in tool_names:
        if callable(tool):
            ...
            if tool_name in registry:
                ...
                # They should be resolved via xoneaiagents.tools.__getattr__
                if isinstance(tool_obj, tuple):
                    # Try to import from xoneaiagents.tools instead
                    try:
            ...
            if not found:
                # Try to import from xoneaiagents.tools
                try:
                    from xoneaiagents import tools as agent_tools
                    if hasattr(agent_tools, tool_name):
                        resolved.append(getattr(agent_tools, tool_name))

...
Bridges xoneaiagents.tools registry/lazy TOOL_MAPPINGS into
xoneai.mcp_server registry WITHOUT duplicating tool definitions
...
def is_bridge_available() -> bool:
    """Check if xoneaiagents.tools is available."""
    try:
        import importlib.util
        spec = importlib.util.find_spec("xoneaiagents.tools")
...
def get_tool_mappings() -> Dict[str, Any]:
    """Get TOOL_MAPPINGS from xoneaiagents.tools."""
    try:
...
def _infer_tool_hints(tool_name: str) -> Dict[str, bool]:
    ...
    Args:
        tool_name: Full tool name (e.g., "xoneai.memory.show")
...
def _extract_category(tool_name: str) -> Optional[str]:
    ...
    if len(parts) >= 2:
        return parts[-2]  # e.g., "xoneai.memory.show" -> "memory"
    return None


def register_xoneai_tools(
    namespace_prefix: str = "xoneai.agents.",
    skip_on_collision: bool = True,
) -> int:
    """
    Bridge xoneaiagents tools to MCP registry.
    ...
    if not is_bridge_available():
        logger.debug("xoneaiagents.tools not available, skipping bridge")
        return 0
    ...
    for tool_name, mapping_info in tool_mappings.items():
        ...
        try:
            ...
            tool_def = MCPToolDefinition(
                name=full_name,
                description=f"XoneAI Agents tool: {tool_name}",
                handler=handler,
    ...
    logger.info(f"Registry bridge registered {registered_count} tools from xoneaiagents")

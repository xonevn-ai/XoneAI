...
Maps XoneAI capabilities (audio, images, embeddings, etc.) to MCP tools.
"""
...
def register_capability_tools() -> None:
    ...
    # Audio tools
    @register_tool("xoneai.audio.transcribe")
    ...
    @register_tool("xoneai.audio.speech")
    def audio_speech(
        text: str,
        model: str = "tts-1",
        voice: str = "alloy",
    ...
    # Image tools
    @register_tool("xoneai.images.generate")
    def images_generate(
        prompt: str,
        model: str = "dall-e-3",
        size: str = "1024x1024",
    ...
    # Embedding tools
    @register_tool("xoneai.embed.create")
    def embed_create(
        text: str,
    ...
    # Moderation tools
    @register_tool("xoneai.moderate.check")
    ...
    # Chat completion
    @register_tool("xoneai.chat.completion")
    def chat_completion(
        message: str,
        ...
        from xoneai.capabilities import chat_completion
        messages = []
        if system_prompt:
    ...
    # Search
    @register_tool("xoneai.search")
    def search_web(query: str) -> str:
        """Search the web."""
        from xoneai.capabilities import search
        result = search(query=query)
        return str(result)
    
    # Guardrails
    @register_tool("xoneai.guardrails.check")

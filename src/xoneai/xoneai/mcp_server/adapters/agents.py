...
Maps XoneAI agent operations to MCP tools.
"""
...
def register_agent_tools() -> None:
    ...
    @register_tool("xoneai.agent.chat")
    def agent_chat(
        message: str,
        model: str = "gpt-4o-mini",
        instructions: Optional[str] = None,
    ) -> str:
        """Chat with a XoneAI agent."""
        try:
            ...
            return "Error: xoneaiagents not installed"
        except Exception as e:
            return f"Error: {e}"
    
    @register_tool("xoneai.agent.run")
    def agent_run(
        task: str,
        model: str = "gpt-4o-mini",
        instructions: Optional[str] = None,
    ) -> str:
        """Run a task with a XoneAI agent."""
        try:
            from xoneaiagents import Agent
    ...
    @register_tool("xoneai.workflow.run")
    def workflow_run(
        yaml_content: str,
    ) -> str:
    ...
    @register_tool("xoneai.workflow.run_file")
    ...
    @register_tool("xoneai.research.run")
    def research_run(
        topic: str,
        depth: str = "medium",
    ) -> str:

...
- xoneai mcp serve --transport stdio|http-stream
- xoneai mcp list-tools
- xoneai mcp config-generate
"""
...
class MCPServerCLI:
    ...
    def _print_help(self) -> None:
        """Print help message."""
        help_text = """
[bold cyan]XoneAI MCP Server (Protocol Version 2025-11-25)[/bold cyan]

Run XoneAI as an MCP server for Claude Desktop, Cursor, Windsurf, and other MCP clients.

[bold]Usage:[/bold]
  xoneai mcp <command> [options]
...
[bold]Serve Options:[/bold]
  ...
  --name <name>           Server name (default: xoneai)
...
[bold]Examples:[/bold]
  ...
  xoneai mcp serve --transport http-stream --port 8080 --api-key mykey

  # Start with custom allowed origins
  xoneai mcp serve --transport http-stream --allowed-origins "http://localhost:3000"
...
[bold]Recipe Server Examples:[/bold]
  ...
  xoneai mcp serve-recipe support-reply --transport stdio

  # Serve with HTTP Stream transport
  xoneai mcp serve-recipe ai-video-editor --transport http-stream --port 8080

  # Generate config for recipe server
  xoneai mcp config-generate-recipe support-reply --client claude-desktop
"""
    ...
    def cmd_serve(self, args: List[str]) -> int:
        ...
        parser = argparse.ArgumentParser(prog="xoneai mcp serve")
        ...
        parser.add_argument("--name", default="xoneai")
        ...
        try:
            ...
            server = MCPServer(
                name=parsed.name,
                instructions="XoneAI MCP Server - AI agent capabilities exposed via MCP protocol.",
            )
            ...
            print("Install with: pip install xoneai[mcp]", file=sys.stderr)
            return self.EXIT_ERROR
    ...
    def cmd_list_tools(self, args: List[str]) -> int:
        ...
        parser = argparse.ArgumentParser(prog="xoneai mcp list-tools")
        parser.add_argument("--json", action="store_true", help="Output in JSON format")
    ...
    def _print_tools_help(self) -> None:
        ...
        help_text = """
[bold cyan]XoneAI MCP Tools Management[/bold cyan]

[bold]Usage:[/bold]
  xoneai mcp tools <subcommand> [options]
...
[bold]Search Options:[/bold]
  xoneai mcp tools search "<query>" [options]
...
[bold]Info Options:[/bold]
  xoneai mcp tools info <tool-name>
    --json              Output in JSON format

[bold]Schema Options:[/bold]
  xoneai mcp tools schema <tool-name>
    --json              Output in JSON format (default)

[bold]Examples:[/bold]
  # Search for web-related tools
  xoneai mcp tools search "web"

  # Search read-only tools in memory category
  xoneai mcp tools search --category memory --read-only

  # Get tool info
  xoneai mcp tools info xoneai.memory.show

  # Get tool schema
  xoneai mcp tools schema xoneai.workflow.run
"""
        self._print_rich(help_text)
    
    def cmd_tools_search(self, args: List[str]) -> int:
        """Search tools by query, category, or tags."""
        parser = argparse.ArgumentParser(prog="xoneai mcp tools search")
    ...
    def cmd_tools_info(self, args: List[str]) -> int:
        ...
        parser = argparse.ArgumentParser(prog="xoneai mcp tools info")
    ...
    def cmd_tools_schema(self, args: List[str]) -> int:
        ...
        parser = argparse.ArgumentParser(prog="xoneai mcp tools schema")
        parser.add_argument("name", help="Tool name")
    ...
    def cmd_config_generate(self, args: List[str]) -> int:
        ...
        parser = argparse.ArgumentParser(prog="xoneai mcp config-generate")
    ...
    def cmd_doctor(self, args: List[str]) -> int:
        ...
        try:
            ...
            print("\n[bold cyan]XoneAI MCP Server Health Check[/bold cyan]\n")
            ...
            if all_deps and any_api_key:
                ...
                self._print_error("Missing required dependencies. Install with: pip install xoneai[mcp]")
                return self.EXIT_ERROR
            else:

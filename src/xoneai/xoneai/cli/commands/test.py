"""
XoneAI Test CLI Command

Provides a unified interface for running tests with tier and provider options.

Usage:
    xoneai test                      # Run main tier (default)
    xoneai test --tier smoke         # Run smoke tests only
    xoneai test --tier extended      # Run extended tests
    xoneai test --provider openai    # Run OpenAI provider tests
    xoneai test --live               # Enable live API tests
    xoneai test --parallel auto      # Run with parallelization
    
    # Interactive mode tests
    xoneai test interactive --csv tests.csv
    xoneai test interactive --suite smoke
    xoneai test interactive --list
    xoneai test interactive --generate-template
"""
...
app = typer.Typer(help="Run XoneAI test suite with tier and provider options")
...
def _set_environment(tier: str, provider: Optional[str], live: bool):
    ...
    os.environ["XONEAI_TEST_TIER"] = tier
    
    if live:
        os.environ["XONEAI_ALLOW_NETWORK"] = "1"
        os.environ["XONEAI_LIVE_TESTS"] = "1"
    else:
        os.environ["XONEAI_ALLOW_NETWORK"] = "0"
        os.environ["XONEAI_LIVE_TESTS"] = "0"
    
    if provider:
        os.environ["XONEAI_TEST_PROVIDERS"] = provider
...
def run(
    ...
    """
    Run the XoneAI test suite.
    
    Examples:
        xoneai test                      # Run main tier
        xoneai test --tier smoke         # Fast smoke tests
        xoneai test --tier extended      # Extended tests
        xoneai test --provider openai --live  # OpenAI live tests
        xoneai test --parallel auto      # Parallel execution
    """
    # Validate tier
    ...
    # Try current directory first, then src/xoneai
    test_dirs = [
        os.getcwd(),
        os.path.join(os.getcwd(), "src", "xoneai"),
...
def _get_builtin_suite_path(suite: str):
    ...
    package_dir = Path(__file__).parent.parent.parent.parent  # Up to src/xoneai
...
def interactive(
    ...
    Examples:
        xoneai test interactive --csv tests.csv
        xoneai test interactive --suite smoke
        xoneai test interactive --suite tools --keep-artifacts
        xoneai test interactive --list
        xoneai test interactive --generate-template
    """
    from pathlib import Path
    ...
    if list_suites:
        ...
        typer.echo("")
        typer.echo("Usage: xoneai test interactive --suite <name>")
        typer.echo("       XONEAI_LIVE_INTERACTIVE=1 xoneai test interactive --suite advanced")
        return
    
    # Generate template
    if generate_template:
        # Lazy import
        from xoneai.cli.features.csv_test_runner import generate_csv_template
    ...
    if not csv and not suite:
        ...
        typer.echo("")
        typer.echo("Examples:")
        typer.echo("  xoneai test interactive --csv tests.csv")
        typer.echo("  xoneai test interactive --suite smoke")
        typer.echo("  xoneai test interactive --suite advanced")
        typer.echo("  xoneai test interactive --list")
        raise typer.Exit(1)
    ...
    # Lazy import runner
    from xoneai.cli.features.csv_test_runner import CSVTestRunner
...
def _run_advanced_suite(
    ...
    live_enabled = os.environ.get("XONEAI_LIVE_INTERACTIVE", "0") == "1"
    has_api_key = bool(os.environ.get("OPENAI_API_KEY"))
    
    if not live_enabled:
        typer.echo("\n⚠️  Cannot run Advanced tests: XONEAI_LIVE_INTERACTIVE=1 not set")
        typer.echo("")
        typer.echo("Prerequisites:")
        typer.echo("  1. Set XONEAI_LIVE_INTERACTIVE=1")
...
def info():
    ...
    typer.echo("📊 XoneAI Test Suite Information")
    typer.echo("=" * 50)
    typer.echo("")
    ...
    env_vars = {
        "XONEAI_TEST_TIER": os.environ.get("XONEAI_TEST_TIER", "main"),
        "XONEAI_ALLOW_NETWORK": os.environ.get("XONEAI_ALLOW_NETWORK", "0"),
        "XONEAI_LIVE_TESTS": os.environ.get("XONEAI_LIVE_TESTS", "0"),
        "XONEAI_TEST_PROVIDERS": os.environ.get("XONEAI_TEST_PROVIDERS", "openai"),
        "XONEAI_LOCAL_SERVICES": os.environ.get("XONEAI_LOCAL_SERVICES", "0"),
    }

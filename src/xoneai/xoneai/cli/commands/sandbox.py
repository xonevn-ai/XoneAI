"""
Sandbox command group for XoneAI CLI.
...
def sandbox_status(
    ...
    Examples:
        xoneai sandbox status
        xoneai sandbox status --json
    """
    try:
...
def sandbox_explain(
    ...
    Examples:
        xoneai sandbox explain
        xoneai sandbox explain --session main
        xoneai sandbox explain --agent work
    """
    try:
        from rich.console import Console
...
def sandbox_list(
    ...
    Examples:
        xoneai sandbox list
        xoneai sandbox list --browser
        xoneai sandbox list --json
    """
    try:
...
def sandbox_recreate(
    ...
    Examples:
        xoneai sandbox recreate --all
        xoneai sandbox recreate --session main
        xoneai sandbox recreate --agent mybot
        xoneai sandbox recreate --browser --force
    """
    ...
    if info["docker_available"]:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "ps", "--filter", "name=xoneai-sbx-", "--format", "{{.Names}}\t{{.Status}}"],
                capture_output=True,
                text=True,
                timeout=5,
            )
...
def _get_sandbox_containers(browser_only: bool = False):
    ...
    try:
        ...
        filter_name = "xoneai-browser-" if browser_only else "xoneai-sbx-"
        result = subprocess.run(
...
def sandbox_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]XoneAI Sandbox - Container Management[/bold cyan]

Manage sandbox containers with: xoneai sandbox <command>
...
[bold]Examples:[/bold]
  ...
  xoneai sandbox explain --agent work
  xoneai sandbox list --browser
  xoneai sandbox recreate --all --force
"""
        try:

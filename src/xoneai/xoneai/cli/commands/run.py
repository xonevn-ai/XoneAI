"""
Run command group for XoneAI CLI.
...
def run_main(
    ...
    framework: Optional[str] = typer.Option(None, "--framework", "-f", help="Framework: xoneai, crewai, autogen"),
    ...
    Examples:
        xoneai run agents.yaml
        xoneai run "What is the weather?"
        xoneai run agents.yaml --interactive
        xoneai run "What is 2+2?" --profile
    """
    ...
    if not target:
        output.print_panel(
            ...
            "  --framework, -f   Framework (xoneai, crewai, autogen)\n"
...
def _run_from_file(
    ...
    try:
        # Use existing XoneAI class
        from xoneai.cli.main import XoneAI
        
        xone = XoneAI(
            agent_file=file_path,
            framework=framework or "xoneai",
        )
        
        # Set model if provided
        if model:
            xone.config_list[0]['model'] = model
...
def _run_prompt(
    ...
    try:
        ...
        if output_mode == "actions":
            ...
            if approval:
                from xoneai.cli.features.approval import resolve_approval_config
        ...
        xone.args = args
        result = xone.handle_direct_prompt(prompt)

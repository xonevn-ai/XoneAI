"""
Plugins command group for XoneAI CLI.
...
def plugins_list(
    ...
    Examples:
        xoneai plugins list
        xoneai plugins list --enabled
        xoneai plugins list --json
    """
    try:
...
def plugins_info(
    ...
    Examples:
        xoneai plugins info memory-core
        xoneai plugins info browser-tool
    """
    try:
        plugins = _get_available_plugins()
...
def plugins_enable(
    ...
    Examples:
        xoneai plugins enable memory-core
        xoneai plugins enable browser-tool
    """
    try:
...
def plugins_disable(
    ...
    Examples:
        xoneai plugins disable memory-core
        xoneai plugins disable browser-tool
    """
    try:
        config_path = _get_config_path()
        config = _load_config(config_path)
...
def plugins_create(
    ...
    description: str = typer.Option("A XoneAI plugin", "--description", "-d", help="Plugin description"),
    author: str = typer.Option("", "--author", "-a", help="Plugin author"),
    output_dir: str = typer.Option(None, "--output", "-o", help="Output directory (default: ~/.xone/plugins/)"),
):
    """Create a new single-file plugin from template.
    
    Creates a new plugin with WordPress-style docstring header in the
    user's plugin directory (~/.xone/plugins/).
    
    Examples:
        xoneai plugins create weather
        xoneai plugins create weather --description "Weather API tools"
        xoneai plugins create weather -o ./my_plugins/
    """
    try:
        from pathlib import Path
        from xoneaiagents.plugins.discovery import get_plugin_template, ensure_plugin_dir
        ...
        typer.echo(f"  2. Run 'xoneai plugins discover' to verify")
        typer.echo(f"  3. Use tools with Agent(tools=['example_tool'])")
        
    except ImportError as e:
        typer.echo(f"Error: xoneaiagents package not found. Install with: pip install xoneaiagents", err=True)
        raise typer.Exit(1)
    except Exception as e:
...
def plugins_install(
    ...
    Copies a plugin file to the user's plugin directory (~/.xone/plugins/).
    
    Examples:
        xoneai plugins install ./my_weather_plugin.py
        xoneai plugins install ./my_weather_plugin.py --force
    """
    try:
        from pathlib import Path
        import shutil
        from xoneaiagents.plugins.discovery import ensure_plugin_dir
        from xoneaiagents.plugins.parser import parse_plugin_header_from_file
        ...
        typer.echo(f"Error: xoneaiagents package not found.", err=True)
        raise typer.Exit(1)
    except typer.Exit:
        raise
    except Exception as e:
...
def plugins_discover(
    ...
    Scans ~/.xone/plugins/ and ./.xone/plugins/ for valid plugins.
    
    Examples:
        xoneai plugins discover
        xoneai plugins discover --load
        xoneai plugins discover --json
    """
    try:
        from xoneaiagents.plugins.discovery import discover_plugins, discover_and_load_plugins
        ...
        if json_output:
            ...
            if not plugins:
                ...
                console.print("\nCreate one with: xoneai plugins init <name>")
                return
...
def plugins_remove(
    ...
    Examples:
        xoneai plugins remove weather
        xoneai plugins remove weather_plugin.py --force
    """
    try:
        from pathlib import Path
        from xoneaiagents.plugins.discovery import get_default_plugin_dirs
        ...
        if not plugin_path:
            user_dir = Path.home() / ".xone" / "plugins"
...
def _get_available_plugins():
    ...
    try:
        from xoneaiagents.plugins import get_plugin_registry
...
def plugins_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]XoneAI Plugins - Plugin Management[/bold cyan]

Manage plugins with: xoneai plugins <command>

"""
Replay command group for XoneAI CLI.
...
def replay_list(
    ...
    try:
        from xoneai.replay import list_traces
    ...
    output.print("\nUse 'xoneai replay context <session_id>' to replay a trace.")
...
def replay_context(
    ...
    try:
        from xoneai.replay import ContextTraceReader, ReplayPlayer
        from xoneai.replay.analyzer import TraceAnalyzer, format_stats_output, format_cost_output
    except ImportError:
        output.print_error("Replay module not available")
        raise typer.Exit(1)
    ...
    if not reader.exists:
        ...
        output.print_info("Use 'xoneai replay list' to see available traces.")
        raise typer.Exit(1)
...
def _dump_events(output, events, session_id: str, json_output: bool, full: bool = False):
    ...
    from xoneai.replay.analyzer import TraceAnalyzer
...
def replay_show(
    ...
    try:
        from xoneai.replay import ContextTraceReader
    except ImportError:
        output.print_error("Replay module not available")
        raise typer.Exit(1)
...
def replay_dashboard(
    ...
    try:
        from xoneai.replay import list_traces, ContextTraceReader
        from xoneai.replay.analyzer import TraceAnalyzer
    except ImportError:
        output.print_error("Replay module not available")
        raise typer.Exit(1)
...
def replay_cleanup(
    ...
    try:
        from xoneai.replay.storage import cleanup_old_traces, list_traces
    except ImportError:
        output.print_error("Replay module not available")
        raise typer.Exit(1)

"""
RAG command group for XoneAI CLI.
...
def rag_profiler(enabled: bool, profile_out: Optional[Path], profile_top: int = 20):
    ...
    Uses the existing xoneai.profiler infrastructure when available,
    ...
    profiler = None
    try:
        from xoneai.profiler import Profiler
        Profiler.enable()
        Profiler.clear()
        profiler = Profiler
    except ImportError:
...
def rag_index(
    ...
    Equivalent to: xoneai knowledge index
    
    Examples:
        xoneai rag index ./docs
        xoneai rag index paper.pdf --collection research
        xoneai rag index ./data --chunking semantic --chunk-size 256
        xoneai rag index ./docs --profile --profile-out ./profile.json
    """
    from rich.console import Console
    ...
    with rag_profiler(profile, profile_out, profile_top) as profile_data:
        try:
            ...
            knowledge_config = {
                "vector_store": {
                    ...
                    "config": {
                        ...
                        "path": f"./.xone/knowledge/{collection}",
                    }
                }
            }
            ...
            console.print("Install with: pip install 'xoneaiagents[knowledge]'")
            raise typer.Exit(1)
        except Exception as e:
...
def rag_query(
    ...
    Examples:
        xoneai rag query "What is the main finding?"
        xoneai rag query "Summarize the document" --collection research
        xoneai rag query "Key points?" --top-k 10 --no-citations
        xoneai rag query "Summary?" --hybrid --rerank
...
def rag_chat(
    ...
    Examples:
        xoneai rag chat
        xoneai rag chat --collection research
        xoneai rag chat --hybrid --rerank
        xoneai rag chat --no-stream
    """
    from rich.console import Console
    from rich.panel import Panel
    from rich.markdown import Markdown
...
def rag_eval(
    ...
    Examples:
        xoneai rag eval golden_queries.json
        xoneai rag eval tests.json --collection research --output results.json
        xoneai rag eval tests.json --profile --profile-out ./profile.json
    """
    import json
    from rich.console import Console
...
def rag_serve(
    ...
    DEPRECATED: Use `xoneai serve rag` instead.
    ...
    Examples:
        xoneai rag serve
        xoneai rag serve --collection research --port 9000
        xoneai rag serve --hybrid --rerank
        xoneai rag serve --openai-compat --port 8080
        xoneai rag serve --profile --profile-out ./profile.json
    """
    import sys
    
    # Print deprecation warning
    print("\n\033[93m⚠ DEPRECATION WARNING:\033[0m", file=sys.stderr)
    print("\033[93m'xoneai rag serve' is deprecated and will be removed in a future version.\033[0m", file=sys.stderr)
    print("\033[93mPlease use 'xoneai serve rag' instead.\033[0m\n", file=sys.stderr)
    ...
    with rag_profiler(profile, profile_out, profile_top) as profile_data:
        try:
            ...
            console.print("Install with: pip install 'xoneai[rag-api]'")
            raise typer.Exit(1)

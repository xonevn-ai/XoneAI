...
def _get_tools(tool_names: List[str]) -> List:
    ...
    resolved_names = set()
    try:
        from xoneaiagents import tools as tool_module
        ...
        console.print("[red]Error: xoneaiagents not installed[/red]")
...
def _run_tracked_task(
    ...
    # The decorator checks XONEAI_AUTO_APPROVE env var independently of
    # the Agent's approval= parameter.
    # ========================================================================
    prev_auto_approve = os.environ.get("XONEAI_AUTO_APPROVE")
    if auto_approve:
...
When you have fully completed the task, say 'Task completed' or 'Done'.""",
    ...
    try:
        ...
        # Restore original XONEAI_AUTO_APPROVE env var
        if prev_auto_approve is None:
            os.environ.pop("XONEAI_AUTO_APPROVE", None)
        else:
            os.environ["XONEAI_AUTO_APPROVE"] = prev_auto_approve
...
def tracker_main(ctx: typer.Context):
    ...
    Commands:
        ...
        xoneai tracker run "Search for Python best practices"
        
        xoneai tracker judge "What is 2+2?" --expected "4"
        
        xoneai tracker tools
        
        xoneai tracker batch tasks.json
    """
    pass
...
def tracker_run(
    ...
    Examples:
        ...
        xoneai tracker run "Search for Python best practices and summarize"
        
        xoneai tracker run "Read config.yaml and explain its structure" -v
        
        xoneai tracker run "Find trending AI news" --tools search_web,web_crawl
    """
...
def tracker_batch(
    ...
    Example:
        ...
        xoneai tracker batch tasks.json -o results.json
    """
    import json
...
def tracker_tools():
    ...
    console.print("[dim]  xoneai tracker 'task' --tools search_web,read_file[/dim]")
...
def tracker_judge(
    ...
    Examples:
        ...
        xoneai tracker judge "Calculate fibonacci(10) using execute_code"
        
        xoneai tracker judge "Search for AI news" --criteria "Must use search_web"
        
        xoneai tracker judge "What is 2+2?" --expected "4" --threshold 8.0
    """
    # Resolve tools
    tool_names = AUTONOMY_DEFAULT_TOOLS.copy()
    if extended:
    ...
    try:
        ...
        console.print("[red]Error: xoneaiagents.eval not available[/red]")
        console.print("[dim]Install with: pip install xoneaiagents[/dim]")
    except Exception as e:

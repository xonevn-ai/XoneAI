...
- `xoneai index` - Index documents into knowledge base
- `xoneai query` - Query with structured answer and citations
- `xoneai chat` - Conversational interface with knowledge (handled by chat.py)
...
def index_command(
    ...
    Use `xoneai query` to get answers with citations from the indexed knowledge.
    
    Examples:
        xoneai index ./docs
        xoneai index paper.pdf --collection research
        xoneai index ./data --profile --profile-out ./profile.json
    """
    from rich.console import Console
...
def query_command(
    ...
    Examples:
        xoneai query "What is the main finding?"
        xoneai query "Summarize the document" --collection research
        xoneai query "Key points?" --top-k 10 --no-citations
        xoneai query "Summary?" --hybrid --rerank
    """
    from rich.console import Console
    ...
    with retrieval_profiler(profile, profile_out, profile_top) as profile_data:
        try:
            ...
            from xoneaiagents.rag.retrieval_config import RetrievalConfig, RetrievalPolicy, CitationsMode
            ...
            retrieval_config = RetrievalConfig(
                ...
                persist_path=f"./.xone/knowledge/{collection}",
                collection_name=collection,
            )
...
def search_command(
    ...
    For answers with citations, use `xoneai query`.
    
    Examples:
        xoneai search "capital of France"
        xoneai search "main findings" --collection research --top-k 10
    """
    from rich.console import Console

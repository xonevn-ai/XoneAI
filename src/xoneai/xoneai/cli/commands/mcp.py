"""
MCP command group for XoneAI CLI.
...
def mcp_list(
    ...
    if not servers:
        ...
        output.print("\nAdd a server with: xoneai mcp add <name> <command>")
        return
...
def mcp_sync(
    ...
    for srv in servers_to_sync:
        ...
        try:
            ...
            output.print_error("  MCP package not installed. Install with: pip install xoneaiagents[mcp]")
    ...
    try:
        ...
        if server:
            ...
            if not tools:
                ...
                output.print("\nRun 'xoneai mcp sync' to index tools")
                return
        else:
...
def mcp_run(
    ...
    DEPRECATED: Use `xoneai serve mcp --name <server>` instead.
    ...
    Examples:
        xoneai mcp run my-server
        xoneai mcp run my-server --transport sse --port 8080
    """
    import sys
    
    # Print deprecation warning
    print("\n\033[93m⚠ DEPRECATION WARNING:\033[0m", file=sys.stderr)
    print("\033[93m'xoneai mcp run' is deprecated and will be removed in a future version.\033[0m", file=sys.stderr)
    print("\033[93mPlease use 'xoneai serve mcp --name <server>' instead.\033[0m\n", file=sys.stderr)
    ...
    try:
        ...
        if transport == "stdio":
            ...
            # Try to use xoneaiagents MCP server wrapper
            try:
                from xoneaiagents.mcp import MCP
...
def mcp_auth(
    ...
    Examples:
        xoneai mcp auth github
        xoneai mcp auth tavily --timeout 60
    """
    output = get_output_controller()
    loader = get_config_loader()
...
def mcp_logout(
    ...
    Examples:
        xoneai mcp logout github
        xoneai mcp logout tavily --yes
    """
    output = get_output_controller()

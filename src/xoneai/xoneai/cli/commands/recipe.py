"""
Recipe command group for XoneAI CLI.
...
def recipe_create(
    ...
    Examples:
        xoneai recipe create "Build a web scraper for news"
        xoneai recipe create "Research AI trends" --no-optimize
        xoneai recipe create "Research AI" --agents "researcher:role=AI Researcher,goal=Find papers"
        xoneai recipe create "Research AI" --tools "researcher:internet_search,arxiv"
    """
...
def recipe_run(
    ...
    Examples:
        xoneai recipe run ai-url-blog-generator --var url="https://example.com/article"
        xoneai recipe run ai-dynamic-blog-generator --var topic="LangGraph 0.3" --var style="coding"
        xoneai recipe run ai-wordpress-post-generator --save --verbose
        xoneai recipe run ai-topic-gatherer --save --name my-test-run --var topic="AI"
    """
    from xoneai.cli.main import XoneAI
    import sys
...
def recipe_info(
    ...
    Examples:
        xoneai recipe info image-to-blog-generator
        xoneai recipe info url-to-blog-generator --verbose
    """
    ...
    try:
        from xoneai.recipe.core import get_template_search_paths
    ...
    if not recipe_path:
        ...
        console.print("[dim]Hint: Run 'xoneai recipe list' to see available recipes[/dim]")
        raise typer.Exit(1)
...
def recipe_judge(
    ...
    Examples:
        xoneai recipe judge run-abc123
        xoneai recipe judge run-abc123 --goal "Analyze image and create blog post"
        xoneai recipe judge run-abc123 --memory
        xoneai recipe judge run-abc123 --knowledge
        xoneai recipe judge run-abc123 --yaml agents.yaml --output plan.yaml
        xoneai recipe judge run-abc123 --dry-run
    """
    ...
    try:
        ...
        if yaml_file:
            ...
            if not dry_run:
                ...
                print(f"   Run 'xoneai recipe apply {output_file}' to apply fixes")
        else:
...
def recipe_optimize(
    ...
    Examples:
        xoneai recipe optimize my-recipe
        xoneai recipe optimize my-recipe "improve error handling"
        xoneai recipe optimize my-recipe --iterations 5 --threshold 9.0
        xoneai recipe optimize my-recipe --input '{"query": "test"}'
        xoneai recipe optimize my-recipe --criteria "Water flow is optimal: no leaks, pressure within range"
    """
    from pathlib import Path
    import logging
    ...
    try:
        from xoneai.cli.features.recipe_optimizer import RecipeOptimizer
        
        # Create custom judge if criteria provided
        custom_judge = None
        if criteria:
            try:
                from xoneaiagents.eval import Judge
                custom_judge = Judge(criteria=criteria)
                print(f"   Custom criteria: {criteria[:50]}{'...' if len(criteria) > 50 else ''}")
            except ImportError:
                print("   ⚠️ Custom criteria requires xoneaiagents. Using default judge.")
...
def recipe_serve(
    ...
    DEPRECATED: Use `xoneai serve recipe` instead.
    
    Serves recipes as REST API endpoints.
    
    Examples:
        xoneai recipe serve
        xoneai recipe serve my-recipe --port 8080
        xoneai recipe serve --host 0.0.0.0 --port 8000
        xoneai recipe serve --config serve.yaml
        xoneai recipe serve --api-key my-secret-key
        xoneai recipe serve --workers 4
    """
    import sys
    
    # Print deprecation warning
    print("\n\033[93m⚠ DEPRECATION WARNING:\033[0m", file=sys.stderr)
    print("\033[93m'xoneai recipe serve' is deprecated and will be removed in a future version.\033[0m", file=sys.stderr)
    print("\033[93mPlease use 'xoneai serve recipe' instead.\033[0m\n", file=sys.stderr)
...
def recipe_apply(
    ...
    Examples:
        xoneai recipe apply judge_plan.yaml
        xoneai recipe apply judge_plan.yaml --confirm
        xoneai recipe apply judge_plan.yaml --fix-ids fix_001,fix_002
        xoneai recipe apply judge_plan.yaml --dry-run
    """
    from xoneai.replay import JudgePlan, PlanApplier
    from pathlib import Path

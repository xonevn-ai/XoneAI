"""
Gateway command group for XoneAI CLI.
...
app = typer.Typer(
    help="Manage the XoneAI Gateway server",
...
def gateway_start(
    ...
    Examples:
        xoneai gateway start
        xoneai gateway start --config gateway.yaml
        xoneai gateway start --agents agents.yaml --port 9000
    """
...
def gateway_status(
    ...
    Examples:
        xoneai gateway status
        xoneai gateway status --port 9000
    """
    from ..features.gateway import GatewayHandler
...
def gateway_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]XoneAI Gateway - Multi-Bot WebSocket Server[/bold cyan]

Manage the gateway server: xoneai gateway <command>

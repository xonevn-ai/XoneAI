"""
Session command group for XoneAI CLI.
...
def _create_backend(backend_type: str, storage_path: Optional[str]):
    ...
    try:
        if backend_type == "file":
            ...
            return FileBackend(storage_dir=storage_path or "~/.xone/sessions")
        elif backend_type == "sqlite":
            from xoneaiagents.storage import SQLiteBackend
            db_path = storage_path or "~/.xone/sessions.db"
...
def session_resume(
    ...
    if not session:
        output.print_error(
            ...
            remediation="Use 'xoneai session list' to see available sessions"
        )
        raise typer.Exit(1)
...
def session_import(
    ...
    try:
        from xoneai.cli.interactive import InteractiveCore

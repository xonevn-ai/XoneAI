"""
Profile command group for XoneAI CLI.
...
def profile_query(
    ...
    Examples:
        ...
        xoneai profile query "Hello" --limit 20
        xoneai profile query "Test" --deep --show-callers --show-callees
        xoneai profile query "Test" --save ./profile_results
    """
    ...
    try:
        ...
        typer.echo("Install xoneaiagents: pip install xoneaiagents", err=True)
        raise typer.Exit(1)
...
def profile_imports():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-X", "importtime", "-c", "import xoneaiagents"],
            capture_output=True,
            text=True,
            timeout=60,
        )
...
def profile_startup():
    ...
    subprocess.run(
        [sys.executable, "-c", "import xoneai; import xoneai.cli"],
        capture_output=True,
        text=True,
    )
...
@app.command("suite")
def profile_suite(
    output_dir: Optional[str] = typer.Option("/tmp/xoneai_profile_suite", "--output", "-o", help="Output directory for results"),
    ...
    Examples:
        xoneai profile suite
        xoneai profile suite --output ./my_profile_results
...
def profile_snapshot(
    ...
    Examples:
        ...
        xoneai profile snapshot v2.0 --format json
    """
    try:
    ...
    if compare:
        ...
        if not baseline_snap:
            typer.echo("❌ No baseline found. Create one with: xoneai profile snapshot --baseline", err=True)
            raise typer.Exit(1)
        ...
        result = run_profile_suite(
            output_dir="/tmp/xoneai_snapshot_compare",
            scenarios=scenarios,
            output="minimal",
        )
        ...
        result = run_profile_suite(
            output_dir="/tmp/xoneai_snapshot",
            scenarios=scenarios,
            verbose=True,
        )
...
def profile_optimize(
    ...
    Examples:
        ...
        xoneai profile optimize --prewarm
        xoneai profile optimize --lite
    """
    try:
        from ..features.profiler import (
            PrewarmManager,
    ...
    if show_config:
        ...
        typer.echo("  XONEAI_LITE_MODE=1")
        typer.echo("  XONEAI_SKIP_TYPE_VALIDATION=1")
        typer.echo("  XONEAI_MINIMAL_IMPORTS=1")
        return
    ...
    if lite:
        ...
        typer.echo("   export XONEAI_LITE_MODE=1")
        typer.echo("   export XONEAI_SKIP_TYPE_VALIDATION=1")
        typer.echo("   export XONEAI_MINIMAL_IMPORTS=1")

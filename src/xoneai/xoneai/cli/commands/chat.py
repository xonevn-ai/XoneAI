"""
Chat command group for XoneAI CLI.
...
def chat_main(
    ...
    debug: bool = typer.Option(False, "--debug", help="Enable debug logging to ~/.xoneai/async_tui_debug.log"),
    ...
    For browser-based chat UI, use: xoneai ui chat
    
    Examples:
        xoneai chat
        xoneai chat "Hello, how are you?"
        xoneai chat --model gpt-4o --memory
        xoneai chat --continue  # Resume last session
        xoneai chat "Summarize this" --file README.md
        xoneai chat "What is 2+2?" --profile
    """
    import os
    
    # Set workspace if provided
    if workspace:
        os.environ["XONEAI_WORKSPACE"] = workspace
    ...
    if (profile or profile_deep) and not prompt:
        ...
        typer.echo("   Use: xoneai chat \"your prompt\" --profile", err=True)
...
def _run_profiled_chat(
    ...
    from xoneai.cli.features.cli_profiler import (
        CLIProfileConfig,
        CLIProfiler,
    )
...
def _run_legacy_terminal_chat(
    ...
    if prompt:
        ...
        xone._run_chat_mode(prompt, args)
    else:
        # Interactive REPL mode - use _start_interactive_mode
        xone._start_interactive_mode(args)

"""
XoneAI Batch CLI Command.

Run all Python files with XoneAI imports in the current directory.
Designed for quick debugging - running multiple local scripts at once.

Usage:
    xoneai batch                    # Run all XoneAI scripts in current dir
    xoneai batch --sub              # Include subdirectories
    xoneai batch --sub --depth 2    # Limit recursion depth
    xoneai batch --parallel         # Run in parallel with async reporting
    xoneai batch list               # List discovered files
    xoneai batch stats              # Show statistics
    xoneai batch report             # View latest report
"""
...
app = typer.Typer(
    help="Run all XoneAI scripts in current folder for quick debugging",
...
def batch_run(
    ...
    """
    Run all XoneAI scripts in the current folder.
    
    Quick debugging tool for running multiple Python files that use
    xoneaiagents or xoneai. Unlike 'xoneai run' (which runs
    agent configs) or 'xoneai examples' (which runs repo examples),
    this command finds and executes local scripts for rapid testing.
    
    Only files containing 'from xoneaiagents' or 'from xoneai'
    are executed. Test files (test_*.py) are excluded by default.
    
    Examples:
        xoneai batch                    # Run all in current dir
        xoneai batch --sub              # Include subdirectories
        xoneai batch --sub --depth 2    # Limit depth
        xoneai batch --parallel         # Run in parallel
        xoneai batch --timeout 120      # Custom timeout
    """
    ...
    from xoneai.suite_runner import BatchSource, SuiteExecutor, RunResult
    ...
    if not items:
        if ci:
            print(f"INFO: No XoneAI scripts found in {search_path}")
        else:
            typer.echo(f"ℹ️ No XoneAI scripts found in {search_path}")
            if not recursive:
    ...
    if not quiet:
        if ci:
            ...
            typer.echo("=" * 60)
            typer.echo("XoneAI Batch Runner")
            typer.echo("=" * 60)
...
def _run_parallel(
    ...
    from xoneai.suite_runner import RunResult, RunReport, SuiteReporter
    from datetime import timezone
    
    if ci:
        print("=" * 60)
        print("XoneAI Batch Runner (Parallel Mode)")
        print("=" * 60)
        print(f"Path: {search_path}")
        ...
        typer.echo("=" * 60)
        typer.echo("XoneAI Batch Runner (Parallel Mode)")
        typer.echo("=" * 60)
        typer.echo(f"Path: {search_path}")
...
def batch_list(
    ...
    """
    List discovered XoneAI scripts without running them.
    
    Examples:
        xoneai batch list
        xoneai batch list --sub
        xoneai batch list --groups
    """
    from xoneai.suite_runner import BatchSource
    ...
    if show_groups:
        ...
        if not groups:
            typer.echo(f"No XoneAI scripts found in {search_path}")
            return
    ...
    typer.echo(f"Found {len(items)} XoneAI scripts in {search_path}\n")
...
def batch_report(
    ...
    Examples:
        xoneai batch report
        xoneai batch report --format failures
        xoneai batch report --dir ~/Downloads/reports/batch/20240115_120000
    """
    import json
    ...
    if not target_dir or not target_dir.exists():
        ...
        typer.echo("   Run 'xoneai batch' first to generate a report.")
        raise typer.Exit(1)

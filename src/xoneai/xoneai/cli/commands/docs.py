"""
Docs command group for XoneAI CLI.

Provides documentation commands including code execution validation.

Usage:
    xoneai docs run                    # Run all Python blocks from docs
    xoneai docs list                   # List discovered code blocks
    xoneai docs run --dry-run          # Extract only, no execution
    xoneai docs cli run-all            # Validate all CLI commands from docs
"""
...
def _get_default_docs_path() -> Path:
    ...
    candidates = [
        Path.home() / "XoneAIDocs" / "docs",
        Path("/Users/xonevn/XoneAIDocs/docs"),
        Path.cwd() / "docs",
...
def docs_run(
    ...
    Examples:
        xoneai docs run
        xoneai docs run --docs-path /path/to/docs --timeout 120
        xoneai docs run --group models --group tools --max-snippets 10
        xoneai docs run --include "quickstart*" --fail-fast
    """
    # Lazy import to avoid loading at CLI startup
    from xoneai.suite_runner import DocsSource, SuiteExecutor, RunResult
    ...
    if not quiet:
        typer.echo("=" * 60)
        typer.echo("XoneAI Docs Code Execution")
        typer.echo("=" * 60)
...
def docs_list(
    ...
    Examples:
        xoneai docs list
        xoneai docs list --groups
        xoneai docs list --group models --code
    """
    from xoneai.suite_runner import DocsSource
...
def docs_stats(
    ...
    Examples:
        xoneai docs stats
        xoneai docs stats --group models --group tools
    """
...
def docs_run_all(
    ...
    Examples:
        xoneai docs run-all
        xoneai docs run-all --sequential
        xoneai docs run-all --workers 8 --timeout 120
    """
    import json
    ...
    from xoneai.suite_runner import RunReport, RunResult, SuiteReporter
...
def docs_generate(
    package: Optional[Path] = typer.Option(
        None,
        "--package", "-p",
        help="Path to xoneaiagents package (default: auto-detect)",
    ),
    output: Optional[Path] = typer.Option(
        None,
        "--output", "-o",
        help="Output directory for MDX files (default: ~/XoneAIDocs/docs/sdk/reference/xoneaiagents)",
    ),
    ...
    Parses xoneaiagents package using griffe and generates Mintlify-compatible
    MDX documentation for all public symbols in _LAZY_IMPORTS.
    
    Examples:
        xoneai docs generate                    # Generate all reference docs
        xoneai docs generate --dry-run          # Preview without writing
        xoneai docs generate --module agent     # Generate only agent module
        xoneai docs generate --update-nav       # Also update docs.json
    """
    import subprocess
    import time
    ...
    # Default paths
    default_package = Path("/Users/xonevn/xoneai-package/src/xoneai-agents/xoneaiagents")
    default_output = Path("/Users/xonevn/XoneAIDocs/docs/sdk/reference/xoneaiagents")
    generator_script = Path("/Users/xonevn/XoneAI-tools/scripts/generate-sdk-docs.py")
    ...
    typer.echo("=" * 60)
    typer.echo("XoneAI SDK Documentation Generator")
    typer.echo("=" * 60)
...
def docs_report(
    ...
    Examples:
        xoneai docs report                    # View latest report
        xoneai docs report ./my-report        # View specific report
        xoneai docs report --wide --limit 0   # Full details
        xoneai docs report --match failed     # Only failures
        xoneai docs report --format json      # JSON output
    """
    # Lazy import to avoid loading at CLI startup
    from xoneai.suite_runner.report_viewer import view_report
...
def docs_api_md(
    ...
    covering all public exports from xoneaiagents, xoneai, CLI, and TypeScript.
    
    Examples:
        xoneai docs api-md              # Generate api.md
        xoneai docs api-md --check      # Check if api.md is up to date
        xoneai docs api-md --stdout     # Print to stdout
    """
    from xoneai._dev.api_md import generate_api_md
...
def cli_run_all(
    docs_path: Optional[Path] = typer.Option(
        None,
        "--docs-path", "-p",
        help="Path to documentation directory (default: ~/XoneAIDocs/docs/cli)",
    ),
    timeout: int = typer.Option(
        10,
        "--timeout", "-t",
    ...
    Discovers xoneai CLI commands in bash code blocks and validates
    them by running with --help to ensure they exist and work.
    
    Examples:
        xoneai docs cli run-all
        xoneai docs cli run-all --max-items 10
        xoneai docs cli run-all --group cli --workers 8
        xoneai docs cli run-all --ci --timeout 5
    """
    import subprocess
    from concurrent.futures import ThreadPoolExecutor, as_completed
    from xoneai.suite_runner import CLIDocsSource, RunResult, RunReport, SuiteReporter
    ...
    if docs_path:
        ...
        candidates = [
            Path.home() / "XoneAIDocs" / "docs" / "cli",
            Path("/Users/xonevn/XoneAIDocs/docs/cli"),
            Path.cwd() / "docs" / "cli",
        ]
        docs = None
        for candidate in candidates:
    ...
    if ci:
        print("=" * 60)
        print("XoneAI CLI Docs Validator")
        print("=" * 60)
...
def cli_list(
    ...
    Examples:
        xoneai docs cli list
        xoneai docs cli list --groups
        xoneai docs cli list --runnable
    """
    from xoneai.suite_runner import CLIDocsSource
...
def cli_report(
    ...
    Examples:
        xoneai docs cli report
        xoneai docs cli report ./my-report
        xoneai docs cli report --format json
    """
    from xoneai.suite_runner.report_viewer import view_report

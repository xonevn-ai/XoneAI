"""
Serve command group for XoneAI CLI.

Provides unified server management for ALL XoneAI server types.
All server commands are consolidated under `xoneai serve <type>`.
...
app = typer.Typer(
    help="Unified server management - start any XoneAI server",
...
def serve_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]XoneAI Serve - Unified Server Management[/bold cyan]

Start any XoneAI server with: xoneai serve <type>
...
[bold]Examples:[/bold]
  xoneai serve agents --port 8000
  xoneai serve gateway --port 8765
  xoneai serve mcp --transport sse
  xoneai serve unified

[bold]Discovery:[/bold]
  All HTTP servers expose /__xoneai__/discovery for endpoint discovery.
"""
        try:
...
# =============================================================================
# UNIFIED SERVER COMMANDS - All server types under `xoneai serve <type>`
# =============================================================================
...
def serve_agents(
    ...
    Examples:
        xoneai serve agents
        xoneai serve agents --file my-agents.yaml --port 8000
    """
    try:
        from ..features.serve import handle_serve_command
...
def serve_gateway(
    ...
    Examples:
        xoneai serve gateway
        xoneai serve gateway --port 8765 --agents agents.yaml
    """
    output = get_output_controller()
    ...
    try:
        ...
        output.print("Install with: pip install xoneai[api]")
        raise typer.Exit(4)
...
def serve_mcp(
    ...
    Examples:
        xoneai serve mcp --transport stdio
        xoneai serve mcp --transport sse --port 8080
        xoneai serve mcp --name my-server
    """
    output = get_output_controller()
    ...
    if name:
        ...
        try:
            ...
            if transport == "stdio":
                ...
                from xoneaiagents.mcp import MCP, ToolsMCPServer
                cmd_string = " ".join(cmd)
            ...
            output.print("Install with: pip install xoneaiagents[mcp]")
            raise typer.Exit(4)
    else:
        ...
        try:
            ...
            mcp_server = ToolsMCPServer(name="xoneai-mcp")
            if transport == "stdio":
                mcp_server.run()
            else:
                mcp_server.run_sse(host=host, port=port)
...
def serve_acp(
    ...
    Examples:
        xoneai serve acp
        xoneai serve acp --workspace ./my-project
        xoneai serve acp --model gpt-4o
    """
    output = get_output_controller()
    ...
    try:
        ...
        output.print("Install with: pip install xoneai[acp]")
        raise typer.Exit(4)
...
def serve_lsp(
    ...
    Examples:
        xoneai serve lsp
        xoneai serve lsp --language python
    """
    output = get_output_controller()
...
def serve_ui(
    ...
    Examples:
        xoneai serve ui
        xoneai serve ui --type chat --port 8084
        xoneai serve ui --type code --port 8086
    """
    output = get_output_controller()
    ...
    try:
        ...
        output.print("Install with: pip install xoneai[ui]")
        raise typer.Exit(4)
...
def serve_rag(
    ...
    Examples:
        xoneai serve rag
        xoneai serve rag --collection research --port 9000
    """
    output = get_output_controller()
...
def serve_registry(
    ...
    Examples:
        xoneai serve registry
        xoneai serve registry --port 7777 --token mysecret
    """
    output = get_output_controller()
...
def serve_docs(
    ...
    Examples:
        xoneai serve docs
        xoneai serve docs --port 3000 --path ./docs
    """
    output = get_output_controller()
...
def serve_scheduler(
    ...
    Examples:
        xoneai serve scheduler
        xoneai serve scheduler --config schedule.yaml --daemon
    """
    output = get_output_controller()
...
def serve_recipe(
    ...
    Examples:
        xoneai serve recipe
        xoneai serve recipe --port 8765 --reload
    """
    try:
        from ..features.serve import handle_serve_command
...
def serve_unified(
    ...
    Examples:
        xoneai serve unified
        xoneai serve unified --port 8765 --reload
    """
    try:
        from ..features.serve import handle_serve_command

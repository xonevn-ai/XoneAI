"""
Bot command group for XoneAI CLI.
...
def bot_start(
    ...
    Examples:
        xoneai bot start --config bot.yaml
        xoneai bot start -c my-telegram-bot.yaml
    """
...
def bot_telegram(
    ...
    Examples:
        ...
        xoneai bot telegram --agent agents.yaml --browser --web
        xoneai bot telegram --tools DuckDuckGoTool WikipediaTool --memory
    """
...
def bot_discord(
    ...
    Examples:
        xoneai bot discord --token $DISCORD_BOT_TOKEN
        xoneai bot discord --agent agents.yaml --browser --web
        xoneai bot discord --tools DuckDuckGoTool --memory
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_slack(
    ...
    Examples:
        xoneai bot slack --token $SLACK_BOT_TOKEN
        xoneai bot slack --agent agents.yaml --browser --web
        xoneai bot slack --tools DuckDuckGoTool --memory --app-token $SLACK_APP_TOKEN
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_whatsapp(
    ...
    Examples:
        xoneai bot whatsapp --token $WHATSAPP_ACCESS_TOKEN --phone-id $WHATSAPP_PHONE_NUMBER_ID
        xoneai bot whatsapp --mode web
        xoneai bot whatsapp --mode web --creds-dir ~/.myapp/wa-creds
        xoneai bot whatsapp --agent agents.yaml --memory --web
    """
    from ..features.bots_cli import BotHandler, BotCapabilities
...
def bot_callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None:
        help_text = """
[bold cyan]XoneAI Bot - Messaging Bots with Full Agent Capabilities[/bold cyan]

Start a bot on any platform with: xoneai bot <platform>
...
[bold]Examples:[/bold]
  xoneai bot telegram --token $TELEGRAM_BOT_TOKEN
  xoneai bot slack --agent agents.yaml --browser --web
  xoneai bot discord --tools DuckDuckGoTool --memory --model gpt-4o
  xoneai bot whatsapp --token $WHATSAPP_ACCESS_TOKEN --phone-id $WHATSAPP_PHONE_NUMBER_ID
  xoneai bot whatsapp --mode web
"""
        try:

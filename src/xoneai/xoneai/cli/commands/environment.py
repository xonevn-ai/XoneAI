"""
Environment command group for XoneAI CLI.
...
def env_view(
    ...
    filter_prefix: Optional[str] = typer.Option(
        ...
        "--filter",
        "-f",
        help="Filter by prefix (e.g., OPENAI, XONEAI)",
    ),
):
...
def env_doctor(
    ...
    """Run environment diagnostics (alias for 'xoneai doctor env')."""
    output = get_output_controller()

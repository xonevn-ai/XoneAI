"""
XoneAI Examples CLI Command.

Run and manage example files with reporting and diagnostics.

Usage:
    xoneai examples run                    # Run all examples
    xoneai examples run --path ./examples  # Custom path
    xoneai examples list                   # List discovered examples
"""
...
def _get_default_examples_path() -> Path:
    ...
    candidates = [
        Path.home() / "xoneai-package" / "examples",
        Path("/Users/xonevn/xoneai-package/examples"),
        Path.cwd() / "examples",
...
def run(
    ...
    Examples:
        xoneai examples run
        xoneai examples run --path ./examples --timeout 120
        xoneai examples run --group python --group mcp --max-items 5
        xoneai examples run --include "context/*" --exclude "*_wow.py"
        xoneai examples run --fail-fast --no-stream
    """
    # Lazy import to avoid loading at CLI startup
    from xoneai.suite_runner import ExamplesSource, SuiteExecutor, RunResult
    ...
    if not quiet:
        typer.echo("=" * 60)
        typer.echo("XoneAI Examples Runner")
        typer.echo("=" * 60)
...
def list_examples(
    ...
    Examples:
        xoneai examples list
        xoneai examples list --groups
        xoneai examples list --group python --metadata
    """
    from xoneai.suite_runner import ExamplesSource
...
def info(
    ...
    Examples:
        xoneai examples info ./examples/context/01_basic.py
    """
    from xoneai.suite_runner import ExamplesSource
...
def examples_stats(
    ...
    Examples:
        xoneai examples stats
        xoneai examples stats --group python --group mcp
    """
...
def examples_run_all(
    ...
    Examples:
        xoneai examples run-all
        xoneai examples run-all --sequential
        xoneai examples run-all --workers 8 --timeout 120
    """
    import json
...
def examples_report(
    ...
    Examples:
        xoneai examples report                    # View latest report
        xoneai examples report ./my-report        # View specific report
        xoneai examples report --wide --limit 0   # Full details
        xoneai examples report --match failed     # Only failures
        xoneai examples report --format json      # JSON output
    """
    # Lazy import to avoid loading at CLI startup

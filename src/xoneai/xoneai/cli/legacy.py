"""
Legacy compatibility adapter for XoneAI CLI.
...
def is_legacy_invocation(argv: List[str]) -> bool:
    ...
    Legacy patterns:
    - xoneai "prompt"  (direct prompt)
    - xoneai agents.yaml  (file path)
    - xoneai --legacy-flag value  (legacy-only flags)
    - xoneai <legacy_command>  (commands not yet in Typer)
    """
    if not argv:
        return False
    ...
    # NOTE: --interactive and --chat-mode have been removed - use 'xoneai chat' instead
    legacy_flags = [
...
def main_with_legacy_support():
    ...
    if '--version' in argv or '-V' in argv:
        ...
        print(f"XoneAI version {__version__}")
        return
    ...
    if argv and argv[0] == 'browser':
        try:
            ...
            sys.argv = ['xoneai-browser'] + argv[1:]
            browser_app()
            return
        except SystemExit as e:

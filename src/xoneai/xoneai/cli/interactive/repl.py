"""
Interactive REPL for XoneAI.

This is the main entry point for interactive chat mode.
Uses XoneIO for input/output (prompt_toolkit is used SYNCHRONOUSLY).
"""
...
from .xone_io import XoneIO, IOConfig
...
class InteractiveREPL:
    """
    Interactive REPL for XoneAI chat.
    
    Key design:
    - Uses XoneIO for all I/O (prompt_toolkit used synchronously)
    ...
    def _get_agent(self):
        ...
        if self._agent is None:
            try:
                ...
                self.io.tool_error(f"Failed to import xoneaiagents: {e}")
                raise
...
def start_interactive(
    ...
    **kwargs
    ...
    This is the main entry point for `xoneai chat` and `xoneai` (no args).
    """
    config = REPLConfig(
        model=model,
        verbose=verbose,

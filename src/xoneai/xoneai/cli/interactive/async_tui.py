...
# ============================================================================
# Debug Logging - Only enabled with --debug flag or XONE_DEBUG=1
# ============================================================================
...
def _init_debug_logging():
    ...
    _debug_log_file = os.path.expanduser("~/.xoneai/async_tui_debug.log")
...
if os.environ.get("XONE_DEBUG", "").lower() in ("1", "true", "yes"):
    _init_debug_logging()
...
from xoneai.cli.branding import get_logo, get_version
...
class AsyncTUIConfig:
    ...
    debug: bool = False  # Enable debug logging to file (~/.xoneai/async_tui_debug.log)
...
class AsyncTUI:
        ...
        if self._agent is None:
            ...
            try:
                ...
                agent_config = {
                    ...
                    "instructions": "You are Xone AI, a helpful assistant. Be concise and helpful. You have access to tools for file operations, code execution, and web search.",
                ...
                logger.error(f"Failed to import xoneaiagents: {e}")
                raise RuntimeError(f"Failed to import xoneaiagents: {e}")
    ...
    async def _start_runtime(self):
        ...
        try:
            from xoneai.cli.features.interactive_runtime import create_runtime
    ...
    def _load_tools(self):
        ...
        try:
            from xoneai.cli.features.interactive_tools import get_interactive_tools
        ...
        # Fallback: try to load basic tools directly from xoneaiagents
        logger.debug("Falling back to direct xoneaiagents.tools import...")
        try:
  ...
  /debug           Toggle debug logging to ~/.xoneai/async_tui_debug.log
...
Tips:
  ...
  Use --debug flag or /debug command to enable debug logging"""
            ...
            log_file = os.path.expanduser("~/.xoneai/async_tui_debug.log")
            self.messages.append(ChatMessage(
                role="system", 
...
Example: /handoff code "refactor the auth module" """
    ...
    def _execute_in_background(self, prompt: str):
        ...
        def run():
            ...
            self._status_text = "Xone AI is thinking..."
            self._update_output()
            ...
            while not done[0]:
                ...
                self._status_text = f"{spinner[idx]} Xone AI is thinking... ({elapsed:.1f}s){queue_info}"
                self._update_output()
    ...
    def _run_with_prompt_toolkit(self) -> None:
        ...
        # Setup history
        history_file = self.config.history_file or str(Path.home() / ".xone" / "history")

"""
XoneIO - Interactive CLI I/O handler inspired by Aider.
...
class XoneIO:
    """
    Main I/O handler for XoneAI interactive CLI.
    ...
    def __init__(self, config: Optional[IOConfig] = None):
        ...
        if self.config.input_history_file:
            ...
            self._history_file = Path.home() / ".xone" / "input_history"
    ...
    def print_welcome(self, model: str = "gpt-4o-mini", tools_count: int = 0, session_id: str = "") -> None:
        ...
        if self.console and self.config.pretty:
            ...
            welcome = Text()
            welcome.append("XoneAI Interactive Mode\n", style="bold cyan")

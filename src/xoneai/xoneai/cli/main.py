...
# This warning is harmless and occurs when running as `python -m xoneai.cli.main`
...
# from xoneai.auto import AutoGenerator  # Lazy: imported in auto/init commands
# from xoneai.agents_generator import AgentsGenerator  # Lazy: imported when running agents
# REMOVED: from xoneai.inbuilt_tools import * - causes ~3200ms crewai import
# REMOVED: from xoneai.inc.config import generate_config - causes ~3500ms langchain import
...
def _get_chainlit_run():
    ...
    if "CHAINLIT_APP_ROOT" not in os.environ:
        chainlit_root = os.path.join(os.path.expanduser("~"), ".xone")
...
try:
    CALL_MODULE_AVAILABLE = importlib.util.find_spec("xoneai.api.call") is not None
...
XONEAI_AVAILABLE = importlib.util.find_spec("xoneaiagents") is not None
...
def _get_call_module():
    ...
    Raises:
        ImportError: If xoneai.api.call is not installed
    """
    if not CALL_MODULE_AVAILABLE:
        raise ImportError(
            "Call feature is not installed. Install with: pip install \"xoneai[call]\""
        )
    from xoneai.api import call as call_module
    return call_module
...
def _get_autogen():
    ...
    if not AUTOGEN_AVAILABLE:
        raise ImportError(
            "AutoGen is not installed. Install with: pip install \"xoneai[autogen]\""
        )
    import autogen
    return autogen
...
# Suppress xoneaiagents INFO logs unless LOGLEVEL is explicitly set to debug/info
if os.environ.get('LOGLEVEL', '').upper() not in ('DEBUG', 'INFO'):
    logging.getLogger('xoneaiagents').setLevel(logging.WARNING)
    logging.getLogger('xoneaiagents.llm').setLevel(logging.WARNING)
    logging.getLogger('xoneaiagents.llm.llm').setLevel(logging.WARNING)
...
class XoneAI:
    def __init__(self, agent_file="agents.yaml", framework="", auto=False, init=False, agent_yaml=None, tools=None):
        """
        Initialize the XoneAI object with default parameters.
        """
    ...
    def main(self):
        """
        The main function of the XoneAI object. It parses the command-line arguments,
        ...
        self.args = args
        invocation_cmd = "xoneai"
        version_string = f"XoneAI version {__version__}"
        ...
        if args.command:
            ...
            if args.command == "persistence":
                from xoneai.cli.features.persistence import handle_persistence_command
                handle_persistence_command(unknown_args)
                return None
            
            # Handle schedule command
            if args.command == "schedule":
                from xoneai.cli.features.agent_scheduler import AgentSchedulerHandler
        ...
        if args.deploy:
            if args.schedule or args.schedule_config:
                ...
                from xoneai.scheduler import create_scheduler
                ...
                from xoneai.cli.features.deploy import DeployHandler
        ...
        if args.command == 'train':
            if not TRAIN_AVAILABLE:
                ...
                print("\npip install \"xoneai[train]\"")
                print("Or run: xoneai train init\n")
                sys.exit(1)
            ...
            if 'init' in sys.argv:
                from xoneai.setup.setup_conda_env import main as setup_conda_main
                setup_conda_main()
            ...
            try:
                ...
                if 'xone_env' in result.decode('utf-8'):
                    print("Conda environment 'xone_env' found.")
                    conda_env_exists = True
                else:
                    print("Conda environment 'xone_env' not found. Setting it up...")
                    from xoneai.setup.setup_conda_env import main as setup_conda_main
                    setup_conda_main()
                    print("All packages installed.")
                    ...
                    try:
                        result = subprocess.check_output(['conda', 'env', 'list'])
                        conda_env_exists = 'xone_env' in result.decode('utf-8')
            ...
            if conda_available and conda_env_exists:
                try:
                    ...
                    stream_subprocess(['conda', 'run', '--no-capture-output', '--name', 'xone_env', 'python', '-u', train_script_path, 'train'], env=env)
        ...
        if args.ui:
            ...
            if save_replay:
                try:
                    from xoneai.replay import ContextTraceWriter
                    from xoneaiagents.trace.context_events import ContextTraceEmitter, set_context_emitter
            ...
            try:
                ...
                if trace_emitter_token:
                    from xoneaiagents.trace.context_events import reset_context_emitter
    ...
    def parse_args(self):
        """
        Parse the command-line arguments for the XoneAI CLI.
        """
        ...
        # Check if we're being used as a library (not from xoneai CLI)
        # Skip CLI parsing to avoid conflicts with applications like Fabric
        is_library_usage = (
            'xoneai' not in sys.argv[0] and
            not in_test_env
        )
        ...
        parser = argparse.ArgumentParser(prog="xoneai", description="xoneAI command-line interface")
        parser.add_argument("--framework", choices=["crewai", "autogen", "xoneai"], help="Specify the framework")
        ...
        parser.add_argument("--call", action="store_true", help="Start the XoneAI Call server")
        ...
        parser.add_argument("--api-url", type=str, default="http://127.0.0.1:8005", help="XoneAI API URL for n8n to call (default: http://127.0.0.1:8005)")
        ...
        if getattr(args, 'claudecode', False):
            os.environ["XONEAI_CLAUDECODE_ENABLED"] = "true"
        ...
        if args.command == 'replay':
            ...
            register_commands()
            import sys as _sys
            _sys.argv = ['xoneai', 'replay'] + unknown_args
            try:
                typer_app()
        ...
        if args.command in special_commands:
            ...
            if args.command == 'chat':
                ...
                os.environ["XONEAI_CODE_MODE"] = "true"
                self._start_interactive_mode(args)
                sys.exit(0)
                ...
                if not CHAINLIT_AVAILABLE:
                    ...
                    print("\npip install \"xoneai[ui]\"\n")
                    sys.exit(1)
                self.create_chainlit_interface()
                sys.exit(0)

            elif args.command == 'context':
                if not XONEAI_AVAILABLE:
                    print("[red]ERROR: XoneAI Agents is not installed. Install with:[/red]")
                    print("\npip install xoneaiagents\n")
                    sys.exit(1)
                
                if not args.url:
                    print("[red]ERROR: --url is required for context command[/red]")
                    print("Usage: xoneai context --url <repository_url> --goal <goal> [--auto-analyze]")
                    sys.exit(1)
                ...
                if not research_query:
                    ...
                    print("Usage: xoneai research \"Your research query\"")
                    print("       xoneai research --model deep-research-pro \"Your query\"")
                    sys.exit(1)
                ...
                skills_parser = argparse.ArgumentParser(prog="xoneai skills")
                ...
                import sys as _sys
                _sys.argv = ['xoneai', 'profile'] + unknown_args
                typer_app()
                sys.exit(0)
                ...
                if unknown_args and unknown_args[0] == 'agent-centric':
                    ...
                    print("Usage: xoneai audit agent-centric [--scan|--fix|--check] PATH")
                    print("\nOptions:")
                ...
                # Serve command - launch XoneAI servers
                ...
                agents_parser = argparse.ArgumentParser(prog="xoneai agents")
                ...
                if unknown_args:
                    ...
                    print("  xoneai deploy doctor [--all] [--provider aws|azure|gcp] [--json]")
                    print("  xoneai deploy init [--file FILE] [--type api|docker|cloud]")
                    print("  xoneai deploy validate [--file FILE] [--json]")
                    print("  xoneai deploy plan [--file FILE] [--json]")
                    print("  xoneai deploy status [--file FILE] [--json] [--verbose]")
                    print("  xoneai deploy destroy [--file FILE] [--yes] [--force] [--json]")
                    print("  xoneai deploy run [--file FILE] [--type api|docker|cloud] [--provider aws|azure|gcp]")
                    print("  xoneai deploy api [--file FILE]")
                    print("  xoneai deploy docker [--file FILE]")
                    print("  xoneai deploy cloud --provider aws|azure|gcp [--file FILE]")
                sys.exit(0)
            ...
            # Capabilities CLI commands
                ...
                from xoneai.browser.cli import app as browser_app
                import sys as _sys
                _sys.argv = ['xoneai', 'browser'] + unknown_args
                try:
                    browser_app()
                except SystemExit as e:
                    sys.exit(e.code if e.code else 0)
        ...
        if not args.command and (args.init or args.auto or args.framework):
            if not CREWAI_AVAILABLE and not AUTOGEN_AVAILABLE and not XONEAI_AVAILABLE:
                print("[red]ERROR: No framework is installed. Please install at least one framework:[/red]")
                print("\npip install \"xoneai\\[crewai]\"  # For CrewAI")
                print("pip install \"xoneai\\[autogen]\"  # For AutoGen")
                print("pip install \"xoneai\\[crewai,autogen]\"  # For both frameworks\n")
                print("pip install xoneaiagents # For Agents\n")  
                sys.exit(1)
            ...
            if rewrite_tools:
                if os.path.isfile(rewrite_tools):
                    ...
                    try:
                        ...
                        import xoneaiagents.tools as tools_module
    ...
    def handle_memory_command(self, action: str, action_args: list, user_id: str = None):
        ...
        try:
            ...
            if action == 'show':
                ...
                if not action_args:
                    print("[red]ERROR: Content required. Usage: xoneai memory add \"Your memory content\"[/red]")
                    return
                ...
                if not action_args:
                    print("[red]ERROR: Query required. Usage: xoneai memory search \"query\"[/red]")
                    return
                ...
                if not action_args:
                    print("[red]ERROR: Session name required. Usage: xoneai memory save <session_name>[/red]")
                    return
                ...
                if not action_args:
                    print("[red]ERROR: Session name required. Usage: xoneai memory resume <session_name>[/red]")
                    return
                session_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Checkpoint ID required. Usage: xoneai memory restore <checkpoint_id>[/red]")
                    return
                ...
                print("  xoneai memory show                    - Show memory statistics")
                print("  xoneai memory add <content>           - Add to long-term memory")
                print("  xoneai memory search <query>          - Search memories")
                print("  xoneai memory clear [short|all]       - Clear memory")
                print("  xoneai memory save <session_name>     - Save session")
                print("  xoneai memory resume <session_name>   - Resume session")
                print("  xoneai memory sessions                - List saved sessions")
                print("  xoneai memory compress                - Compress short-term memory")
                print("  xoneai memory checkpoint [name]       - Create checkpoint")
                print("  xoneai memory restore <checkpoint_id> - Restore checkpoint")
                print("  xoneai memory checkpoints             - List checkpoints")
                print("\n[bold]Options:[/bold]")
                print("  --user-id <id>                           - User ID for memory isolation")
            else:
                print(f"[red]Unknown memory action: {action}[/red]")
                print("Use 'xoneai memory help' for available commands")
                
        except ImportError as e:
            print(f"[red]ERROR: Failed to import memory module: {e}[/red]")
            print("Make sure xoneaiagents is installed: pip install xoneaiagents")
        except Exception as e:
    ...
    def handle_rules_command(self, action: str, action_args: list):
        ...
        """
        try:
            from xoneaiagents.memory import RulesManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            ...
            if action == 'list':
                ...
                if all_rules:
                    ...
                    print("[yellow]No rules found. Create XONE.md, CLAUDE.md, or files in .xone/rules/[/yellow]")
                    
            elif action == 'show':
                if not action_args:
                    print("[red]ERROR: Rule name required. Usage: xoneai rules show <name>[/red]")
                    return
                ...
                if len(action_args) < 2:
                    print("[red]ERROR: Name and content required. Usage: xoneai rules create <name> <content>[/red]")
                    return
                rule_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Rule name required. Usage: xoneai rules delete <name>[/red]")
                    return
                rule_name = action_args[0]
                ...
                print("  xoneai rules list                     - List all loaded rules")
                print("  xoneai rules show <name>              - Show specific rule details")
                print("  xoneai rules create <name> <content>  - Create a new rule")
                print("  xoneai rules delete <name>            - Delete a rule")
                print("  xoneai rules stats                    - Show rules statistics")
                print("\n[bold]Supported Rule Files:[/bold]")
                print("  XONE.md, CLAUDE.md, AGENTS.md, GEMINI.md")
                print("  .cursorrules, .windsurfrules")
                print("  .xone/rules/*.md, ~/.xone/rules/*.md")
            else:
                print(f"[red]Unknown rules action: {action}[/red]")
                print("Use 'xoneai rules help' for available commands")
    ...
    def handle_workflow_command(self, action: str, action_args: list, variables: dict = None, args=None):
        ...
        """
        try:
            from xoneaiagents import Agent as XoneAgent
            from xoneaiagents.memory import WorkflowManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            ...
            if action == 'list':
                ...
                if workflows:
                    ...
                    print("[yellow]No workflows found. Create files in .xone/workflows/[/yellow]")
                    
            elif action == 'run':
                if not action_args:
                    print("[red]ERROR: Workflow name required. Usage: xoneai workflow run <name>[/red]")
                    return
                ...
                if not action_args:
                    print("[red]ERROR: Workflow name required. Usage: xoneai workflow show <name>[/red]")
                    return
                workflow_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Workflow name required. Usage: xoneai workflow create <name>[/red]")
                    return
                workflow_name = action_args[0]
                ...
                manager.create_workflow(
                    ...
                    steps=[
                        {"name": "Step 1", "action": "First step - edit this in .xone/workflows/"},
                        {"name": "Step 2", "action": "Second step - edit this in .xone/workflows/"}
                    ]
                )
                print(f"[green]✅ Workflow created: {workflow_name}[/green]")
                print(f"[cyan]Edit the workflow in .xone/workflows/{workflow_name}.md[/cyan]")
                
            elif action == 'validate':
                # Validate a YAML workflow file
                if not action_args:
                    print("[red]ERROR: YAML file required. Usage: xoneai workflow validate <file.yaml>[/red]")
                    return
                ...
                print("  xoneai workflow list                  - List available workflows")
                print("  xoneai workflow run <name>            - Execute a workflow")
                print("  xoneai workflow run <file.yaml>       - Execute a YAML workflow")
                print("  xoneai workflow show <name>           - Show workflow details")
                print("  xoneai workflow create <name>         - Create a new workflow")
                print("  xoneai workflow validate <file.yaml>  - Validate a YAML workflow")
                print("  xoneai workflow template <name>       - Create from template")
                print('  xoneai workflow auto "topic"          - Auto-generate workflow')
                print("\n[bold]Templates:[/bold]")
                ...
                print("\n[bold]Examples:[/bold]")
                print("  xoneai workflow run 'Research Blog' --tools tavily --save")
                print("  xoneai workflow run research.yaml --var topic='AI trends'")
                print("  xoneai workflow template routing --output my_workflow.yaml")
            else:
                print(f"[red]Unknown workflow action: {action}[/red]")
                print("Use 'xoneai workflow help' for available commands")
    ...
    def _run_yaml_workflow(self, yaml_file: str, action_args: list, variables: dict = None, args=None):
        ...
        try:
            from xoneaiagents.workflows import WorkflowManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            import uuid
            ...
            if context_auto_compact is True or context_strategy or context_threshold:
                ...
                try:
                    from xoneaiagents.context import ManagerConfig
                    config_kwargs = {"auto_compact": True}
                    if context_strategy:
                        from xoneaiagents.context import OptimizerStrategy
                        strategy_map = {
    ...
    def _validate_yaml_workflow(self, yaml_file: str):
        ...
        """
        try:
            from xoneaiagents.workflows import YAMLWorkflowParser
            from rich import print
            from rich.table import Table
            from rich.console import Console
            import yaml
    ...
    def _create_workflow_from_template(self, template_name: str = None, output_file: str = None):
        ...
        print(f"[cyan]Run with: xoneai workflow run {output_file}[/cyan]")
    ...
    def _auto_generate_workflow(self, action_args: list):
        ...
        if not topic:
            print('[red]Usage: xoneai workflow auto "topic" --pattern <pattern>[/red]')
        ...
        try:
            from xoneai.auto import WorkflowAutoGenerator
    ...
    def handle_hooks_command(self, action: str):
        ...
        """
        try:
            from xoneaiagents.memory import HooksManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            import json
            ...
            if action == 'list':
                ...
                if stats.get('total_hooks', 0) > 0:
                    ...
                    print("[yellow]No hooks configured. Create .xone/hooks.json[/yellow]")
                ...
                hooks_dir = os.path.join(os.getcwd(), ".xone")
                ...
                print("  xoneai hooks list                     - List configured hooks")
                print("  xoneai hooks stats                    - Show hooks statistics")
                print("  xoneai hooks init                     - Create hooks.json template")
                ...
                print("Use 'xoneai hooks help' for available commands")
    ...
    def handle_docs_command(self, action: str, action_args: list):
        ...
        if action in code_validation_actions:
            from xoneai.cli.commands.docs import app as docs_app
            import typer
        ...
        try:
            from xoneaiagents.memory import DocsManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            ...
            if action == 'list':
                ...
                if all_docs:
                    ...
                    print("[yellow]No docs found. Create files in .xone/docs/[/yellow]")
                    
            elif action == 'show':
                if not action_args:
                    print("[red]ERROR: Doc name required. Usage: xoneai docs show <name>[/red]")
                    return
                ...
                if len(action_args) < 2:
                    print("[red]ERROR: Name and content required. Usage: xoneai docs create <name> <content>[/red]")
                    return
                doc_name = action_args[0]
                content = ' '.join(action_args[1:])
                docs.create_doc(
                ...
                if not action_args:
                    print("[red]ERROR: Doc name required. Usage: xoneai docs delete <name>[/red]")
                    return
                doc_name = action_args[0]
                ...
                print("  xoneai docs list                     - List all docs")
                print("  xoneai docs show <name>              - Show specific doc")
                print("  xoneai docs create <name> <content>  - Create a new doc")
                print("  xoneai docs delete <name>            - Delete a doc")
                print("\n[bold]Doc Location:[/bold]")
                print("  .xone/docs/*.md, ~/.xone/docs/*.md")
            else:
                print(f"[red]Unknown docs action: {action}[/red]")
                print("Use 'xoneai docs help' for available commands")
    ...
    def handle_mcp_command(self, action: str, action_args: list):
        ...
        """
        try:
            from xoneaiagents.memory import MCPConfigManager
            from rich import print
            from rich.table import Table
            from rich.console import Console
            ...
            if action == 'list':
                ...
                if all_configs:
                    ...
                    print("[yellow]No MCP configs found. Create files in .xone/mcp/[/yellow]")
                    
            elif action == 'show':
                if not action_args:
                    print("[red]ERROR: Config name required. Usage: xoneai mcp show <name>[/red]")
                    return
                ...
                if len(action_args) < 2:
                    print("[red]ERROR: Name and command required. Usage: xoneai mcp create <name> <command> [args...][/red]")
                    return
                config_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Config name required. Usage: xoneai mcp delete <name>[/red]")
                    return
                config_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Config name required. Usage: xoneai mcp enable <name>[/red]")
                    return
                config_name = action_args[0]
                ...
                if not action_args:
                    print("[red]ERROR: Config name required. Usage: xoneai mcp disable <name>[/red]")
                    return
                config_name = action_args[0]
                ...
                # Start MCP server
                from xoneai.mcp_server.cli import handle_mcp_command as mcp_serve_handler
                ...
                print("  xoneai mcp serve                             - Start MCP server (STDIO)")
                print("  xoneai mcp serve --transport http-stream     - Start HTTP Stream server")
                print("  xoneai mcp list-tools                        - List available MCP tools")
                print("  xoneai mcp config-generate --client claude-desktop  - Generate client config")
                print("\n[bold cyan]Config Management:[/bold cyan]")
                print("  xoneai mcp list                              - List all MCP configs")
                print("  xoneai mcp show <name>                       - Show specific config")
                print("  xoneai mcp create <name> <cmd> [args...]     - Create a new config")
                print("  xoneai mcp delete <name>                     - Delete a config")
                print("  xoneai mcp enable <name>                     - Enable a config")
                print("  xoneai mcp disable <name>                    - Disable a config")
                print("\n[bold]Config Location:[/bold]")
                print("  .xone/mcp/*.json, ~/.xone/mcp/*.json")
                print("\n[bold]Examples:[/bold]")
                print("  xoneai mcp serve --transport stdio")
                print("  xoneai mcp serve --transport http-stream --port 8080")
                print("  xoneai mcp create filesystem npx -y @modelcontextprotocol/server-filesystem .")
            else:
                print(f"[red]Unknown mcp action: {action}[/red]")
                print("Use 'xoneai mcp help' for available commands")
...
Provide ONLY the commit message, no explanations."""
    ...
    def handle_direct_prompt(self, prompt):
        ...
        - @doc:name - Include doc from .xone/docs/
        ...
        mention_context = ""
        try:
            from xoneaiagents.tools.mentions import MentionsParser
        ...
        if XONEAI_AVAILABLE:
            ...
            if hasattr(self, 'args'):
                ...
                if getattr(self.args, 'trust', False) or getattr(self.args, 'approve_level', None):
                    from xoneaiagents.approval import set_approval_callback, ApprovalDecision
                    ...
                    if getattr(self.args, 'trust', False):
                        ...
                        def level_based_approve(function_name, arguments, risk_level):
                            ...
                            if tool_level_value <= max_level_value:
                                ...
                                from xoneaiagents.approval import console_approval_callback
                ...
                if getattr(self.args, 'external_agent', None):
                    ...
                    try:
                        ...
                        if integration.is_available:
                            ...
                            # Run the external agent directly instead of XoneAI agent
                            import asyncio
                            try:
            ...
            agent = XoneAgent(**agent_config)
            
            # AutoRag - Automatic RAG retrieval decision
            if hasattr(self, 'args') and getattr(self.args, 'auto_rag', False):
                from xoneaiagents import AutoRagAgent
...
Now, {final_instruction.lower()}:"""
                ...
                final_agent = XoneAgent(**final_agent_config)
    ...
    def _run_with_status_display(self, agent, prompt):
        ...
        # Import callback registration from xoneaiagents
        ...
        try:
            from xoneaiagents import register_display_callback, sync_display_callbacks
    ...
    def _handle_serve_command(self, args, unknown_args):
        ...
        # Check if command itself is a yaml file (xoneai agents.yaml --serve)
        ...
        if not os.path.exists(yaml_file):
            print(f"[red]ERROR: File not found: {yaml_file}[/red]")
            print("\nUsage:")
            print("  xoneai serve agents.yaml")
            print("  xoneai serve agents.yaml --port 8005")
            print("  xoneai agents.yaml --serve")
            sys.exit(1)
        ...
        xone.launch(port=port, host=host)
    ...
    def create_chainlit_chat_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            chat_ui_path = os.path.join(os.path.dirname(xoneai.__file__), 'ui', 'chat.py')
            _get_chainlit_run()([chat_ui_path])
        else:
            print("ERROR: Chat UI is not installed. Please install it with 'pip install \"xoneai[chat]\"' to use the chat UI.")
    ...
    def create_code_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            code_ui_path = os.path.join(os.path.dirname(xoneai.__file__), 'ui', 'code.py')
            _get_chainlit_run()([code_ui_path])
        else:
            print("ERROR: Code UI is not installed. Please install it with 'pip install \"xoneai[code]\"' to use the code UI.")
    ...
    def create_chainlit_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            public_folder = os.path.join(os.path.dirname(xoneai.__file__), 'public')
            if not os.path.exists("public"):
                ...
                logging.info("Public folder already exists.")
            chainlit_ui_path = os.path.join(os.path.dirname(xoneai.__file__), 'ui', 'agents.py')
            _get_chainlit_run()([chainlit_ui_path])
        else:
            print("ERROR: Chainlit is not installed. Please install it with 'pip install \"xoneai[ui]\"' to use the UI.")
    ...
    def create_realtime_interface(self):
        ...
        if CHAINLIT_AVAILABLE:
            ...
            if not os.path.exists(os.path.join(root_path, "public")):
                ...
                logging.info("Public folder already exists.")
            realtime_ui_path = os.path.join(os.path.dirname(xoneai.__file__), 'ui', 'realtime.py')
            _get_chainlit_run()([realtime_ui_path])
        else:
            print("ERROR: Realtime UI is not installed. Please install it with 'pip install \"xoneai[realtime]\"' to use the realtime UI.")
    ...
    def handle_research_command(self, query: str, model: str = None, verbose: bool = False, save: bool = False, query_rewrite: bool = False, tools_path: str = None, rewrite_tools: str = None) -> str:
        ...
        try:
            ...
            if tools_list:
                from xoneaiagents import Agent, Task, Agents
    ...
    def _start_interactive_mode(self, args):
        ...
        try:
            ...
            from xoneai.cli.features.message_queue import (
            ...
            from xoneai.cli.session import get_session_store
            ...
            try:
                ...
                from xoneai.cli.features.at_mentions import CombinedCompleter
            ...
            console.print("\n[bold cyan]XoneAI Interactive Mode[/bold cyan]")
            ...
            while running:
                try:
                    ...
                    if worker_state.get('waiting_for_approval_input'):
                        try:
                            from xoneaiagents.approval import ApprovalDecision
    ...
    def _load_interactive_tools(self):
        ...
        - Env vars: XONE_TOOLS_DISABLE=acp,lsp
        """
    ...
    def _load_basic_tools_fallback(self):
        ...
        tools_list = []
        try:
            from xoneaiagents.tools import (
            ...
            try:
                from xoneaiagents.tools import read_file as tool_read_file
                tools_list.append(tool_read_file)
            except ImportError:
                pass
            try:
                from xoneaiagents.tools import write_file as tool_write_file
                tools_list.append(tool_write_file)
            except ImportError:
                pass
            try:
                from xoneaiagents.tools import list_files as tool_list_files
                tools_list.append(tool_list_files)
            except ImportError:
                pass
            try:
                from xoneaiagents.tools import execute_command
    ...
    def _handle_stats_command(self, console, session_state):
        ...
        from xoneai.cli.features.cost_tracker import get_pricing
...
Provide a concise summary (max 200 words):"""
    ...
    def _handle_context_command(self, console, args, session_state):
        ...
        """
        try:
            from xoneai.cli.features.context_manager import (
    ...
    def _run_chat_mode(self, prompt, args):
        ...
        Usage: xoneai "your prompt" --chat
               xoneai "your prompt" --chat-mode  (alias)
    ...
    def _start_execution_worker(self, tools_list, console, session_state):
        ...
        import queue as queue_module
        from rich.panel import Panel
        from xoneai.cli.features.message_queue import ProcessingState
        ...
        # Check if trust mode is enabled (via --trust flag or XONE_APPROVAL_MODE=auto env var)
        trust_mode = getattr(self.args, 'trust', False) if hasattr(self, 'args') else False
        approval_mode_env = os.environ.get("XONE_APPROVAL_MODE", "").lower()
    ...
    def _process_interactive_prompt(self, prompt, tools_list, console, show_profiling=False, session_state=None):
        ...
        try:
            ...
            # Set up auto-approval if XONE_APPROVAL_MODE=auto
            approval_mode_env = os.environ.get("XONE_APPROVAL_MODE", "").lower()
            trust_mode = getattr(self.args, 'trust', False) if hasattr(self, 'args') else False
            if approval_mode_env == "auto" or trust_mode:
                try:
                    from xoneaiagents.approval import set_approval_callback, ApprovalDecision
                    def auto_approve_all(function_name, arguments, risk_level):
                        return ApprovalDecision(approved=True, reason="Auto-approved via XONE_APPROVAL_MODE=auto")
                    set_approval_callback(auto_approve_all)
                except ImportError:
                    pass

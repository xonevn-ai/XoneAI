...
def create_knowledge_app():
    ...
    def search_command(
        ...
        Examples:
            xoneai knowledge search "What is the API key?"
            xoneai knowledge search "config" --strategy hybrid --rerank
            xoneai knowledge search "auth" --compress --max-context-tokens 2000
            xoneai knowledge search "*.py" --include "*.py" --exclude "test_*"
        """
        try:
            from xoneaiagents.knowledge import Knowledge
            from xoneaiagents.rag import (
                SmartRetriever, ContextCompressor, RetrievalResult,
            )
        except ImportError as e:
            typer.echo(f"Error: xoneaiagents not installed: {e}", err=True)
            raise typer.Exit(1)
    ...
    def summarize_command(
        ...
        Examples:
            xoneai knowledge summarize ./docs
            xoneai knowledge summarize ./src --levels 2 --rebuild
            xoneai knowledge summarize ./project --output ./summaries
        """
        try:
            from xoneaiagents.rag import HierarchicalSummarizer
            from xoneaiagents.knowledge.indexing import CorpusStats
        except ImportError as e:
            typer.echo(f"Error: xoneaiagents not installed: {e}", err=True)
            raise typer.Exit(1)

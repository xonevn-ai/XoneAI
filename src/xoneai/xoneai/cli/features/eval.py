"""
Evaluation CLI feature for XoneAI.
...
class EvalHandler:
    ...
    def run_performance(
        ...
        """
        try:
            from xoneaiagents.eval import PerformanceEvaluator
            from xoneai.agents_generator import AgentsGenerator
        except ImportError as e:
            logger.error(f"Failed to import evaluation modules: {e}")
            return {"error": str(e)}
    ...
    def run_reliability(
        ...
        """
        try:
            from xoneaiagents.eval import ReliabilityEvaluator
            from xoneai.agents_generator import AgentsGenerator
        except ImportError as e:
            logger.error(f"Failed to import evaluation modules: {e}")
            return {"error": str(e)}
...
def handle_eval_command(args) -> int:
    ...
    if isinstance(args, list):
        parser = argparse.ArgumentParser(prog="xoneai eval")
        ...
        if not args.eval_type:
            parser.print_help()
            print("\n[bold]Examples:[/bold]")
            print("  xoneai eval accuracy --prompt \"What is 2+2?\" --expected \"4\"")
            print("  xoneai eval performance --agent agents.yaml --input \"Hello\"")
            return 0

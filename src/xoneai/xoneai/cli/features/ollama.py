...
class OllamaHandler:
    ...
    def provider(self):
        ...
        if self._provider is None:
            try:
                from xoneai.providers import OllamaProvider
    ...
    def wmp_hooks(self):
        ...
        if self._wmp_hooks is None and self.weak_model_proof:
            try:
                from xoneai.wmp import WMPHooks, WeakModelProofConfig
    ...
    def execute(
        ...
        if self.weak_model_proof:
            try:
                from xoneai.wmp import WMPExecutor, WeakModelProofConfig

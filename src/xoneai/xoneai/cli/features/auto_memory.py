...
Usage: xoneai "Learn about user preferences" --auto-memory
"""
...
class AutoMemoryHandler(FlagHandler):
    ...
    Example:
        ...
        xoneai "Remember this context" --auto-memory --user-id myuser
    """
    ...
    def _get_auto_memory(self, user_id: str = None):
        ...
        try:
            from xoneaiagents.memory import AutoMemory, FileMemory
            # AutoMemory requires a FileMemory instance
            memory = FileMemory(user_id=user_id or "default")
            return AutoMemory(memory=memory)
        except ImportError:
            self.print_status(
                "AutoMemory requires xoneaiagents. Install with: pip install xoneaiagents",
                "error"
            )
            return None

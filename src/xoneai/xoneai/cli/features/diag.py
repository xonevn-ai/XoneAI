"""
Diagnostics export CLI handler for XoneAI.
...
class DiagHandler:
    ...
    def setup_subparser(subparsers) -> None:
        ...
        export_parser.add_argument(
            "-o", "--output",
            type=str,
            help="Output path (default: xoneai-diag-{timestamp}.zip)"
        )
        export_parser.add_argument(
            "--include-logs",
            action="store_true",
    ...
    def handle_export(args: argparse.Namespace) -> int:
        ...
        output_path = args.output or f"xoneai-diag-{timestamp}.zip"
    ...
    def _collect_system_info() -> dict:
        ...
        try:
            ...
            info["xoneai"]["version"] = __version__
        except ImportError:
            info["xoneai"]["version"] = "unknown"
        
        # Get xoneaiagents version
        try:
            import xoneaiagents
            info["xoneai"]["agents_version"] = getattr(xoneaiagents, '__version__', 'unknown')
        except ImportError:
            info["xoneai"]["agents_version"] = "not installed"
    ...
    def _collect_traces(workspace: str, tmppath: Path) -> List[str]:
        ...
        trace_patterns = ["*_trace.json", "xoneai_trace*.json", "*.trace"]
...
def run_diag_command(args: List[str]) -> int:
    ...
    parser = argparse.ArgumentParser(prog="xoneai diag")

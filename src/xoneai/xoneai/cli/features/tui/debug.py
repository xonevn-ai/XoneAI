"""
Debug and Simulation CLI Commands for XoneAI TUI.
...
def create_debug_app():
    ...
    def tui_main(
        ...
        chat, use: xoneai chat
        
        Examples:
            xoneai tui
            xoneai tui --workspace ./project
            xoneai tui --session abc123
        """
        ...
        try:
            ...
            console.print("[red]Textual is required for TUI. Install with:[/red]\n  pip install xoneai[tui]")
            raise typer.Exit(1)
        
        # Set debug environment
        if debug:
            import os
            os.environ["XONEAI_TUI_DEBUG"] = "1"
        
        if log_jsonl:
            import os
            os.environ["XONEAI_TUI_JSONL"] = log_jsonl
    ...
    def tui_simulate(
        ...
        if not mock:
            if not os.environ.get("XONEAI_REAL_LLM"):
                console.print("[red]Real LLM mode requires XONEAI_REAL_LLM=1 environment variable[/red]")
                raise typer.Exit(1)
    ...
    def tui_snapshot(
        ...
        if json_output:
            ...
            # Pretty print
            console.print("─" * 60)
            console.print(f"◉ XoneAI Snapshot │ {time.strftime('%Y-%m-%d %H:%M:%S')}")
            console.print("─" * 60)
...
def create_doctor_command():
    ...
    def doctor_tui():
        ...
        # Print results
        console.print("\n[bold]XoneAI TUI Diagnostics[/bold]\n")
        ...
        if all_ok:
            ...
            console.print("\n[red]Some checks failed. Run 'pip install xoneai[tui]' to install missing dependencies.[/red]")
            raise typer.Exit(1)

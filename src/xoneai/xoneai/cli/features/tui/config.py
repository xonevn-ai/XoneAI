...
class TUIConfig:
    ...
    def from_env(cls) -> "TUIConfig":
        ...
        if os.environ.get("XONEAI_TUI_CTRL_KEYS", "").lower() in ("1", "true", "yes"):
            config.enable_ctrl_keys = True
        
        # Check env vars for enabling Function keys (opt-in)
        if os.environ.get("XONEAI_TUI_FN_KEYS", "").lower() in ("1", "true", "yes"):
            config.enable_fn_keys = True
        
        # Check for disabling single-char shortcuts
        if os.environ.get("XONEAI_TUI_NO_SINGLE_CHAR", "").lower() in ("1", "true", "yes"):
            config.enable_single_char = False
        
        # Check for custom leader key
        leader = os.environ.get("XONEAI_TUI_LEADER", "")

...
NOT supported for:
- xoneai ui ... (browser-based)
- xoneai tui (Textual TUI)
- xoneai call (long-running server)
"""
...
def reject_profiling_for_command(command_name: str):
    ...
    if command_name in PROFILING_INELIGIBLE_COMMANDS:
        raise typer.BadParameter(
            ...
            f"  xoneai \"prompt\" --profile\n"
            f"  xoneai chat \"prompt\" --profile\n"
            f"  xoneai code \"prompt\" --profile\n"
            f"  xoneai run agents.yaml --profile"
        )
...
def run_with_profiling(
    ...
    try:
        ...
        raise ImportError("xoneaiagents not installed. Install with: pip install xoneaiagents")

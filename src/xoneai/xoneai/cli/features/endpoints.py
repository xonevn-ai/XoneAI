...
Provides CLI commands for interacting with XoneAI endpoints:
...
All commands use the canonical `xoneai endpoints` prefix.
...
- No impact on xoneaiagents import time
...
class EndpointsHandler:
    ...
    def base_url(self) -> str:
        ...
        return os.environ.get("XONEAI_ENDPOINTS_URL", self.DEFAULT_URL)
    ...
    def api_key(self) -> Optional[str]:
        ...
        return os.environ.get("XONEAI_ENDPOINTS_API_KEY")
    ...
    def _print_help(self):
        """Print help message."""
        help_text = """
[bold cyan]XoneAI Endpoints[/bold cyan]

Unified client CLI for interacting with XoneAI endpoints.

[bold]Usage:[/bold]
  xoneai endpoints <command> [options]
...
[bold]Global Options:[/bold]
  ...
  --api-key <key>   API key (or set XONEAI_ENDPOINTS_API_KEY)
...
[bold]Environment Variables:[/bold]
  XONEAI_ENDPOINTS_URL      Default server URL
  XONEAI_ENDPOINTS_API_KEY  API key for authentication

[bold]Examples:[/bold]
  xoneai endpoints list
  xoneai endpoints list --type agents-api
  xoneai endpoints list --format json --tags audio,video
  xoneai endpoints describe my-agent
  xoneai endpoints describe my-agent --schema
  xoneai endpoints invoke my-agent --input-json '{"query": "hello"}'
  xoneai endpoints invoke my-recipe --input ./data.json --stream
  xoneai endpoints health
  xoneai endpoints health --url http://localhost:8000
  xoneai endpoints types
  xoneai endpoints discovery
"""
    ...
    def _get_provider(self, provider_type: str, url: Optional[str] = None, api_key: Optional[str] = None):
        ...
        try:
            from xoneai.endpoints import get_provider
            return get_provider(
                provider_type,
    ...
    def _try_unified_discovery(self, url: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        result = self._make_request("GET", "/__xoneai__/discovery", url=url)
    ...
    def _handle_invoke_result(self, result: Dict, name: str, parsed: Dict) -> int:
        ...
        if result.get("status") == 401:
            self._print_error("Authentication required. Use --api-key or set XONEAI_ENDPOINTS_API_KEY")
            return self.EXIT_AUTH_ERROR

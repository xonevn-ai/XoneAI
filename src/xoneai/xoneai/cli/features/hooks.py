"""
Hooks CLI Feature for XoneAI.

Provides CLI commands for managing and testing hooks.

Commands:
- xoneai hooks list                    # List registered hooks
- xoneai hooks test <event> <target>   # Test hooks for an event
- xoneai hooks validate <config>       # Validate hooks configuration
- xoneai hooks run <command>           # Run a command hook manually
"""
...
class HooksHandler:
    ...
    def _get_registry(self):
        ...
        if self._registry is None:
            from xoneaiagents.hooks import HookRegistry
    ...
    def _get_runner(self):
        ...
        if self._runner is None:
            from xoneaiagents.hooks import HookRunner
    ...
    def test_hook(
        ...
        """
        from xoneaiagents.hooks import HookEvent
        from xoneaiagents.hooks.events import BeforeToolInput, BeforeAgentInput
...
"""
    ...
    def run_command_hook(
        ...
        Returns:
            Execution result
        """
        from xoneaiagents.hooks import HookEvent, HookRegistry, HookRunner
        from xoneaiagents.hooks.events import BeforeToolInput
        from datetime import datetime
...
def handle_hooks_command(args: List[str], verbose: bool = False):
    ...
    Usage:
        xoneai hooks list [--format json|table]
        xoneai hooks test <event> [target] [--input '{"key": "value"}']
        xoneai hooks validate <config_path>
        xoneai hooks run <command> [--event <event>] [--input <json>] [--timeout <seconds>]
        xoneai hooks load <config_path>
    """
    handler = HooksHandler(verbose=verbose)
    
    if not args:
        print("Usage: xoneai hooks <command> [options]")
        print("\nCommands:")
    ...
    if command == "list":
        ...
        if len(args) < 2:
            print("Usage: xoneai hooks test <event> [target] [--input <json>]")
            return
        ...
        if len(args) < 2:
            print("Usage: xoneai hooks validate <config_path>")
            return
        handler.validate_config(args[1])
    
    elif command == "run":
        if len(args) < 2:
            print("Usage: xoneai hooks run <command> [--event <event>] [--input <json>] [--timeout <seconds>]")
            return
        ...
        if len(args) < 2:
            print("Usage: xoneai hooks load <config_path>")
            return
        ...
        print("\n  xoneai hooks test <event> [target] [--input <json>]")

...
Usage: xoneai todo list
       xoneai todo add "New task"
       xoneai "Plan project" --todo
"""
...
class TodoHandler(CommandHandler, FlagHandler):
    ...
    def __init__(self, verbose: bool = False, workspace: str = None):
        ...
        self.workspace = workspace or os.path.join(os.path.expanduser("~"), ".xone")
    ...
    def get_help_text(self) -> str:
        return """
Todo Commands:
  xoneai todo list                    - List all todos
  xoneai todo add <task>              - Add a new todo
  xoneai todo complete <id>           - Mark todo as complete
  xoneai todo delete <id>             - Delete a todo
  xoneai todo clear                   - Clear all todos

Flag Usage:
  xoneai "Plan project" --todo        - Generate todos from task
"""
    ...
    def action_add(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai todo add <task>", "error")
            return {}
    ...
    def action_complete(self, args: List[str], **kwargs) -> bool:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai todo complete <id>", "error")
            return False
    ...
    def action_delete(self, args: List[str], **kwargs) -> bool:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai todo delete <id>", "error")
            return False

...
class TemplatesHandler:
    ...
    def loader(self):
        ...
        if self._loader is None:
            from xoneai.templates import TemplateLoader
    ...
    def registry(self):
        ...
        if self._registry is None:
            from xoneai.templates import TemplateRegistry
    ...
    def cache(self):
        ...
        if self._cache is None:
            from xoneai.templates import TemplateCache
    ...
    def _print_help(self):
        """Print help message."""
        help_text = """
[bold cyan]XoneAI Templates[/bold cyan]

[bold]Usage:[/bold]
  xoneai templates <command> [options]
...
[bold]Examples:[/bold]
  xoneai templates list
  xoneai templates search video
  xoneai templates info transcript-generator
  xoneai templates install github:xonevn-ai/agent-recipes/transcript-generator
  xoneai templates run transcript-generator ./audio.mp3
  xoneai templates add github:user/repo/my-template
  xoneai templates add-sources github:xonevn-ai/Agent-Recipes
  xoneai init my-project --template transcript-generator
"""
        try:
    ...
    def cmd_list(self, args: List[str]) -> int:
        ...
        try:
            from xoneai.templates.discovery import TemplateDiscovery
    ...
    def cmd_search(self, args: List[str]) -> int:
        ...
        if not args or args[0].startswith("--"):
            print("Usage: xoneai templates search <query>")
            return 1
        ...
        try:
            from xoneai.templates import search_templates
    ...
    def cmd_info(self, args: List[str]) -> int:
        ...
        if not args or args[0].startswith("--"):
            print("Usage: xoneai templates info <template>")
            return 1
        ...
        try:
            ...
            if discovered:
                ...
                from xoneai.templates import load_template
            ...
            from xoneai.templates.dependency_checker import DependencyChecker
...
"""
    ...
    def cmd_install(self, args: List[str]) -> int:
        ...
        if not args or args[0].startswith("--"):
            print("Usage: xoneai templates install <uri>")
            return 1
        
        uri = args[0]
        
        try:
            from xoneai.templates import install_template
    ...
    def cmd_cache(self, args: List[str]) -> int:
        ...
        if not args:
            print("Usage: xoneai templates cache <clear|list|size>")
            return 1
        ...
        if subcmd == "clear":
            ...
            try:
                from xoneai.templates import clear_cache
    ...
    def cmd_run(self, args: List[str]) -> int:
        ...
        if not args or args[0].startswith("--"):
            print("Usage: xoneai templates run <template> [args...] [--strict-tools] [--offline]")
            return 1
        ...
        try:
            from xoneai.templates.loader import TemplateLoader
            from xoneai.templates.discovery import TemplateDiscovery
            ...
            if strict_tools:
                from xoneai.templates.dependency_checker import DependencyChecker, StrictModeError
                checker = DependencyChecker()
                try:
            ...
            tool_registry = None
            from xoneai.templates.tool_override import create_tool_registry_with_overrides, resolve_tools
    ...
    def cmd_init(self, args: List[str]) -> int:
        ...
        if not args or args[0].startswith("--"):
            print("Usage: xoneai templates init <project-name> --template <template>")
            return 1
        ...
        try:
            ...
            print("  xoneai run workflow.yaml")
    ...
    def cmd_add(self, args: List[str]) -> int:
        ...
        """
        if not args:
            print("[red]Usage: xoneai templates add <source>[/red]")
        ...
        if source.startswith("./") or source.startswith("/"):
            ...
            if path.exists() and path.is_dir():
                ...
                # Copy to ~/.xone/templates/
                templates_dir = Path.home() / ".xone" / "templates"
    ...
    def cmd_add_sources(self, args: List[str]) -> int:
        ...
        """
        if not args:
            print("[red]Usage: xoneai templates add-sources <source>[/red]")
            return 1
    ...
    def cmd_remove_sources(self, args: List[str]) -> int:
        ...
        """
        if not args:
            print("[red]Usage: xoneai templates remove-sources <source>[/red]")
            return 1
    ...
    def cmd_browse(self, args: List[str]) -> int:
        ...
        # Default catalog URL
        catalog_url = "https://xonevn-ai.github.io/xoneai-template-catalog"
    ...
    def cmd_catalog(self, args: List[str]) -> int:
        ...
        """
        if not args:
            print("Usage: xoneai templates catalog <build|sync> [options]")
            print("\nSubcommands:")
            print("  build     Build catalog locally")
            print("  sync      Sync catalog sources from GitHub")
            print("\nExamples:")
            print("  xoneai templates catalog build --out ./dist")
            print("  xoneai templates catalog sync --source agent-recipes")
            return 0
    ...
    def _catalog_build(self, args: List[str]) -> int:
        ...
        catalog_repo = Path.home() / "xoneai-template-catalog"
        if not catalog_repo.exists():
            # Try to find it relative to this package
            possible_paths = [
                Path(__file__).parent.parent.parent.parent.parent.parent / "xoneai-template-catalog",
                Path.cwd() / "xoneai-template-catalog",
            ]
    ...
    def _catalog_sync(self, args: List[str]) -> int:
        ...
        if catalog_repo.exists() and (catalog_repo / "scripts" / "sync-sources.js").exists():
            ...
            cache_path = Path(cache_dir) if cache_dir else Path.home() / ".xone" / "cache"
            ...
            if target.exists():
                ...
                try:
                    subprocess.run([
                        "git", "clone", "--depth=1",
                        "https://github.com/xonevn-ai/Agent-Recipes.git",
                        str(target)
                    ], check=True)

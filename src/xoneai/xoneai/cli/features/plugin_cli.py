...
def create_plugin_app():
    ...
    def init_command(
        ...
        output: Optional[str] = typer.Option(None, "--output", "-o", help="Output directory (default: ./.xone/plugins/)"),
        ...
        Examples:
            xoneai plugin init weather_tools
            xoneai plugin init my_plugin --author "John Doe" --with-hook
            xoneai plugin init custom --output ./my_plugins/
        """
        try:
            from xoneaiagents.plugins.discovery import get_plugin_template, ensure_plugin_dir
        except ImportError:
            typer.echo("Error: xoneaiagents not installed", err=True)
            raise typer.Exit(1)
        ...
        typer.echo("  from xoneaiagents import Agent")
    ...
    def list_command(
        ...
        - ./.xone/plugins/ (project-level)
        - ~/.xone/plugins/ (user-level)
        
        Examples:
            xoneai plugin list
            xoneai plugin list --json
            xoneai plugin list --dir ./my_plugins/
        """
        try:
            from xoneaiagents.plugins.discovery import discover_plugins, get_default_plugin_dirs
        except ImportError:
            typer.echo("Error: xoneaiagents not installed", err=True)
            raise typer.Exit(1)
        ...
        if directory:
            ...
            project_dir = Path.cwd() / ".xone" / "plugins"
        ...
        if not dirs:
            ...
            typer.echo("  mkdir -p .xone/plugins")
            typer.echo("  # or")
            typer.echo("  mkdir -p ~/.xone/plugins")
            return
        ...
        if not plugins:
            ...
            typer.echo("  xoneai plugin init my_plugin")
            return
    ...
    def load_command(
        ...
        Examples:
            xoneai plugin load ./my_plugin.py
            xoneai plugin load ~/.xone/plugins/weather.py
        """
        try:
            from xoneaiagents.plugins.discovery import load_plugin
        except ImportError:
            typer.echo("Error: xoneaiagents not installed", err=True)
            raise typer.Exit(1)
    ...
    def discover_command(
        ...
        Examples:
            xoneai plugin discover
            xoneai plugin discover --verbose
        """
        try:
            from xoneaiagents.plugins.discovery import discover_and_load_plugins
        except ImportError:
            typer.echo("Error: xoneaiagents not installed", err=True)
            raise typer.Exit(1)
    ...
    def template_command(
        ...
        Examples:
            ...
            xoneai plugin template > my_plugin.py
            xoneai plugin template --with-hook
        """
        try:
            from xoneaiagents.plugins.discovery import get_plugin_template
        except ImportError:
            typer.echo("Error: xoneaiagents not installed", err=True)
            raise typer.Exit(1)

"""
Gateway CLI commands for XoneAI.
...
class GatewayHandler:
    ...
    def start(
        ...
        try:
            ...
            from xoneaiagents.gateway import GatewayConfig
        except ImportError as e:
            print(f"Error: Gateway requires additional dependencies. {e}")
            print("Install with: pip install xoneai[api]")
            return
...
def handle_gateway_command(args) -> int:
    ...
    if isinstance(args, list):
        parser = argparse.ArgumentParser(
            prog="xoneai gateway",
            description="Manage the XoneAI Gateway server",
        )

"""
Persistence CLI feature for XoneAI.
...
- xoneai persistence resume  - Resume a session
"""
...
def print_persistence_help():
    ...
    help_text = """
XoneAI Persistence Commands
==============================

Usage: xoneai persistence <command> [options]
...
Doctor Command:
  xoneai persistence doctor [options]
  ...
  Environment Variables:
    XONE_CONVERSATION_URL  Default conversation store URL
    XONE_KNOWLEDGE_URL     Default knowledge store URL
    XONE_STATE_URL         Default state store URL

Run Command:
  xoneai persistence run [options] "prompt"
...
Resume Command:
  xoneai persistence resume --session-id ID [options]
...
Export/Import Commands:
  xoneai persistence export --session-id ID [options]
  xoneai persistence import --file FILE [options]
...
Examples:
  ...
  xoneai persistence doctor --conversation-url "postgresql://localhost/mydb"
  
  # Run agent with persistence
  xoneai persistence run --session-id "my-session" "Hello, remember my name is Alice"
  
  # Resume a session
  xoneai persistence resume --session-id "my-session" --show-history
  
  # Continue a session
  xoneai persistence resume --session-id "my-session" --continue "What's my name?"
  
  # Export a session
  xoneai persistence export --session-id "my-session" --output session.jsonl
  
  # Import a session
  xoneai persistence import --file session.jsonl
"""
    print(help_text)
...
def parse_persistence_args(args: list) -> dict:
    ...
    config = {
        "session_id": None,
        "user_id": "default",
        "conversation_url": os.getenv("XONE_CONVERSATION_URL"),
        "knowledge_url": os.getenv("XONE_KNOWLEDGE_URL"),
        "state_url": os.getenv("XONE_STATE_URL"),
...
def handle_doctor(args: list) -> bool:
    ...
    print("=" * 50)
    print("XoneAI Persistence Doctor")
    print("=" * 50)
    ...
    if config["conversation_url"] or config["all_stores"]:
        ...
        if url:
            ...
            print("\n[Conversation Store] Not configured (set --conversation-url or XONE_CONVERSATION_URL)")
    ...
    if config["knowledge_url"] or config["all_stores"]:
        ...
        if url:
            ...
            print("\n[Knowledge Store] Not configured (set --knowledge-url or XONE_KNOWLEDGE_URL)")
    ...
    if config["state_url"] or config["all_stores"]:
        ...
        if url:
            ...
            print("\n[State Store] Not configured (set --state-url or XONE_STATE_URL)")
...
def handle_run(args: list) -> bool:
    ...
    if not config["prompt"]:
        ...
        print("Usage: xoneai persistence run [options] \"your prompt\"")
        return True
    
    if not config["conversation_url"]:
        print("Error: No conversation URL configured")
        print("Set --conversation-url or XONE_CONVERSATION_URL")
        return True
...
def handle_migrate(args: list) -> bool:
    ...
    try:
        from xoneaiagents import db
        from xoneai.persistence.migrations import MigrationManager
...
def _test_conversation_store(url: str) -> tuple:
    ...
    try:
        from xoneai.persistence.factory import create_conversation_store
...
def _test_knowledge_store(url: str) -> tuple:
    ...
    try:
        from xoneai.persistence.factory import create_knowledge_store
...
def _test_state_store(url: str) -> tuple:
    ...
    try:
        from xoneai.persistence.factory import create_state_store

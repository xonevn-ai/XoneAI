...
Usage: xoneai session list
       xoneai session show <name>
       xoneai session resume <name>
       xoneai session delete <name>
"""
...
class SessionHandler(CommandHandler):
    ...
    def __init__(self, verbose: bool = False, user_id: str = "xone"):
    ...
    def get_help_text(self) -> str:
        return """
Session Commands:
  xoneai session list                 - List all saved sessions
  xoneai session show <name>          - Show session details
  xoneai session resume <name>        - Resume a session (load into memory)
  xoneai session delete <name>        - Delete a session
...
"""
    ...
    def _get_memory(self):
        ...
        if self._memory is None:
            try:
                from xoneaiagents.memory.file_memory import FileMemory
                self._memory = FileMemory(user_id=self.user_id)
            except ImportError:
                self.print_status(
                    "Session requires xoneaiagents. Install with: pip install xoneaiagents",
                    "error"
                )
                return None
        return self._memory
    ...
    def action_show(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai session show <name>", "error")
            return {}
    ...
    def action_resume(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai session resume <name>", "error")
            return {}
    ...
    def action_delete(self, args: List[str], **kwargs) -> bool:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai session delete <name>", "error")
            return False

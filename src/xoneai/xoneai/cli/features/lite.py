...
Provides access to the lightweight xoneaiagents.lite subpackage.
Usage: xoneai lite [run|info]
"""
...
class LiteHandler(FlagHandler):
    ...
    Commands:
        xoneai lite run "prompt"     - Run a lite agent with custom LLM
        xoneai lite info             - Show lite package info
        xoneai lite example          - Show example usage
    """
    ...
    def check_dependencies(self) -> Tuple[bool, str]:
        ...
        if importlib.util.find_spec("xoneaiagents.lite") is not None:
            return True, ""
        return False, "Lite package requires xoneaiagents>=0.5.0"
    ...
    def get_info(self) -> Dict[str, Any]:
        ...
        """
        import importlib.util
        if importlib.util.find_spec("xoneaiagents.lite") is None:
            return {
                "available": False,
                "error": "xoneaiagents.lite not found"
            }
    ...
    def show_example(self) -> str:
        ...
        """
        example = '''
# Example: Using xoneaiagents.lite with custom LLM

from xoneaiagents.lite import LiteAgent, tool
...
from xoneaiagents.lite import create_openai_llm_fn
...
'''
    ...
    def run_lite_agent(
        ...
        """
        try:
            from xoneaiagents.lite import LiteAgent, create_openai_llm_fn, create_anthropic_llm_fn
    ...
    def handle(self, subcommand: str = "info", **kwargs) -> Any:
        ...
        if subcommand == "info":
            ...
            if info.get("available"):
                print("xoneaiagents.lite - Lightweight Agent Package")
                print("=" * 50)

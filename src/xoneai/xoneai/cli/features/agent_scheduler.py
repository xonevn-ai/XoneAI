...
class AgentSchedulerHandler:
    ...
    def handle_daemon_command(subcommand: str, args, unknown_args=None) -> int:
        ...
        Returns:
            Exit code
        """
        from xoneai.scheduler.state_manager import SchedulerStateManager
        from xoneai.scheduler.daemon_manager import DaemonManager
    ...
    def _handle_start(args, unknown_args, state_manager, daemon_manager) -> int:
        ...
        if not unknown_args:
            ...
            print("\nUsage:")
            print('  xoneai schedule start <name> "Your task" --interval hourly')
            print('  xoneai schedule start <name> --recipe <recipe-name> --interval hourly')
            print("\nExample:")
            print('  xoneai schedule start news-checker "Check AI news" --interval hourly')
            print('  xoneai schedule start news-checker --recipe news-monitor --interval hourly')
            return 1
        ...
        if existing and daemon_manager.get_status(existing.get('pid', 0))['is_alive']:
            ...
            print(f"   Use 'xoneai schedule stop {name}' to stop it first")
            return 1
        ...
        print(f"   PID: {pid}")
        print(f"   Logs: ~/.xoneai/logs/{name}.log")
        print(f"\nManage:")
        print(f"   xoneai schedule list")
        print(f"   xoneai schedule describe {name}")
        print(f"   xoneai schedule logs {name} --follow")
        print(f"   xoneai schedule stop {name}")
    ...
    def _handle_stop(unknown_args, state_manager, daemon_manager) -> int:
        ...
        if not unknown_args:
            ...
            print("\nUsage: xoneai schedule stop <name>")
            return 1
        ...
        if not state:
            ...
            print("\nList schedulers with: xoneai schedule list")
            return 1
    ...
    def _handle_logs(unknown_args, daemon_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: xoneai schedule logs <name> [-f]")
            return 1
    ...
    def _handle_restart(unknown_args, state_manager, daemon_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: xoneai schedule restart <name>")
            return 1
    ...
    def _handle_delete(unknown_args, state_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: xoneai schedule delete <name>")
            return 1
    ...
    def _handle_describe(unknown_args, state_manager, daemon_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: xoneai schedule describe <name>")
            return 1
    ...
    def _handle_save(unknown_args, state_manager) -> int:
        ...
        if not unknown_args:
            print("❌ Error: Please provide scheduler name")
            print("\nUsage: xoneai schedule save <name> [output.yaml]")
            return 1
        ...
        try:
            ...
            print(f"\nRun with:")
            print(f"  xoneai schedule {output_file}")
            return 0
        except Exception as e:
    ...
    def handle_schedule_command(args, unknown_args, daemon_mode=False) -> int:
        ...
        Supports two modes:
        1. YAML mode: xoneai schedule agents.yaml
        2. Prompt mode: xoneai schedule "Your task here" --interval hourly
        ...
        """
        try:
            from xoneai.scheduler import AgentScheduler
            from xoneaiagents import Agent
        except ImportError:
            print("Error: xoneai.scheduler or xoneaiagents module not found")
            print("Please ensure XoneAI is properly installed")
            print("pip install xoneai xoneaiagents")
            return 1
        ...
        if verbose:
            ...
            logging.getLogger('xoneai.scheduler').setLevel(logging.WARNING)
        ...
        try:
            ...
            if first_arg and not is_yaml_mode:
                ...
                if ' ' not in first_arg and not first_arg.endswith('.yaml') and not first_arg.endswith('.yml'):
                    try:
                        from xoneai.recipe.bridge import resolve
            ...
            if is_yaml_mode:
                ...
                if not first_arg:
                    ...
                    print("\nExamples:")
                    print("  xoneai schedule agents.yaml")
                    print("  xoneai schedule my-recipe --interval hourly")
                    print('  xoneai schedule "Check news every hour" --interval hourly')
                    return 1

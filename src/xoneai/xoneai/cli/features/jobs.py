"""
Jobs CLI Feature for XoneAI.

Provides CLI commands for managing async jobs.

Commands:
- xoneai run submit <prompt>     # Submit a new job
- xoneai run status <job_id>     # Get job status
- xoneai run result <job_id>     # Get job result
- xoneai run cancel <job_id>     # Cancel a job
- xoneai run list                # List jobs
"""
...
def handle_run_command(args: List[str], verbose: bool = False):
    ...
    Usage:
        xoneai run submit "Your prompt here" [--wait] [--agent-file agents.yaml]
        xoneai run status <job_id>
        xoneai run result <job_id>
        xoneai run cancel <job_id>
        xoneai run list [--status <status>]
    """
    import argparse
    
    parser = argparse.ArgumentParser(prog="xoneai run", description="Manage async jobs")
    ...
    submit_parser.add_argument("--framework", default="xoneai", help="Framework to use")

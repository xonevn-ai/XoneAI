"""
n8n Integration Handler for XoneAI CLI.

Converts XoneAI agents.yaml workflows to n8n workflow JSON format
...
class N8nHandler(FlagHandler):
    ...
    Converts XoneAI YAML workflows to n8n-compatible JSON format
    ...
    def __init__(self, verbose: bool = False, n8n_url: str = "http://localhost:5678",
        ...
        Args:
            ...
            use_execute_command: Use Execute Command nodes (runs xoneai directly)
                                 instead of HTTP Request nodes
            api_url: XoneAI API URL that n8n will call (for tunnel/cloud deployments)
        """
        super().__init__(verbose=verbose)
        self.n8n_url = n8n_url
        self.xoneai_api_url = api_url
    ...
    def execute(self, yaml_path: str, n8n_url: Optional[str] = None, 
        ...
        if workflow_id:
            ...
            if self._activate_workflow(workflow_id, api_key):
                ...
                if trigger_node:
                    webhook_path = trigger_node["parameters"].get("path", "xoneai")
    ...
    def _create_workflow_via_api(self, workflow_json: Dict[str, Any], 
        ...
        try:
            ...
            workflow_data = {
                "name": workflow_json.get("name", "XoneAI Workflow"),
    ...
    def convert_yaml_to_n8n(self, yaml_path: str, use_webhook: bool = True) -> Dict[str, Any]:
        """
        Convert a XoneAI YAML workflow to n8n JSON format.
        ...
        workflow_name = yaml_content.get('name', 'XoneAI Workflow')
    ...
    def _create_trigger_node(self, use_webhook: bool = False, 
                              webhook_path: str = "xoneai") -> Dict[str, Any]:
    ...
    def _create_per_agent_node(self, agent_id: str, agent_config: Dict,
        ...
        return {
            ...
            "parameters": {
                "method": "POST",
                "url": f"{self.xoneai_api_url}/agents/{agent_url_id}",
                "sendBody": True,
                "specifyBody": "json",
    ...
    def _create_xoneai_workflow_node(self, workflow_name: str,
                                         agent_names: List[str],
                                         position: List[int]) -> Dict[str, Any]:
        """
        Create a single HTTP Request node that triggers the entire XoneAI workflow.
        
        The XoneAI API will run all agents sequentially when called.
        ...
        return {
            "id": "xoneai_workflow",
            "name": f"XoneAI: {workflow_name}",
            "type": "n8n-nodes-base.httpRequest",
            "typeVersion": 4.2,
            "position": position,
            "parameters": {
                "method": "POST",
                "url": f"{self.xoneai_api_url}/agents",
                "sendBody": True,
                "specifyBody": "json",

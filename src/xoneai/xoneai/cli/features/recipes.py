...
- xone recipes list - List available recipes
- xone recipes info <name> - Show recipe details
- xone recipes run <name> <input> - Run a recipe
- xone recipes doctor <name> - Check recipe dependencies
- xone recipes explain <name> - Show recipe execution plan
- xone recipes init <name> - Initialize recipe in current directory
"""
...
RECIPE_PATHS = [
    Path.home() / ".xone" / "templates",
    Path.home() / ".config" / "xone" / "templates",
    Path.cwd() / ".xone" / "templates",
    Path("/Users/xonevn/Agent-Recipes/agent_recipes/templates"),
]
...
def check_dependencies(recipe: Dict[str, Any]) -> Dict[str, bool]:
    ...
    for tool in requires.get("tools", []):
        try:
            from xoneai_tools.recipe_tools import __all__ as available_tools
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    if not args.skip_checks:
        ...
        if missing:
            ...
            print("Run 'xone recipes doctor {args.name}' for details.")
            if not args.force:
                return 1
    ...
    # TODO: Implement actual recipe execution via xoneaiagents
...
def handle_recipes_command(args: argparse.Namespace) -> int:
    ...
    if not hasattr(args, "func"):
        ...
        print("Usage: xone recipes <command> [options]")
        print("\nCommands:")

"""
Onboarding wizard for XoneAI bots.
...
class OnboardWizard:
    ...
    def run(self) -> None:
        ...
        if Confirm.ask("\nInstall as background service (daemon)?", default=False):
            try:
                from xoneai.daemon import install_daemon
        ...
        console.print(Panel(
            ...
            f"Start your bot:\n"
            f"  [cyan]xoneai bot start --config {self.config_path}[/cyan]\n\n"
            f"Check health:\n"
            f"  [cyan]xoneai doctor[/cyan]\n\n"
            f"View status:\n"
            f"  [cyan]xoneai bot status[/cyan]",
            title="✅ Done",
            border_style="green",
        ))
    ...
    def _run_plain(self) -> None:
        ...
        with open("bot.yaml", "w") as f:
            f.write(yaml_content)
        print("\n✓ Written to bot.yaml")
        print("Start with: xoneai bot start --config bot.yaml")


def run_onboard() -> None:
    """Entry point for 'xoneai onboard' command."""

"""
Checkpoints CLI Feature for XoneAI.

Provides CLI commands for managing file-level checkpoints.

Commands:
- xoneai checkpoint save <message>     # Save a checkpoint
- xoneai checkpoint restore <id>       # Restore to a checkpoint
- xoneai checkpoint list               # List all checkpoints
- xoneai checkpoint diff [from] [to]   # Show diff between checkpoints
- xoneai checkpoint delete             # Delete all checkpoints
"""
...
def handle_checkpoint_command(args: List[str], workspace_dir: Optional[str] = None, verbose: bool = False):
    ...
    Usage:
        xoneai checkpoint save <message>     # Save a checkpoint
        xoneai checkpoint restore <id>       # Restore to a checkpoint
        xoneai checkpoint list [--limit N]   # List checkpoints
        xoneai checkpoint diff [from] [to]   # Show diff
        xoneai checkpoint delete             # Delete all checkpoints
    """
    handler = CheckpointsHandler(workspace_dir=workspace_dir, verbose=verbose)
    
    if not args:
        print("Usage: xoneai checkpoint <command> [options]")
        print("\nCommands:")
    ...
    if command == "save":
        if len(args) < 2:
            print("Usage: xoneai checkpoint save <message>")
            return
        ...
        if len(args) < 2:
            print("Usage: xoneai checkpoint restore <checkpoint_id>")
            return

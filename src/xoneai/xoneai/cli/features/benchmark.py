"""
Comprehensive Benchmarking CLI for XoneAI.

Provides deep profiling and comparison across all execution paths:
- OpenAI SDK (baseline)
- XoneAI Agent (Python SDK)
- XoneAI CLI (fast path)
...
Usage: xoneai benchmark profile "What is 2+2?"
"""
...
class ModuleBreakdown:
    ...
    xoneai_modules: List[Tuple[str, float]] = field(default_factory=list)  # (file, cumulative_ms)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            "xoneai": [{"file": f, "cumulative_ms": t} for f, t in self.xoneai_modules[:20]],
...
class BenchmarkHandler(FlagHandler):
    """
    Comprehensive benchmarking handler for XoneAI.
    
    Commands:
        xoneai benchmark profile [query]  - Run full benchmark suite
        xoneai benchmark sdk [query]      - Benchmark OpenAI SDK only
        xoneai benchmark agent [query]    - Benchmark Agent only
        xoneai benchmark cli [query]      - Benchmark CLI only
        xoneai benchmark compare [query]  - Quick comparison table
    """
    ...
    PATHS = [
        ...
        "xoneai_cli",
        "xoneai_cli_profile",
        "xoneai_workflow_single",
        "xoneai_workflow_multi",
        "xoneai_litellm",
        "litellm_standalone",
    ]
    ...
    def _extract_module_breakdown(self, functions: List[FunctionStat]) -> ModuleBreakdown:
        ...
        for func in functions:
            ...
            if "xoneai" in file_path and "agents" not in file_path:
                xoneai_modules[func.file] = xoneai_modules.get(func.file, 0) + cumul
            elif "xoneaiagents" in file_path or ("xoneai" in file_path and "agents" in file_path):
        ...
        return ModuleBreakdown(
            xoneai_modules=to_sorted_list(xoneai_modules),
    ...
    def benchmark_xoneai_agent_deep(self, prompt: str, iteration: int, is_cold: bool, limit: int = 30) -> BenchmarkRun:
        """Benchmark XoneAI Agent with deep cProfile profiling (in-process)."""
        t0 = time.perf_counter()
        ...
        try:
            ...
            return BenchmarkRun(
                path_name="xoneai_agent",
                iteration=iteration,
                is_cold=is_cold,
                timings=PhaseTimings(),
                success=False,
                error="xoneaiagents not installed"
            )
...
'''
    ...
    def benchmark_xoneai_agent(self, prompt: str, iteration: int, is_cold: bool) -> BenchmarkRun:
        """Benchmark XoneAI Agent (Python SDK)."""
        code = f'''
import time
import json
import sys
sys.path.insert(0, "{self._get_project_root()}/src/xoneai")
...
'''
    ...
    def benchmark_xoneai_cli(self, prompt: str, iteration: int, is_cold: bool, with_profile: bool = False) -> BenchmarkRun:
        """Benchmark XoneAI CLI."""
        t0 = time.perf_counter()
        
        cmd = ["xoneai", prompt, "--llm", self.DEFAULT_MODEL]
        if with_profile:
            cmd.append("--profile")
        ...
        return BenchmarkRun(
            path_name="xoneai_cli_profile" if with_profile else "xoneai_cli",
            iteration=iteration,
            is_cold=is_cold,
            timings=PhaseTimings(
                total_ms=total_ms,
    ...
    def benchmark_xoneai_workflow_single(self, prompt: str, iteration: int, is_cold: bool) -> BenchmarkRun:
        """Benchmark XoneAI Workflow with single agent."""
        code = f'''
import time
import json
import sys
...
'''
        ...
        return BenchmarkRun(
            path_name="xoneai_workflow_single",
            iteration=iteration,
            is_cold=is_cold,
            timings=PhaseTimings(
                import_ms=timings.get("import_ms", 0),
    ...
    def benchmark_xoneai_workflow_multi(self, prompt: str, iteration: int, is_cold: bool) -> BenchmarkRun:
        """Benchmark XoneAI Workflow with multiple agents."""
        code = f'''
import time
import json
import sys
...
'''
        ...
        return BenchmarkRun(
            path_name="xoneai_workflow_multi",
            iteration=iteration,
            is_cold=is_cold,
            timings=PhaseTimings(
                import_ms=timings.get("import_ms", 0),
    ...
    def benchmark_xoneai_litellm(self, prompt: str, iteration: int, is_cold: bool) -> BenchmarkRun:
        """Benchmark XoneAI Agent using LiteLLM path (openai/ prefix)."""
        code = f'''
import time
import json
import sys
...
'''
    ...
    def run_full_benchmark(self, prompt: str = None, iterations: int = None, 
        ...
        if verbose:
            print("=" * 70)
            print("XoneAI Comprehensive Benchmark Suite")
            print("=" * 70)
        ...
        if deep:
            ...
            benchmark_methods = {
                ...
                "xoneai_agent": lambda p, i, c: self.benchmark_xoneai_agent_deep(p, i, c, limit=limit),
                "xoneai_cli": lambda p, i, c: self.benchmark_xoneai_cli(p, i, c, with_profile=False),
                "xoneai_cli_profile": lambda p, i, c: self.benchmark_xoneai_cli(p, i, c, with_profile=True),
                "xoneai_workflow_single": self.benchmark_xoneai_workflow_single,
                "xoneai_workflow_multi": self.benchmark_xoneai_workflow_multi,
                "xoneai_litellm": self.benchmark_xoneai_litellm,
                "litellm_standalone": self.benchmark_litellm_standalone,
            }
        else:
            benchmark_methods = {
                "openai_sdk": self.benchmark_openai_sdk,
                "xoneai_agent": self.benchmark_xoneai_agent,
                "xoneai_cli": lambda p, i, c: self.benchmark_xoneai_cli(p, i, c, with_profile=False),
                "xoneai_cli_profile": lambda p, i, c: self.benchmark_xoneai_cli(p, i, c, with_profile=True),
                "xoneai_workflow_single": self.benchmark_xoneai_workflow_single,
                "xoneai_workflow_multi": self.benchmark_xoneai_workflow_multi,
        ...
        for path_name in paths:
            ...
            if verbose:
                deep_indicator = " [DEEP]" if deep and path_name in ["openai_sdk", "xoneai_agent"] else ""
    ...
    def handle(self, subcommand: str = "profile", prompt: str = None, 
        ...
        if subcommand == "profile":
            ...
            # Agent only
            report = self.run_full_benchmark(prompt, iterations, paths=["openai_sdk", "xoneai_agent"])
            self.print_report(report)
            return report
        
        elif subcommand == "cli":
            # CLI only
            report = self.run_full_benchmark(prompt, iterations, paths=["openai_sdk", "xoneai_cli", "xoneai_cli_profile"])
            self.print_report(report)
            return report

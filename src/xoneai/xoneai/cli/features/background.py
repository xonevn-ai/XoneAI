"""
Background Agents CLI Feature for XoneAI.

Provides CLI commands for managing background agent tasks.

Commands:
- xoneai background list              # List background tasks
- xoneai background status <id>       # Get task status
...
class BackgroundHandler:
    ...
    async def _get_runner(self):
        ...
        if self._runner is None:
            from xoneaiagents.background import BackgroundRunner
    ...
    async def list_tasks(self, status: Optional[str] = None) -> List[dict]:
        ...
        status_enum = None
        if status:
            from xoneaiagents.background import TaskStatus
            status_map = {
...
def handle_background_command(args: List[str], verbose: bool = False):
    ...
    Usage:
        xoneai background list [--status <status>] [--json] [--page N] [--page-size N]
        xoneai background status <task_id> [--json]
        xoneai background cancel <task_id> [--json]
        xoneai background clear [--all] [--older-than SEC] [--json]
        xoneai background submit --recipe <name> [--input JSON] [--session-id ID] [--timeout SEC] [--json]
    """
    import argparse
    import json
    
    parser = argparse.ArgumentParser(prog="xoneai background", description="Manage background tasks")

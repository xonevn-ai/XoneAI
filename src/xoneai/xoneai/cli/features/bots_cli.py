...
class BotHandler:
    ...
    def start_telegram(
        ...
        if capabilities and capabilities.auto_approve:
            os.environ["XONEAI_AUTO_APPROVE"] = "true"
            logger.info("Auto-approve enabled for all tool executions")
        
        try:
            from xoneai.bots import TelegramBot
    ...
    def start_discord(
        ...
        try:
            from xoneai.bots import DiscordBot
        except ImportError as e:
    ...
    def start_slack(
        ...
        try:
            from xoneai.bots import SlackBot
        except ImportError as e:
    ...
    def start_whatsapp(
        ...
        try:
            from xoneai.bots import WhatsAppBot
        except ImportError as e:
            print(f"Error: WhatsAppBot import failed. {e}")
            if mode == "web":
                print("Install with: pip install 'xoneai[bot-whatsapp-web]'")
            else:
    ...
    def _build_tools(self, capabilities: BotCapabilities) -> List:
        ...
        try:
            from xoneaiagents.tools import execute_command, search_web
            tools.append(execute_command)
            tools.append(search_web)
            logger.info("Default tools enabled: execute_command, search_web")
        except ImportError:
            logger.warning("Default tools not available from xoneaiagents.tools")
        ...
        if capabilities.web_search:
            try:
                ...
                logger.warning("Web search tool not available. Install xoneai-tools.")
        ...
        if capabilities.tts:
            try:
                from xoneai.tools.audio import create_tts_tool
        ...
        if capabilities.stt:
            try:
                from xoneai.tools.audio import create_stt_tool
    ...
    def _resolve_tool_by_name(self, name: str):
        ...
        try:
            import xoneai_tools
            tool_class = getattr(xoneai_tools, name, None)
            if tool_class:
                return tool_class()
        except ImportError:
            pass
...
Examples:
  # Basic bot
  xoneai bot telegram --token $TELEGRAM_BOT_TOKEN

  # Bot with agent YAML
  xoneai bot slack --token $SLACK_BOT_TOKEN --agent agents.yaml

  # Bot with browser and web search
  xoneai bot discord --token $DISCORD_BOT_TOKEN --browser --web

  # Bot with tools
  xoneai bot telegram --token $TELEGRAM_BOT_TOKEN --tools DuckDuckGoTool WikipediaTool

  # Full-featured bot
  xoneai bot slack --token $SLACK_BOT_TOKEN --agent agents.yaml --browser --web --memory --model gpt-4o
""",
        )
    ...
    if platform == "telegram":
        ...
        print("")
        print("Usage: xoneai bot <platform> [options]")
        print("")
        ...
        print("")
        print("Examples:")
        print("  xoneai bot telegram --token $TELEGRAM_BOT_TOKEN")
        print("  xoneai bot slack --token $SLACK_BOT_TOKEN --agent agents.yaml --browser --web")
        print("  xoneai bot discord --token $DISCORD_BOT_TOKEN --tools DuckDuckGoTool --memory")
        return 1

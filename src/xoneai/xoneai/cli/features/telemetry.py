...
class TelemetryHandler(FlagHandler):
    ...
    def check_dependencies(self) -> Tuple[bool, str]:
        ...
        if _load_telemetry():
            return True, ""
        return False, "Telemetry requires xoneaiagents[telemetry]. Install with: pip install xoneaiagents[telemetry]"
    ...
    def enable(self, **kwargs) -> bool:
        ...
        if not _load_telemetry():
            self.print_status(
                "Telemetry not available. Install with: pip install xoneaiagents[telemetry]",
                "warning"
            )
            return False

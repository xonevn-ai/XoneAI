...
Usage: xoneai workflow run research.yaml
       xoneai workflow validate research.yaml
       xoneai workflow list
       xoneai workflow create --template routing
"""
...
class WorkflowHandler(CommandHandler):
    ...
    Example:
        xoneai workflow run research.yaml --var topic="AI"
        xoneai workflow validate research.yaml
        xoneai workflow list
        xoneai workflow create --template routing --output my_workflow.yaml
    """
...
"""
    ...
    def get_help_text(self) -> str:
        return """
Workflow Commands:
  xoneai workflow run <file.yaml>           - Run a YAML workflow
  xoneai workflow run <file.yaml> --var key=value  - Run with variables
  xoneai workflow validate <file.yaml>      - Validate a YAML workflow
  xoneai workflow list                      - List workflows in .xone/workflows/
  xoneai workflow create --template <name>  - Create from template
  xoneai workflow auto "topic" --pattern <pattern>  - Auto-generate workflow
...
"""
    ...
    def _get_manager(self):
        ...
        if self._manager is None:
            try:
                ...
                self.print_status(
                    "Workflows require xoneaiagents. Install with: pip install xoneaiagents",
                    "error"
                )
                return None
    ...
    def action_run(self, args: List[str], **kwargs) -> Any:
        ...
        if not file_path:
            self.print_status("Usage: xoneai workflow run <file.yaml>", "error")
            return None
    ...
    def action_validate(self, args: List[str], **kwargs) -> bool:
        ...
        file_path = self._get_file_from_args(args)
        if not file_path:
            self.print_status("Usage: xoneai workflow validate <file.yaml>", "error")
            return False
    ...
    def action_list(self, args: List[str], **kwargs) -> List[str]:
        """
        List available workflows in .xone/workflows/ directory.
        
        Returns:
            List of workflow file names
        """
        workflows_dir = Path(".xone/workflows")
        
        if not workflows_dir.exists():
            self.print_status("No .xone/workflows/ directory found", "warning")
            self.print_status("Create one with: mkdir -p .xone/workflows", "info")
            return []
        ...
        if not yaml_files:
            self.print_status("No workflow files found in .xone/workflows/", "warning")
            return []
    ...
    def action_create(self, args: List[str], **kwargs) -> Optional[str]:
        ...
        if not template_name:
            self.print_status("Usage: xoneai workflow create --template <name>", "error")
        ...
        self.print_status(f"Run with: xoneai workflow run {output_file}", "info")
    ...
    def action_auto(self, args: List[str], **kwargs) -> Optional[str]:
        ...
        if not topic:
            self.print_status('Usage: xoneai workflow auto "topic" --pattern <pattern>', "error")

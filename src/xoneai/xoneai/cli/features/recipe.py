...
All commands use the canonical `xoneai recipe` prefix.
"""
...
class RecipeHandler:
    ...
    def _print_help(self):
        """Print help message."""
        help_text = """
[bold cyan]XoneAI Recipe[/bold cyan]

[bold]Usage:[/bold]
  xoneai recipe <command> [options]
...
[bold]Commands:[/bold]
  ...
  pack <recipe>     Create a recipe bundle (.xone)
...
[bold]Examples:[/bold]
  xoneai recipe list
  xoneai recipe search video
  xoneai recipe info transcript-generator
  xoneai recipe validate support-reply
  xoneai recipe run support-reply --input '{"ticket_id": "T-123"}'
  xoneai recipe run transcript-generator ./audio.mp3 --json
  xoneai recipe init my-recipe
  xoneai recipe serve --port 8765
"""
    ...
    def cmd_list(self, args: List[str]) -> int:
        ...
        try:
            ...
            if parsed["registry"]:
                from xoneai.recipe.registry import get_registry
                import os
                registry = get_registry(
                    registry=parsed["registry"],
                    token=parsed["token"] or os.environ.get("XONEAI_REGISTRY_TOKEN")
                )
    ...
    def _run_background(
        ...
        try:
            from xoneai.recipe.operations import run_background
            ...
            if json_output:
                ...
                print(f"\nCheck status with: xoneai background status {task.task_id}")
...
```bash
xoneai recipe run {parsed["name"]} --input "your input"
```
...
'''
            ...
            print(f"  3. xoneai recipe validate {parsed['name']}")
            print(f"  4. xoneai recipe run {parsed['name']} --input 'test'")
    ...
    def cmd_pack(self, args: List[str]) -> int:
        ...
        try:
            ...
            output_name = parsed["output"] or f"{info.name}-{info.version}.xone"
    ...
    def cmd_export(self, args: List[str]) -> int:
        ...
        try:
            from xoneai.recipe.history import get_history
    ...
    def cmd_serve(self, args: List[str]) -> int:
        ...
        if parsed["config"]:
            try:
                from xoneai.recipe.serve import load_config
        ...
        try:
            from xoneai.recipe.serve import serve
            ...
            self._print_error("Serve dependencies not installed. Run: pip install xoneai[serve]")
    ...
    def cmd_publish(self, args: List[str]) -> int:
        ...
        try:
            ...
            if bundle_path.is_dir():
                ...
                bundle_name = f"{info.name}-{info.version}.xone"
            ...
            if parsed["json"]:
                ...
                print(f"  Registry: {parsed['registry'] or '~/.xone/registry'}")
    ...
    def cmd_sbom(self, args: List[str]) -> int:
        ...
        try:
            from xoneai.recipe.security import generate_sbom
    ...
    def cmd_audit(self, args: List[str]) -> int:
        ...
        try:
            from xoneai.recipe.security import audit_dependencies
    ...
    def cmd_sign(self, args: List[str]) -> int:
        ...
        try:
            from xoneai.recipe.security import sign_bundle
    ...
    def cmd_verify(self, args: List[str]) -> int:
        ...
        try:
            from xoneai.recipe.security import verify_bundle
    ...
    def cmd_policy(self, args: List[str]) -> int:
        ...
        try:
            from xoneai.recipe.policy import PolicyPack, get_default_policy
    ...
    def cmd_judge(self, args: List[str]) -> int:
        ...
        Usage:
            xoneai recipe judge <trace_id> [options]
        ...
        if not parsed.get("trace_id"):
            ...
            print("\nUsage: xoneai recipe judge <trace_id> [--memory|--knowledge|--context]")
            print("\nExamples:")
            print("  xoneai recipe judge run-abc123")
            print("  xoneai recipe judge run-abc123 --memory")
            print("  xoneai recipe judge run-abc123 --knowledge")
            print("  xoneai recipe judge run-abc123 --yaml agents.yaml --output plan.yaml")
            return self.EXIT_VALIDATION_ERROR
        ...
        try:
            from xoneai.replay import (
                ContextTraceReader,
                ContextEffectivenessJudge,
            )
            from xoneai.replay.judge import (
            ...
            if yaml_file:
                ...
                if not dry_run:
                    ...
                    print(f"   Run 'xoneai recipe apply {out_file}' to apply fixes")
            else:
    ...
    def cmd_apply(self, args: List[str]) -> int:
        ...
        Usage:
            xoneai recipe apply <plan_file> [options]
        ...
        if not parsed.get("plan_file"):
            ...
            print("\nUsage: xoneai recipe apply <plan_file> [--confirm] [--dry-run]")
            return self.EXIT_VALIDATION_ERROR
        ...
        try:
            from xoneai.replay.judge import JudgePlan
    ...
    def cmd_create(self, args: List[str]) -> int:
        ...
        Usage:
            xoneai recipe create "GOAL" [options]
        ...
        if not parsed.get("goal"):
            ...
            print("\nUsage: xoneai recipe create \"GOAL\" [options]")
            print("\nExamples:")
            print('  xoneai recipe create "Build a web scraper for news"')
            print('  xoneai recipe create "Research AI trends" --no-optimize')
            print('  xoneai recipe create "Research AI" --agents "researcher:role=AI Researcher,goal=Find papers"')
            print('  xoneai recipe create "Research AI" --tools "researcher:internet_search,arxiv"')
            return self.EXIT_VALIDATION_ERROR
        ...
        try:
            ...
            print(f"   Run with: xoneai recipe run {recipe_path.name}")
    ...
    def cmd_optimize(self, args: List[str]) -> int:
        ...
        Usage:
            xoneai recipe optimize <NAME> [WHAT] [options]
        ...
        if not parsed.get("name"):
            self._print_error("Recipe name required")
            print("\nUsage: xoneai recipe optimize <NAME> [WHAT] [options]")
            print("\nExamples:")
            print('  xoneai recipe optimize my-recipe')
            print('  xoneai recipe optimize my-recipe "improve error handling"')
            return self.EXIT_VALIDATION_ERROR

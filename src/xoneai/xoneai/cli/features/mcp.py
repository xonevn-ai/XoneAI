...
Usage: xoneai "prompt" --mcp "npx -y @modelcontextprotocol/server-filesystem ."
"""
...
class MCPHandler(FlagHandler):
    ...
    Example:
        xoneai "List files" --mcp "npx -y @modelcontextprotocol/server-filesystem ."
        xoneai "Search web" --mcp "npx -y @modelcontextprotocol/server-brave-search" --mcp-env "BRAVE_API_KEY=xxx"
    """
    ...
    def check_dependencies(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if importlib.util.find_spec("xoneaiagents") is not None:
                ...
                if MCP is not None:
                    return True, ""
            return False, "MCP requires xoneaiagents with MCP support"
        except ImportError:

"""
ACP health checks for XoneAI Doctor.
"""
...
def check_acp_module(config=None) -> CheckResult:
    ...
    try:
        ...
        spec = importlib.util.find_spec("xoneai.acp")
        ...
        if spec is not None:
            ...
            return CheckResult(
                ...
                remediation="ACP module should be part of xoneai package"
            )
...
def check_acp_server(config=None) -> CheckResult:
    ...
    try:
        from xoneai.acp import ACPServer  # noqa: F401
...
def check_acp_config(config=None) -> CheckResult:
    ...
    acp_conf = {
        "approval_mode": os.environ.get("XONEAI_APPROVAL_MODE", "manual"),
        "allow_write": os.environ.get("XONEAI_ALLOW_WRITE", "false"),
        "allow_shell": os.environ.get("XONEAI_ALLOW_SHELL", "false"),
    }

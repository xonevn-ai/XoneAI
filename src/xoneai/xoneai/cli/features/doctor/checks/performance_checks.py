...
@register_check(
    id="performance_xoneai_import",
    title="XoneAI Import Time",
    description="Measure xoneai package import time",
    category=CheckCategory.PERFORMANCE,
    severity=CheckSeverity.INFO,
)
def check_performance_xoneai_import(config: DoctorConfig) -> CheckResult:
    """Measure xoneai package import time."""
    budget_ms = config.budget_ms or 2000  # 2 second default budget
    
    # xoneai is already imported, measure a fresh import of a submodule
    start = time.perf_counter()
    try:
        # Import version module (lightweight)
        from xoneai import version
        elapsed = (time.perf_counter() - start) * 1000
...
@register_check(
    id="performance_xoneaiagents_import",
    title="XoneAI Agents Import Time",
    description="Measure xoneaiagents package import time",
    category=CheckCategory.PERFORMANCE,
    severity=CheckSeverity.INFO,
)
def check_performance_xoneaiagents_import(config: DoctorConfig) -> CheckResult:
    """Measure xoneaiagents package import time."""
...
def check_performance_loaded_modules(config: DoctorConfig) -> CheckResult:
    ...
    return CheckResult(
        ...
        category=CheckCategory.PERFORMANCE,
        status=CheckStatus.PASS,
        message=f"{module_count} modules loaded (xone: {xone_count})",
        metadata={
            "total": module_count,
            "xone": xone_count,

...
@register_check(
    id="obs_xoneai_telemetry",
    title="XoneAI Telemetry",
    description="Check XoneAI built-in telemetry",
    category=CheckCategory.OBSERVABILITY,
    severity=CheckSeverity.INFO,
)
def check_obs_xoneai_telemetry(config: DoctorConfig) -> CheckResult:
    """Check XoneAI built-in telemetry."""
    telemetry_enabled = os.environ.get("XONEAI_TELEMETRY_ENABLED", "").lower() in ("true", "1", "yes")
    ...
    try:
        ...
        if telemetry_enabled:
            return CheckResult(
                ...
                category=CheckCategory.OBSERVABILITY,
                status=CheckStatus.PASS,
                message="XoneAI telemetry enabled",
            )
        else:
            return CheckResult(
                id="obs_xoneai_telemetry",
                ...
                category=CheckCategory.OBSERVABILITY,
                status=CheckStatus.SKIP,
                message="XoneAI telemetry available but not enabled",
                details="Set XONEAI_TELEMETRY_ENABLED=true to enable",
            )
    except ImportError:
        return CheckResult(
            id="obs_xoneai_telemetry",
            ...
            category=CheckCategory.OBSERVABILITY,
            status=CheckStatus.SKIP,
            message="XoneAI telemetry module not available",
        )

...
def check_agents_yaml_schema(config: DoctorConfig) -> CheckResult:
    ...
    if "framework" in data:
        ...
        if framework not in ["xoneai", "crewai", "autogen"]:
...
@register_check(
    id="xone_config_dir",
    title=".xone Config Directory",
    description="Check .xone configuration directory",
    category=CheckCategory.CONFIG,
    severity=CheckSeverity.INFO,
)
def check_xone_config_dir(config: DoctorConfig) -> CheckResult:
    """Check .xone configuration directory."""
    home_dir = Path.home()
    cwd = Path.cwd()
    ...
    if found:
        ...
        return CheckResult(
            ...
            category=CheckCategory.CONFIG,
            status=CheckStatus.SKIP,
            message="No .xone config directory found (will be created on first use)",
        )

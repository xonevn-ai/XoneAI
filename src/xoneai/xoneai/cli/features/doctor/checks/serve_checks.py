...
def check_serve_module(config: DoctorConfig) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            remediation="pip install xoneai[serve]",
        )
...
def check_endpoints_cli(config: DoctorConfig) -> CheckResult:
    ...
    try:
        ...
        return CheckResult(
            ...
            remediation="pip install xoneai",
        )
    except Exception as e:
        return CheckResult(
            id="endpoints_cli",
...
@register_check(
    ...
    description="Check if a XoneAI server is reachable",
...
def check_server_connectivity(config: DoctorConfig) -> CheckResult:
    """Check if a XoneAI server is reachable."""
    import urllib.request
    import urllib.error
    
    url = os.environ.get("XONEAI_ENDPOINTS_URL", "http://localhost:8765")
    ...
    try:
        ...
        return CheckResult(
            ...
            remediation="Start a server: xoneai serve unified --port 8765",
        )
    except Exception as e:
        return CheckResult(
            id="server_connectivity",
...
def check_discovery_endpoint(config: DoctorConfig) -> CheckResult:
    ...
    url = os.environ.get("XONEAI_ENDPOINTS_URL", "http://localhost:8765")
    discovery_url = f"{url}/__xoneai__/discovery"
...
def check_provider_adapters(config: DoctorConfig) -> CheckResult:
    ...
    try:
        from xoneai.endpoints.providers import (
            BaseProvider,
            RecipeProvider,
            AgentsAPIProvider,
            MCPProvider,

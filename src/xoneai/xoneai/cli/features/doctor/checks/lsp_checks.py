"""
LSP health checks for XoneAI Doctor.
"""
...
def check_lsp_module(config=None) -> CheckResult:
    ...
    try:
        ...
        spec = importlib.util.find_spec("xoneaiagents.lsp")
        ...
        if spec is not None:
            return CheckResult(
                ...
                metadata={"module": "xoneaiagents.lsp"}
            )
        else:
            return CheckResult(
                id="lsp_module",
                ...
                remediation="Install xoneaiagents with LSP support"
            )
...
def check_lsp_client(config=None) -> CheckResult:
    ...
    try:
        from xoneaiagents.lsp import LSPClient  # noqa: F401

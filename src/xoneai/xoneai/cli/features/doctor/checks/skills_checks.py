...
def _find_skills_dirs() -> list:
    ...
    locations = [
        Path.cwd() / ".xone" / "skills",
        Path.cwd() / ".claude" / "skills",
        Path.home() / ".xone" / "skills",
        Path.home() / ".config" / "xone" / "skills",
    ]
...
def check_skills_dirs(config: DoctorConfig) -> CheckResult:
    ...
    if dirs:
        ...
        return CheckResult(
            ...
            details="Create .xone/skills/ to add agent skills",
        )
...
@register_check(
    id="skills_xoneai_integration",
    title="XoneAI Skills Integration",
    description="Check XoneAI skills module",
    category=CheckCategory.SKILLS,
    severity=CheckSeverity.LOW,
)
def check_skills_xoneai_integration(config: DoctorConfig) -> CheckResult:
    """Check XoneAI skills module."""
    try:
        from xoneaiagents.skills import SkillManager
        return CheckResult(
            id="skills_xoneai_integration",
            title="XoneAI Skills Integration",
            category=CheckCategory.SKILLS,
            status=CheckStatus.PASS,
            message="XoneAI skills module available",
        )

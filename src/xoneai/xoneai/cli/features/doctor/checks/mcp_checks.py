...
def _find_mcp_config() -> dict:
    ...
    locations = [
        Path.cwd() / ".xone" / "mcp.json",
        Path.cwd() / ".xone" / "mcp" / "config.json",
        Path.home() / ".xone" / "mcp.json",
        Path.home() / ".config" / "xone" / "mcp.json",
    ]
...
def check_mcp_config(config: DoctorConfig) -> CheckResult:
    ...
    if mcp_config:
        ...
        return CheckResult(
            ...
            details="Create .xone/mcp.json to configure MCP servers",
        )
...
@register_check(
    id="mcp_xoneai_integration",
    title="XoneAI MCP Integration",
    description="Check XoneAI MCP integration",
    category=CheckCategory.MCP,
    severity=CheckSeverity.LOW,
)
def check_mcp_xoneai_integration(config: DoctorConfig) -> CheckResult:
    """Check XoneAI MCP integration."""
    try:
        ...
        return CheckResult(
            ...
            category=CheckCategory.MCP,
            status=CheckStatus.PASS,
            message="XoneAI MCP integration available",
        )
        ...
        return CheckResult(
            ...
            details=str(e),
            remediation="Install with: pip install xoneaiagents[mcp]",
        )

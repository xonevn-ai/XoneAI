"""
Bot-specific doctor checks for XoneAI.
...
def check_bot_config(config_path: str = "bot.yaml") -> CheckResult:
    ...
    if not os.path.exists(config_path):
        return CheckResult(
            ...
            remediation="Run 'xoneai onboard' to create one",
            duration_ms=(time.time() - start) * 1000,
        )
    try:
        from xoneai.bots._config_schema import load_and_validate_bot_yaml
        ...
        return CheckResult(
            ...
            remediation="Fix errors in bot.yaml or run 'xoneai onboard'",
            duration_ms=(time.time() - start) * 1000,
        )

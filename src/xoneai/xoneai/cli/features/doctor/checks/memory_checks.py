...
def _find_memory_dirs() -> list:
    ...
    locations = [
        Path.cwd() / ".xone" / "memory",
        Path.cwd() / ".xone" / "sessions",
        Path.home() / ".xone" / "memory",
        Path.home() / ".xone" / "sessions",
        Path.home() / ".config" / "xone" / "memory",
    ]
...
def check_memory_chromadb(config: DoctorConfig) -> CheckResult:
    ...
    chroma_dirs = [
        Path.cwd() / ".xone" / "chroma",
        Path.cwd() / "chroma_db",
        Path.home() / ".xone" / "chroma",
    ]
...
@register_check(
    id="memory_xoneai_module",
    title="XoneAI Memory Module",
    description="Check XoneAI memory module",
    category=CheckCategory.MEMORY,
    severity=CheckSeverity.LOW,
)
def check_memory_xoneai_module(config: DoctorConfig) -> CheckResult:
    """Check XoneAI memory module."""
    try:
        from xoneaiagents.memory import FileMemory, Memory
        return CheckResult(
            id="memory_xoneai_module",
            title="XoneAI Memory Module",
            category=CheckCategory.MEMORY,

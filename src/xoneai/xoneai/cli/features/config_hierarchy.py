"""
Hierarchical Configuration System for XoneAI CLI.
...
CONFIG_FILES = [
    ".xone.json",      # Project-local (hidden)
    "xone.json",       # Project-local (visible)
]

USER_CONFIG_PATH = "~/.config/xone/xone.json"
GLOBAL_CONFIG_PATH = "/etc/xone/xone.json"
...
class HierarchicalConfig:
    ...
    1. Project-level (.xone.json or xone.json in current dir)
    2. User-level (~/.config/xone/xone.json)
    3. Global-level (/etc/xone/xone.json)
    ...
    def save_project_config(self, config: Dict[str, Any], hidden: bool = True) -> None:
        ...
        filename = ".xone.json" if hidden else "xone.json"
        path = os.path.join(self.project_dir, filename)

...
class ToolsHandler(CommandHandler):
    ...
    def get_help_text(self) -> str:
        return """
Tools Commands:
  xoneai tools list                   - List all available tools
  xoneai tools info <name>            - Show tool details
  xoneai tools search <query>         - Search tools by name/description
  xoneai tools doctor                 - Diagnose tool availability and dependencies
  xoneai tools doctor --json          - Output diagnosis as JSON
  xoneai tools resolve <name>         - Resolve a tool name to its source
  xoneai tools discover               - Discover tools from installed packages
  xoneai tools show-sources           - Show all tool sources for a template
  xoneai tools add <source>           - Add tools from package, file, or GitHub
  xoneai tools add-sources <source>   - Add a tool source to persistent config
  xoneai tools remove-sources <source> - Remove a tool source from config

Add Examples:
  xoneai tools add pandas             - Wrap pandas functions as tools
  xoneai tools add ./my_tools.py      - Add local tools file
  xoneai tools add github:user/repo   - Add tools from GitHub
...
"""
    ...
    def _get_registry(self):
        ...
        if self._registry is None:
            try:
                from xoneaiagents.tools import get_registry
                self._registry = get_registry()
            except ImportError:
                self.print_status(
                    "Tools require xoneaiagents. Install with: pip install xoneaiagents",
                    "error"
                )
                return None
    ...
    def action_info(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai tools info <tool_name>", "error")
            return {}
    ...
    def action_search(self, args: List[str], **kwargs) -> List[str]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai tools search <query>", "error")
            return []
    ...
    def action_doctor(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        try:
            from xoneai.templates.tools_doctor import ToolsDoctor
    ...
    def action_resolve(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai tools resolve <tool_name> [--template <template>]", "error")
            return {}
    ...
    def action_discover(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                pkg_tools.append("xoneai_tools.video")
            except ImportError:
                pass
            
            # Check for tools module
            try:
                import xoneai_tools.tools as ext_tools
            ...
            if pkg_tools:
                discovered["xoneai_tools"] = pkg_tools
        except ImportError:
            pass
        
        # Try xoneaiagents built-in tools
        try:
            from xoneaiagents.tools import TOOL_MAPPINGS
            discovered["xoneaiagents.tools"] = list(TOOL_MAPPINGS.keys())[:20]  # Limit
        except ImportError:
            pass
    ...
    def action_show_sources(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        sources = {
            "built_in": "xoneaiagents.tools.TOOL_MAPPINGS",
            "package_discovery": ["xoneai_tools"],
            "default_dirs": [
                "~/.xone/tools",
                "~/.config/xone/tools",
            ],
    ...
    def action_add(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        Examples:
            xoneai tools add pandas
            xoneai tools add ./my_tools.py
            xoneai tools add github:user/repo/tools
        """
        if not args:
            self.print_status("Usage: xoneai tools add <source>", "error")
        ...
        if source.startswith("./") or source.startswith("/") or source.endswith(".py"):
            ...
            if path.exists():
                # Copy to ~/.xone/tools/
                tools_dir = Path.home() / ".xone" / "tools"
        ...
        # Assume it's a package name - but warn that packages need wrapper tools
            ...
            try:
                ...
                self.print_status("   1. Create a file: ~/.xone/tools/my_tools.py", "info")
    ...
    def action_add_sources(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai tools add-sources <source>", "error")
            return {"success": False, "error": "No source provided"}
    ...
    def action_remove_sources(self, args: List[str], **kwargs) -> Dict[str, Any]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai tools remove-sources <source>", "error")
            return {"success": False, "error": "No source provided"}

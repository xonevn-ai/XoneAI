...
Usage: xoneai perf [benchmark|check|import-time|memory]
"""
...
class PerformanceHandler(FlagHandler):
    ...
    Commands:
        xoneai perf benchmark     - Run full performance benchmark
        xoneai perf import-time   - Measure import time only
        xoneai perf memory        - Measure memory usage only
        xoneai perf check         - Run regression check against targets
        xoneai perf lazy-check    - Verify lazy imports are working
    """
    ...
    def measure_import_time(self, runs: int = 5) -> Dict[str, Any]:
        """
        Measure import time for xoneaiagents.
...
"""
    ...
    def measure_memory(self) -> Dict[str, Any]:
        """
        Measure memory usage after importing xoneaiagents.

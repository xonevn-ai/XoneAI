...
Provides CLI commands for launching XoneAI servers:
- xoneai serve agents - Launch agents as HTTP API
- xoneai serve recipe - Launch recipe runner
- xoneai serve mcp - Launch MCP server
- xoneai serve tools - Launch tools as MCP server
- xoneai serve a2a - Launch A2A server
- xoneai serve a2u - Launch A2U event stream server
- xoneai serve unified - Launch unified server with all providers

All servers include the unified discovery endpoint at /__xoneai__/discovery.
"""
...
class ServeHandler:
    ...
    def _print_help(self):
        """Print help message."""
        help_text = """
[bold cyan]XoneAI Serve[/bold cyan]

Launch XoneAI servers with unified discovery support.

[bold]Usage:[/bold]
  xoneai serve <command> [options]
...
[bold]Examples:[/bold]
  xoneai serve agents --file agents.yaml --port 8000
  xoneai serve recipe --port 8765
  xoneai serve mcp --transport http --port 8080
  xoneai serve tools --port 8081
  xoneai serve a2a --port 8082
  xoneai serve unified --port 8765

[bold]Discovery:[/bold]
  All servers expose /__xoneai__/discovery for unified endpoint discovery.
  Use `xoneai endpoints` to interact with any server.
"""
    ...
    def cmd_agents(self, args: List[str]) -> int:
        ...
        try:
            ...
            print("  Discovery: /__xoneai__/discovery")
            ...
            print("Install with: pip install xoneai[serve]")
            return self.EXIT_GENERAL_ERROR
    ...
    def _create_agents_app(self, config: Dict[str, Any]) -> Any:
        ...
        from xoneai.endpoints.discovery import (
            create_discovery_document,
            EndpointInfo,
            ProviderInfo,
        )
        from xoneai.endpoints.server import add_discovery_routes
        ...
        discovery = create_discovery_document(server_name="xoneai-agents")
        ...
        # Create app
        app = FastAPI(
            title="XoneAI Agents API",
            description="HTTP API for XoneAI Agents",
        )
        ...
        async def root():
            return {
                "message": "XoneAI Agents API",
                "endpoints": [path],
                "discovery": "/__xoneai__/discovery",
            }
    ...
    def cmd_recipe(self, args: List[str]) -> int:
        ...
        try:
            ...
            from xoneai.recipe.serve import serve, load_config
    ...
    def cmd_mcp(self, args: List[str]) -> int:
        """Launch MCP server (DEPRECATED - use 'xoneai mcp serve' instead)."""
        import sys
        
        # Print deprecation warning
        print("\n[yellow]⚠ DEPRECATION WARNING:[/yellow]", file=sys.stderr)
        print("[yellow]'xoneai serve mcp' is deprecated and will be removed in a future version.[/yellow]", file=sys.stderr)
        print("[yellow]Please use 'xoneai mcp serve' instead.[/yellow]\n", file=sys.stderr)
        
        # Redirect to new MCP server CLI
        try:
            from xoneai.mcp_server.cli import handle_mcp_command
    ...
    def _create_mcp_app(self, config: Dict[str, Any]) -> Any:
        ...
        # Create discovery document
        discovery = create_discovery_document(server_name="xoneai-mcp")
        discovery.add_provider(ProviderInfo(
            type="mcp",
            name="MCP Server",
    ...
    def cmd_tools(self, args: List[str]) -> int:
        """Launch tools as MCP server (DEPRECATED - use 'xoneai mcp serve' instead)."""
        import sys
        
        # Print deprecation warning
        print("\n[yellow]⚠ DEPRECATION WARNING:[/yellow]", file=sys.stderr)
        print("[yellow]'xoneai serve tools' is deprecated and will be removed in a future version.[/yellow]", file=sys.stderr)
        print("[yellow]Please use 'xoneai mcp serve' instead.[/yellow]\n", file=sys.stderr)
    ...
    def _create_tools_app(self, config: Dict[str, Any]) -> Any:
        ...
        discovery = create_discovery_document(server_name="xoneai-tools-mcp")
        discovery.add_provider(ProviderInfo(
            type="tools-mcp",
        ...
        async def root():
            return {
                "message": "XoneAI Tools MCP Server",
                "discovery": "/__xoneai__/discovery",
            }
    ...
    def _create_a2a_app(self, config: Dict[str, Any]) -> Any:
        ...
        discovery = create_discovery_document(server_name="xoneai-a2a")
        discovery.add_provider(ProviderInfo(
            type="a2a",
            name="A2A Protocol",
        ...
        async def agent_card():
            ...
            return {
                "name": "XoneAI Agent",
                "description": "XoneAI Agent via A2A protocol",
                "version": "1.0.0",
        ...
        async def status():
            ...
            return {"status": "ok", "name": "XoneAI A2A", "version": "1.0.0"}
    ...
    def _create_a2u_app(self, config: Dict[str, Any]) -> Any:
        ...
        from xoneai.endpoints.server import add_discovery_routes
        from xoneai.endpoints.a2u_server import create_a2u_routes
        
        discovery = create_discovery_document(server_name="xoneai-a2u")
        discovery.add_provider(ProviderInfo(
            type="a2u",
    ...
    def _create_unified_app(self, config: Dict[str, Any]) -> Any:
        ...
        discovery = create_discovery_document(server_name="xoneai-unified")
        ...
        app = FastAPI(
            title="XoneAI Unified Server",
            description="Unified server with all XoneAI providers",
        )
        ...
        async def agent_card():
            return {
                "name": "XoneAI Unified",
                "description": "XoneAI Unified Server",
                "version": "1.0.0",
            }

...
Usage:
    ...
    xoneai knowledge query "How to authenticate?"
    xoneai knowledge list
    xoneai knowledge clear
    xoneai knowledge stats
"""
...
class KnowledgeHandler(CommandHandler):
    ...
    Example:
        xoneai knowledge add document.pdf
        xoneai knowledge add ./docs/
        xoneai knowledge query "How to authenticate?" --retrieval fusion
        xoneai knowledge list
        xoneai knowledge clear
        xoneai knowledge stats
    """
    ...
    def get_help_text(self) -> str:
        return """
Knowledge Commands:
  xoneai knowledge add <file|dir|url>   - Add document(s) to knowledge base
  xoneai knowledge query "<question>"   - Query knowledge base with RAG
  xoneai knowledge list                 - List indexed documents
  xoneai knowledge clear                - Clear knowledge base
  xoneai knowledge stats                - Show knowledge base statistics
  xoneai knowledge export <file.json>   - Export knowledge base to JSON file
  xoneai knowledge import <file.json>   - Import knowledge base from JSON file
  xoneai knowledge help                 - Show this help message
...
Examples:
  xoneai knowledge add document.pdf
  xoneai knowledge add ./docs/
  xoneai knowledge query "How to authenticate?" --retrieval-strategy fusion
  xoneai knowledge query "Compare approaches" --reranker simple --query-mode sub_question
"""
    ...
    def _get_knowledge(self):
        ...
        if self._knowledge is None:
            try:
                from xoneaiagents import Knowledge
                ...
                config = {
                    "vector_store": {
                        ...
                        "config": {
                            "path": os.path.join(self.workspace, ".xone", "knowledge")
                        }
                    }
                }
                ...
                self.print_status(
                    "Knowledge requires xoneaiagents. Install with: pip install xoneaiagents[knowledge]",
                    "error"
                )
                return None
    ...
    def action_add(self, args: List[str], **kwargs) -> bool:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai knowledge add <file|directory|url|glob>", "error")
            return False
    ...
    def action_query(self, args: List[str], **kwargs) -> List[Dict[str, Any]]:
        ...
        """
        if not args:
            self.print_status("Usage: xoneai knowledge query <question>", "error")
            return []
    ...
    def action_import(self, args: List[str], **kwargs) -> bool:
        ...
        if not args:
            self.print_status("Usage: xoneai knowledge import <file.json>", "error")
            return False

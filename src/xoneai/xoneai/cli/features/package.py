...
All commands use the canonical `xoneai` or `xoneai package` prefix.
...
class PackageConfig:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        if os.environ.get("XONEAI_PACKAGE_INDEX_URL"):
            self._config["package"]["index_url"] = os.environ["XONEAI_PACKAGE_INDEX_URL"]
        
        if os.environ.get("XONEAI_PACKAGE_EXTRA_INDEX_URLS"):
            urls = os.environ["XONEAI_PACKAGE_EXTRA_INDEX_URLS"].split(",")
...
class PackageHandler:
    ...
    def _print_help(self):
        """Print help message."""
        print("""
XoneAI Package Commands

Usage: xoneai package <command> [options]
       xoneai install <spec...>
       xoneai uninstall <pkg...>

Commands:
  install     Install Python packages (shortcut: xoneai install)
  uninstall   Uninstall Python packages (shortcut: xoneai uninstall)
  list        List installed packages
...
Examples:
  ...
  xoneai install "requests>=2.28" httpx
  xoneai uninstall requests
  xoneai package list
  xoneai package search langchain
  xoneai package index show
  xoneai package index set https://my-index.example.com/simple
...
""")
    ...
    def _index_add(self, args: List[str]) -> int:
        ...
        print(f"  xoneai install <pkg> --extra-index-url {args[0]} --allow-extra-index")
    ...
    def _index_remove(self, args: List[str]) -> int:
        ...
        print(f"  xoneai package index set {DEFAULT_INDEX_URL}")

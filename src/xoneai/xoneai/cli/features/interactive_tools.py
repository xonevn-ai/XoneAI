"""
Interactive Default Tools Provider for XoneAI.

This module provides the canonical source of truth for default tools
in interactive modes (TUI and `xone "prompt"`).
...
class ToolConfig:
    ...
    def from_env(cls) -> "ToolConfig":
        ...
        disable_str = os.environ.get("XONE_TOOLS_DISABLE", "")
        if disable_str:
        ...
        # Check for workspace
        workspace = os.environ.get("XONE_WORKSPACE", "")
        if workspace:
            config.workspace = workspace
        
        # Check approval mode
        approval = os.environ.get("XONE_APPROVAL_MODE", "")
...
def _load_basic_tools() -> Dict[str, Callable]:
    """Lazy load basic tools from xoneaiagents."""
    tools = {}
    
    try:
        from xoneaiagents.tools import read_file
    ...
    try:
        from xoneaiagents.tools import list_files
...
def get_interactive_tools(
    ...
    Both `xoneai tui launch` and `xone "prompt"` should use this.

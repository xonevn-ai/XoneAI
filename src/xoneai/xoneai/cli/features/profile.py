...
Usage: xoneai profile <subcommand> [options]
"""
...
def run_benchmark(prompt: str, model: str = None, iterations: int = 1, verbose: bool = False) -> List[ProfileResult]:
    ...
    try:
        ...
        print("[red]ERROR: xoneaiagents not installed. Install with: pip install xoneaiagents[/red]")
        return []
...
def handle_profile_command(args: List[str]) -> int:
    ...
    Returns:
        Exit code
    """
    parser = argparse.ArgumentParser(prog="xoneai profile", description="Performance profiling and benchmarking")
    ...
    if not parsed_args.subcommand:
        parser.print_help()
        print("\n[bold]Examples:[/bold]")
        print("  xoneai profile benchmark \"What is 2+2?\"")
        print("  xoneai profile benchmark \"Write a poem\" --model gpt-4o --iterations 3")
        print("  xoneai profile compare \"Explain AI\" --models gpt-4o,gpt-4o-mini")
        return 0
    ...
    if parsed_args.subcommand in ['benchmark', 'bench', 'run']:
        if not parsed_args.prompt:
            ...
            print("Usage: xoneai profile benchmark \"Your prompt here\"")
            return 1

"""
Documentation Parity Generator for XoneAI SDKs.

Compares each SDK's implemented features against its documentation.
- Python: xoneaiagents features vs docs/concepts, docs/features, etc.
- Rust: xoneai-rust features vs docs/rust/
- TypeScript: xoneai-ts features vs docs/js/
...
class DocsParityGenerator:
    ...
    def __init__(
        ...
        if docs_root is None:
            docs_root = Path("/Users/xonevn/XoneAIDocs/docs")
        ...
        # Output paths
        self.python_md_output = self.repo_root / "src" / "xoneai-agents" / "DOCS_PARITY.md"
        self.ts_md_output = self.repo_root / "src" / "xoneai-ts" / "DOCS_PARITY.md"
        self.rust_md_output = self.repo_root / "src" / "xoneai-rust" / "DOCS_PARITY.md"
    ...
    def _write_markdown(self, data: Dict, output_path: Path, sdk_name: str, docs_path: str, check: bool = False) -> int:
        ...
        lines.append("---")
        lines.append("")
        lines.append("*Generated by `xoneai._dev.parity.docs_generator`*")
        lines.append("")
    ...
    def _create_doc_stub(self, category_name: str, display_name: str, docs_dir: Path, sdk_name: str) -> bool:
        ...
        content = f'''---
title: "{display_name}"
description: "{display_name} features in XoneAI {sdk_name} SDK"
---
...
def main():
    ...
    parser.add_argument(
        "--copy-docs",
        action="store_true",
        help="Copy parity files to XoneAIDocs folder (default: off)"
    )
    parser.add_argument(
        "--update-nav",
        action="store_true",

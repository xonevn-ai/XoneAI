"""
Feature Parity Tracker Generator for XoneAI SDKs.
...
class ParityTrackerGenerator:
    ...
    def __init__(self, repo_root: Optional[Path] = None):
        ...
        # Output paths
        self.ts_output = self.repo_root / "src" / "xoneai-ts" / "FEATURE_PARITY_TRACKER.json"
        self.ts_md_output = self.repo_root / "src" / "xoneai-ts" / "PARITY.md"
        self.rust_output = self.repo_root / "src" / "xoneai-rust" / "FEATURE_PARITY_TRACKER.json"
        self.rust_md_output = self.repo_root / "src" / "xoneai-rust" / "PARITY.md"
    ...
    def generate(self) -> dict:
        ...
        tracker = {
            ...
            'generatedBy': 'xoneai._dev.parity.generator',
            'sourceOfTruth': 'Python SDK (xoneaiagents)',
    ...
    def _get_current_version(self) -> str:
        ...
        pyproject = self.repo_root / "src" / "xoneai-agents" / "pyproject.toml"
        if pyproject.exists():
            try:
    ...
    def _build_python_section(self, features: PythonFeatures) -> dict:
        ...
        return {
            'path': str(self.repo_root / "src" / "xoneai-agents" / "xoneaiagents"),
    ...
    def _build_wrapper_section(self, features: PythonFeatures) -> dict:
        ...
        return {
            'path': str(self.repo_root / "src" / "xoneai" / "xoneai"),
    ...
    def _build_typescript_section(self, features: TypeScriptFeatures) -> dict:
        ...
        return {
            'path': str(self.repo_root / "src" / "xoneai-ts" / "src"),
    ...
    def write_rust(self, check: bool = False) -> int:
        ...
        tracker = {
            ...
            'rustSDK': {
                'path': str(self.repo_root / "src" / "xoneai-rust"),
    ...
    def generate_markdown(self) -> str:
        ...
        if tracker['pythonCoreSDK'].get('modules'):
            for module, exports in sorted(tracker['pythonCoreSDK']['modules'].items()):
                ...
                lines.append("")
                lines.append("```python")
                lines.append(f"from xoneaiagents import {', '.join(exports[:10])}{'...' if len(exports) > 10 else ''}")
                lines.append("```")
                lines.append("")
                lines.append("</details>")
                lines.append("")
        ...
        if tracker['typescriptSDK'].get('modules'):
            for module, exports in sorted(tracker['typescriptSDK']['modules'].items()):
                ...
                lines.append(f"import {{ {', '.join(exports[:10])}{'...' if len(exports) > 10 else ''} }} from 'xoneai';")
                lines.append("```")
                lines.append("")
                lines.append("</details>")
                lines.append("")
        ...
        lines.append("---")
        lines.append("")
        lines.append("*Generated by `xoneai._dev.parity.generator`*")
        lines.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate feature parity tracker for XoneAI SDKs"
    )
    parser.add_argument(
        '--target', '-t',

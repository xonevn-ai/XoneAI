...
Discovers and extracts xoneai CLI commands from bash code blocks
...
# Pattern to extract xoneai commands (handles multi-line with \)
XONEAI_CMD_PATTERN = re.compile(
    r'^(?:#\s*)?(xoneai\s+[^\n\\]+(?:\\\n[^\n]+)*)',
    re.MULTILINE
)
...
class CLIDocsSource:
    ...
    Parses bash code blocks in .md/.mdx files and extracts xoneai commands.
    ...
    def _extract_commands(self, doc_path: Path, group: str) -> List[RunItem]:
        ...
        for match in BASH_FENCE_PATTERN.finditer(content):
            ...
            # Extract xoneai commands from block
            for cmd_match in XONEAI_CMD_PATTERN.finditer(block_content):
                raw_cmd = cmd_match.group(1)
                
                # Clean up multi-line commands
                cmd = self._clean_command(raw_cmd)
                
                if not cmd or not cmd.startswith('xoneai'):
                    continue
    ...
    def _is_runnable(self, cmd: str) -> tuple:
        ...
        # Extract base command (xoneai <subcommand>)
        parts = cmd.split()
        if len(parts) < 2:
    ...
    def _create_test_command(self, cmd: str) -> str:
        ...
        parts = cmd.split()
        if len(parts) >= 2:
            # xoneai <subcommand> --help
            base_parts = [parts[0], parts[1]]
            
            # Handle nested subcommands (e.g., xoneai batch list)

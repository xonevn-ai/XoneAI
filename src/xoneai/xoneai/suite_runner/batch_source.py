"""
Batch Source - Adapter for running local Python files with XoneAI imports.

Discovers and prepares Python files in the current directory (or specified path)
that contain imports from xoneaiagents or xoneai packages.
...
# Import detection patterns - matches 'from xoneai...' or 'import xoneai...'
XONEAI_IMPORT_PATTERN = re.compile(
    r'^(?:from|import)\s+xoneai(?:agents)?\b',
    re.MULTILINE
)

# Agent-centric detection patterns (reused from examples_source)
AGENT_PATTERN = re.compile(r'\bAgent\s*\(')
AGENTS_PATTERN = re.compile(r'\b(?:Agents|XoneAIAgents)\s*\(')
WORKFLOW_PATTERN = re.compile(r'\bWorkflow\s*\(')

# Directive parsing regex (reused from examples_source)
DIRECTIVE_PATTERN = re.compile(r'^#\s*xoneai:\s*(\w+)=(.+)$', re.MULTILINE)
...
class BatchSource:
    """
    Discovers and prepares local Python files with XoneAI imports.
    ...
    def _has_xoneai_import(self, content: str) -> bool:
        """
        Check if file content contains XoneAI imports.
        
        Args:
            content: File content to check.
            
        Returns:
            True if file contains 'from xoneai...' or 'import xoneai...'.
        """
        ...
        for line in content.split('\n'):
            ...
            if stripped.startswith('#'):
                continue
            # Check for import
            if XONEAI_IMPORT_PATTERN.match(stripped):
                return True
        return False
    ...
    def discover(self) -> List[RunItem]:
        """
        Discover all Python files with XoneAI imports.
        
        Returns:
            List of RunItem objects for files that contain XoneAI imports.
        """
        ...
        for path in files:
            ...
            # Check for XoneAI imports
            if not self._has_xoneai_import(content):
                continue

...
logger = logging.getLogger("xoneai.browser.sessions")
...
class SessionManager:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        Args:
            db_path: Path to SQLite database. Defaults to ~/.xoneai/browser_sessions.db
        """
        if db_path is None:
            home = os.path.expanduser("~")
            xoneai_dir = os.path.join(home, ".xoneai")
            os.makedirs(xoneai_dir, exist_ok=True)
            db_path = os.path.join(xoneai_dir, "browser_sessions.db")

...
Usage:
    from xoneai.browser.diagnostics import run_all_diagnostics
...
logger = logging.getLogger("xoneai.browser.diagnostics")
...
async def check_bridge_server(host: str = "localhost", port: int = 8765) -> DiagnosticResult:
    ...
    try:
        ...
        return DiagnosticResult(
            ...
            details={"health_url": health_url, "hint": "Run: xoneai browser start"},
        )
    except Exception as e:
        return DiagnosticResult(
            name="bridge_server",
...
async def check_extension_loaded(port: int = 9222) -> DiagnosticResult:
    """Check if XoneAI extension is loaded in Chrome.
    
    Looks for service worker with 'xoneai' or known extension ID.
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=5)) as resp:
                ...
                extension_patterns = ["xoneai", "fkmfdklcegbbpipbcimbokpfcfamhpdc"]
                ...
                if service_worker:
                    ...
                    return DiagnosticResult(
                        name="extension_loaded",
                        status=DiagnosticStatus.PASS,
                        message="XoneAI extension loaded",
                    ...
                    return DiagnosticResult(
                        name="extension_loaded",
                        status=DiagnosticStatus.FAIL,
                        message="XoneAI extension not found",
                        details={
                            "port": port,

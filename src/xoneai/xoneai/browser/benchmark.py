"""
Browser Benchmark Module for XoneAI.
...
Usage:
    xoneai browser benchmark run          # Run all benchmarks
    xoneai browser benchmark quick        # Quick test (3 scenarios)
    xoneai browser benchmark report       # Show last results
...
logger = logging.getLogger("xoneai.browser.benchmark")
...
1. Type "XoneAI" in the search box
...
class BrowserBenchmark:
    ...
    def __init__(
        ...
        self.debug = debug
        self.results_dir = Path.home() / ".xoneai" / "browser_benchmarks"
    ...
    async def _ensure_bridge_server(self) -> bool:
        ...
        try:
            self._server_process = subprocess.Popen(
                [sys.executable, "-m", "xoneai.browser.server", "--port", str(self._bridge_port)],
    ...
    async def _ensure_chrome_with_extension(self) -> bool:
        ...
        extension_path = None
        candidates = [
            os.path.expanduser("~/xoneai-chrome-extension/dist"),
            str(Path(__file__).parent.parent.parent.parent.parent / "xoneai-chrome-extension" / "dist"),
        ]
        ...
        if not extension_path:
            print("❌ Extension not found. Build it with: cd ~/xoneai-chrome-extension && npm run build")
            return False
        
        # Create temp profile
        self._temp_profile = tempfile.mkdtemp(prefix="xoneai_benchmark_")
...
def add_benchmark_commands(app):
    ...
    def run_benchmark(
        ...
        Examples:
            xoneai browser benchmark run                    # Run all built-in tests
            xoneai browser benchmark run --quick            # Run 3 quick tests
            xoneai browser benchmark run tests.csv          # Run tests from CSV file
            xoneai browser benchmark run --output results.json
    ...
    def show_report():
        ...
        if report is None:
            print("No benchmark results found. Run 'xoneai browser benchmark run' first.")
            return
    ...
    def generate_template(
        ...
        Example:
            xoneai browser benchmark template my_tests.csv
        """
        import csv
        ...
        with open(output, 'w', newline='', encoding='utf-8') as f:
            ...
            writer.writerow(['Google Search', 'Type "XoneAI" in search box, press Enter, confirm results', 'https://www.google.com', '3', '15', 'search'])

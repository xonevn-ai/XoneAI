...
Uses XoneAI agents to decide browser actions based on observations.
"""

import logging
from typing import Optional, Dict, Any, List, Literal

logger = logging.getLogger("xoneai.browser.agent")
...
class BrowserAgent:
    ...
    This is a thin wrapper that creates a XoneAI agent with browser-specific
    ...
    def _ensure_agent(self):
        """Lazily initialize the XoneAI agent."""
        if self._agent is not None:
            return
        
        logger.debug(f"[AGENT][CALL] _ensure_agent:agent.py → Creating XoneAI Agent with model={self.model}")
        
        try:
            from xoneaiagents import Agent
        except ImportError:
            logger.error(f"[AGENT][ERROR] _ensure_agent:agent.py → xoneaiagents not installed")
            raise ImportError(
                "xoneaiagents is required. Install it with: pip install xoneaiagents"
            )

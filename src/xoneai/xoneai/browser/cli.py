"""Browser CLI commands for XoneAI.

Commands:
    xoneai browser start   - Start the browser automation server
    xoneai browser stop    - Stop the server
    xoneai browser sessions - List active sessions
"""
...
def start_server(
    ...
    to communicate with XoneAI agents.
    
    Example:
        xoneai browser start --port 8765 --model gpt-4o
    """
    try:
        from xoneai.browser import BrowserServer
...
def list_sessions(
    ...
    Example:
        xoneai browser sessions --status running
        xoneai browser sessions --limit 10
    """
    try:
        from xoneai.browser.sessions import SessionManager
    except ImportError as e:
        console.print(f"[red]Error:[/red] {e}")
        raise typer.Exit(1)
...
def show_history(
    ...
    Example:
        xoneai browser history abc12345
    """
    try:
        from xoneai.browser.sessions import SessionManager
    except ImportError as e:
...
def clear_sessions(
    ...
    Example:
        xoneai browser clear --status completed --yes
        xoneai browser clear --all --yes
    """
    try:
        from xoneai.browser.sessions import SessionManager
    except ImportError as e:
...
def _run_alternative_engine(
    ...
    if record_video and not actual_screenshot_dir:
        actual_screenshot_dir = str(Path.home() / ".xoneai" / "browser_screenshots" / datetime.now().strftime("%Y%m%d_%H%M%S"))
...
def run_agent(
    ...
    Launches Chrome with the XoneAI extension and executes the goal.
    ...
    Example:
        xoneai browser run "Search for XoneAI on Google"
        xoneai browser run "task" --engine cdp --headless
        xoneai browser run "task" --engine hybrid --vision
        xoneai browser run "task" --screenshots ./screenshots
    """
    import logging
    import json
    import asyncio
...
def manage_tabs(
    ...
    Example:
        xoneai browser tabs              # List all tabs
        xoneai browser tabs --new https://google.com
        xoneai browser tabs --close 12345
        xoneai browser tabs --focus 12345
    """
    import requests
    import json
...
def list_pages(
    ...
    Example:
        xoneai browser pages
        xoneai browser pages --json
    """
    try:
...
def get_dom_cmd(
    ...
    Example:
        xoneai browser pages  # Get page ID
        xoneai browser dom <page-id>
    """
    import asyncio
    try:
...
def read_content(
    ...
    Example:
        xoneai browser pages  # Get page ID
        xoneai browser content <page-id>
    """
    import asyncio
    try:
...
def get_console_logs(
    ...
    Example:
        xoneai browser pages  # Get page ID
        xoneai browser console <page-id>
    """
    import asyncio
    try:
...
def execute_javascript(
    ...
    Example:
        xoneai browser pages  # Get page ID
        xoneai browser js <page-id> "document.title"
        xoneai browser js <page-id> "document.querySelectorAll('a').length"
    """
    import asyncio
    try:
...
def navigate_tab(
    ...
    Example:
        xoneai browser navigate "https://google.com"
        xoneai browser navigate "https://github.com" --tab ABC123
    """
    import requests
    import json
...
def execute_script(
    ...
    Example:
        xoneai browser execute "document.title"
        xoneai browser execute "window.location.href"
        xoneai browser execute "document.querySelectorAll('a').length"
    """
    import requests
    import json
...
def capture_screenshot(
    ...
    Example:
        xoneai browser screenshot
        xoneai browser screenshot -o page.png
        xoneai browser screenshot --fullpage -o full.png
    """
    import requests
    import json
    import base64
...
def reload_extension(
    ...
    async def do_reload():
        ...
        try:
            ...
            # Find XoneAI extension service worker
            sw = next((t for t in targets if t.get('type') == 'service_worker' 
                       and 'xoneai' in t.get('url', '').lower()), None)
            ...
            if not sw:
                console.print("[yellow]⚠️ XoneAI extension not found[/yellow]")
...
def chrome_start(
    ...
    Example:
        xoneai browser chrome start              # Default port 9222
        xoneai browser chrome start -p 9333      # Custom port
        xoneai browser chrome start --headless   # Headless mode
    """
    import subprocess
    import platform
    import shutil
    ...
    try:
        ...
        for attempt in range(max_attempts):
            ...
            if asyncio.run(check_port()):
                ...
                console.print(f"  xoneai browser chrome status -p {port}")
                console.print(f"  xoneai browser extension load -p {port}")
                return
...
def chrome_status(
    ...
    Example:
        xoneai browser chrome status
        xoneai browser chrome status -p 9333
    """
    import asyncio
    ...
    async def check():
        ...
        console.print(f"[red]✗ Chrome not responding on port {port}[/red]")
        console.print(f"  Start with: xoneai browser chrome start -p {port}")
        return False
...
def extension_load(
    ...
    """Load XoneAI extension into Chrome.
    ...
    Example:
        xoneai browser extension load              # Default path
        xoneai browser extension load ~/my-ext     # Custom path
    """
    from pathlib import Path
    import json
    import webbrowser
    ...
    if path is None:
        path = str(Path.home() / "xoneai-chrome-extension" / "dist")
    ...
    if not ext_path.exists():
        ...
        console.print("")
        console.print("Expected location: ~/xoneai-chrome-extension/dist")
        console.print("Build the extension first:")
        console.print("  cd ~/xoneai-chrome-extension && npm run build")
        raise typer.Exit(1)
    ...
    if not chrome_running:
        ...
        console.print(f"Start Chrome first: xoneai browser chrome start -p {port}")
        raise typer.Exit(1)
...
def extension_validate(
    ...
    Example:
        xoneai browser extension validate
        xoneai browser extension validate ~/my-ext/dist
    """
    from pathlib import Path
...
def doctor_server(
    ...
    try:
        ...
        console.print("   Start with: xoneai browser start")
        raise typer.Exit(1)
    except Exception as e:
...
def doctor_extension(
    ...
    try:
        ...
        sw = next((t for t in targets if t.get('type') == 'service_worker' 
                   and ('xoneai' in t.get('url', '').lower() or 
...
def doctor_test_extension(
    ...
    Examples:
        xoneai browser doctor test-extension
        xoneai browser doctor test-extension --goal "Search for AI"
        xoneai browser doctor test-extension --timeout 30
    """
    import asyncio
    from rich.table import Table
    ...
    try:
        ...
        if result.get("success"):
            ...
            console.print("  1. Bridge server is running: xoneai browser start")
...
def doctor_db():
    ...
    db_path = Path.home() / ".xoneai" / "browser_sessions.db"
...
def doctor_bridge(
    ...
    Examples:
        xoneai browser doctor bridge
        xoneai browser doctor bridge --port 8766
    """
    import asyncio
...
def doctor_api_keys(
    ...
    Examples:
        xoneai browser doctor api-keys
        xoneai browser doctor api-keys --validate
        xoneai browser doctor api-keys --model gemini/gemini-2.0-flash --validate
    """
...
def doctor_agent(
    ...
    Examples:
        xoneai browser doctor agent
        xoneai browser doctor agent --model gemini/gemini-2.0-flash
    """
    import asyncio
...
def doctor_flow(
    ...
    Examples:
        xoneai browser doctor flow
        xoneai browser doctor flow --json
        xoneai browser doctor flow --skip-llm
    """
    import asyncio
...
def doctor_all(
    ...
    Examples:
        xoneai browser doctor
        xoneai browser doctor --json
    """
...
def launch_browser(
    ...
    temp_profile: bool = typer.Option(False, "--temp-profile", help="Use temporary profile (deleted after run). Default: ~/.xoneai/browser_profile"),
    chrome_profile: Optional[str] = typer.Option(None, "--chrome-profile", help="Custom Chrome profile dir. Default: ~/.xoneai/browser_profile"),
):
    """Launch Chrome with extension and optionally run a goal.
    
    This is an all-in-one command that:
    1. Finds the XoneAI Chrome extension
    ...
    Examples:
        xoneai browser launch                           # Just launch Chrome with extension
        xoneai browser launch "Search for AI"           # Launch and run goal (uses extension)
        xoneai browser launch "Search" --engine cdp     # Force CDP mode
        xoneai browser launch "Search" --engine extension  # Force extension mode
        xoneai browser launch --headless --no-server    # Headless mode, no server
    """
    import subprocess
    import platform
    import shutil
    ...
    log_dir = Path.home() / ".xoneai" / "browser_logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    
    # Setup screenshot directory if enabled
    screenshot_dir = None
    if screenshot or debug:
        screenshot_dir = Path.home() / ".xoneai" / "browser_screenshots" / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    if debug:
        ...
        # ===== XONEAI BROWSER LOGS =====
        # Enable DEBUG only for XoneAI browser components
        logging.getLogger("xoneai.browser").setLevel(logging.DEBUG)
        logging.getLogger("xoneai.browser.server").setLevel(logging.DEBUG)
        logging.getLogger("xoneai.browser.agent").setLevel(logging.DEBUG)
        logging.getLogger("xoneai.browser.sessions").setLevel(logging.DEBUG)
        logging.getLogger("xoneai.browser.cdp_agent").setLevel(logging.DEBUG)
        logging.getLogger("xoneai.browser.diagnostics").setLevel(logging.DEBUG)
    ...
    if system == "Darwin":  # macOS
        ...
        candidates = [
            ...
            os.path.join(home, ".xoneai/chrome-for-testing/chrome-mac-arm64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing"),
            os.path.join(home, ".xoneai/chrome-for-testing/chrome-mac-x64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing"),
    ...
    if extension_path is None:
        ext_candidates = [
            Path.home() / "xoneai-chrome-extension" / "dist",
            Path.cwd() / "xoneai-chrome-extension" / "dist",
            Path(__file__).parent.parent.parent.parent.parent / "xoneai-chrome-extension" / "dist",
        ]
    ...
    console.print(f"[bold blue]🚀 XoneAI Browser Launch[/bold blue]")
    ...
    if not no_server:
        ...
        if not server_running:
            ...
            server_process = subprocess.Popen(
                [sys.executable, "-m", "xoneai.browser.server", "--port", str(server_port)],
    ...
    DEFAULT_PROFILE_PATH = os.path.expanduser("~/.xoneai/browser_profile")
    
    # Determine profile to use: --temp-profile = temp, else persistent (default or custom)
    if temp_profile:
        # Use temp profile (deleted after run)
        profile_dir = tempfile.mkdtemp(prefix="xoneai_chrome_")
    ...
    async def verify_extension_loaded():
        """Verify XoneAI extension loaded by checking CDP targets and bridge connection."""
        try:
            import aiohttp
            async with aiohttp.ClientSession() as session:
                # Get CDP targets
                async with session.get(f"http://localhost:{port}/json/list", timeout=aiohttp.ClientTimeout(total=3)) as resp:
                    if resp.status == 200:
                        ...
                        for target in targets:
                            ...
                            # Quick check for XoneAI in title
                            if "XoneAI" in target_title:
                        ...
                        # Check each extension's manifest to find XoneAI
            ...
            async def run_goal():
                ...
                if selected_engine == "extension":
                    ...
                    try:
                        ...
                        async with aiohttp.ClientSession() as session:
                            async with session.get(f"http://localhost:{port}/json") as resp:
                                if resp.status == 200:
                                    targets = await resp.json()
                                    # Find the XoneAI extension service worker or background
                                    ...
                                    for t in targets:
                                        ...
                                        if 'chrome-extension' in target_url and 'XoneAI' not in target_url:
                                        ...
                                        if connections >= 1:
                                            ...
                                            if check_count == 1 or (check_count % 3 == 0):
                                                ...
                                                if debug and check_count == 1:
                                                    ...
                                                    console.print("[dim]   [DEBUG] Try: chrome://extensions/ -> XoneAI -> Click 'Reload'[/dim]")
                        except Exception as e:
                            if debug:
                    ...
                    if not extension_connected:
                        ...
                        console.print("[yellow]Hint: Open Chrome DevTools -> Extensions -> XoneAI -> Background page to check console[/yellow]")
        ...
        while True:
            try:
                ...
                if goal_input.lower() in ("help", "?"):
                    ...
                    console.print('  "Search for XoneAI on Google"')
                ...
                try:
                    ...
                    if record_video or screenshot:
                        goal_screenshot_dir = Path.home() / ".xoneai" / "browser_screenshots" / f"goal_{goal_count}_{datetime.now().strftime('%H%M%S')}"

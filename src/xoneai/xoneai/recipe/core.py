...
def get_template_search_paths() -> List[Path]:
    ...
    Precedence order:
    1. XONEAI_RECIPE_PATH environment variable (colon-separated)
    2. Current working directory ./recipes
    3. User home ~/.xone/recipes
    ...
    env_path = os.environ.get("XONEAI_RECIPE_PATH")
    if env_path:
    ...
    # 3. User home
    home_recipes = Path.home() / ".xone" / "recipes"
...
def reload_registry():
    ...
    import importlib
    try:
        from xoneai import recipe as recipe_module
...
def list_recipes(
    ...
    """
    try:
        from xoneai.templates import TemplateDiscovery
...
def _load_recipe(name: str, offline: bool = False) -> Optional[RecipeConfig]:
    ...
    try:
        from xoneai.templates import TemplateDiscovery, TemplateLoader
...
def _execute_recipe(
    ...
    try:
        ...
        from xoneai.templates.loader import TemplateConfig as LoaderTemplateConfig
        ...
        if "agents" in workflow_config and "tasks" in workflow_config:
            return _execute_xoneai_workflow(
...
def _execute_xoneai_workflow(
    workflow_config: Dict[str, Any],
    config: Dict[str, Any],
    tool_registry: Any,
    options: Dict[str, Any],
) -> Any:
    """Execute a XoneAI agents/tasks workflow."""
    from xoneaiagents import Agent, Task, Agents
    from xoneai.templates.tool_override import resolve_tools
...
def _execute_steps_workflow(
    ...
    """
    Execute a steps-based workflow using xoneaiagents Workflow.

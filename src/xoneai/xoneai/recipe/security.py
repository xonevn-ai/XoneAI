...
def generate_sbom(
    ...
    # Add xoneai as component
    try:
        import xoneai
        xoneai_version = getattr(xoneai, "__version__", "unknown")
    except ImportError:
        xoneai_version = "unknown"
    
    components.append({
        "type": "library",
        "name": "xoneai",
        "version": xoneai_version,
        "purl": f"pkg:pypi/xoneai@{xoneai_version}",
    })
...
def _generate_cyclonedx(
    ...
    return {
        ...
        "metadata": {
            ...
            "tools": [{"name": "xoneai-sbom", "version": "1.0.0"}],
            "component": {
                "type": "application",
                "name": name,
                "version": version,
...
def _generate_spdx(
    ...
    return {
        ...
        "name": f"{name}-{version}",
        "documentNamespace": f"https://ai.xone.vn/sbom/{name}/{version}",
        "creationInfo": {
            "created": _get_timestamp(),
            "creators": ["Tool: xoneai-sbom-1.0.0"],
        },
        "packages": [
            {
...
def sign_bundle(
    ...
    Args:
        bundle_path: Path to .xone bundle
    ...
    # Write signature
    output_path = Path(output_path) if output_path else bundle_path.with_suffix(".xone.sig")
...
def verify_bundle(
    ...
    signature_path = Path(signature_path) if signature_path else bundle_path.with_suffix(".xone.sig")

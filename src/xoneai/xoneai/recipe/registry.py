...
Supports:
- Local filesystem registry (~/.xone/registry)
...
DEFAULT_REGISTRY_PATH = Path.home() / ".xone" / "registry"
DEFAULT_RUNS_PATH = Path.home() / ".xone" / "runs"
...
class LocalRegistry:
    ...
    def publish(
        ...
        Args:
            bundle_path: Path to .xone bundle file
        ...
        # Copy bundle
        bundle_name = f"{name}-{version}.xone"
...
class HttpRegistry:
    ...
    def __init__(
        ...
        Args:
            ...
            token: Authentication token (or set XONEAI_REGISTRY_TOKEN env var)
            timeout: Request timeout in seconds
        """
        self.url = url.rstrip("/")
        self.token = token or os.environ.get("XONEAI_REGISTRY_TOKEN")
    ...
    def _upload_file(self, path: str, file_path: Path, force: bool = False) -> Dict[str, Any]:
        ...
        boundary = f"----XoneAI{uuid.uuid4().hex}"
    ...
    def pull(
        ...
        output_dir = Path(output_dir) if output_dir else Path.cwd()
        bundle_path = output_dir / f"{name}-{version}.xone"

...
def load_agent_yaml_with_schedule(yaml_path: str) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    # Get framework (default to xoneai)
    agent_config['framework'] = config.get('framework', 'xoneai')
...
def create_agent_from_config(agent_config: Dict[str, Any]) -> Any:
    """
    Create a XoneAI Agent instance from configuration.
    ...
    Raises:
        ImportError: If xoneaiagents is not installed
    """
    try:
        from xoneaiagents import Agent
    except ImportError:
        raise ImportError("xoneaiagents package is required. Install with: pip install xoneaiagents")

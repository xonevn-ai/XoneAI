...
class CloudDeployer:
    ...
    def create_dockerfile(self):
        ...
        install the required Python packages (flask, xoneai, gunicorn, and markdown),
        ...
        with open("Dockerfile", "w") as file:
            ...
            file.write("COPY . .\n")
            file.write("RUN pip install flask xoneai==4.5.15 gunicorn markdown\n")
    ...
    def create_api_file(self):
        ...
        This method creates an API file named "api.py" in the current directory. The file contains a basic Flask application that uses the XoneAI library to run a simple agent and returns the output as an HTML page. The application listens on the root path ("/") and uses the Markdown library to format the output.
        """
        with open("api.py", "w") as file:
            file.write("from flask import Flask\n")
            file.write("from xoneai import XoneAI\n")
            file.write("import markdown\n\n")
            file.write("app = Flask(__name__)\n\n")
            file.write("def basic():\n")
            file.write("    xoneai = XoneAI(agent_file=\"agents.yaml\")\n")
            file.write("    return xoneai.run()\n\n")
    ...
    def run_commands(self):
        ...
        commands = [
            ...
            ['gcloud', 'artifacts', 'repositories', 'create', 'xoneai-repository', 
             '--repository-format=docker', '--location=us-central1'],
            ['docker', 'build', '--platform', 'linux/amd64', '-t', 
             f'gcr.io/{project_id}/xoneai-app:latest', '.'],
            ['docker', 'tag', f'gcr.io/{project_id}/xoneai-app:latest',
             f'us-central1-docker.pkg.dev/{project_id}/xoneai-repository/xoneai-app:latest'],
            ['docker', 'push', 
             f'us-central1-docker.pkg.dev/{project_id}/xoneai-repository/xoneai-app:latest'],
            ['gcloud', 'run', 'deploy', 'xoneai-service', 
             '--image', f'us-central1-docker.pkg.dev/{project_id}/xoneai-repository/xoneai-app:latest',

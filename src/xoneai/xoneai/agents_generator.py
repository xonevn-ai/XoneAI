...
# Only try to import xoneai_tools if either CrewAI or AutoGen is available
if CREWAI_AVAILABLE or AUTOGEN_AVAILABLE or XONEAI_AVAILABLE:
    try:
...
class AgentsGenerator:
    def __init__(self, agent_file, framework, config_list, log_level=None, agent_callback=None, task_callback=None, agent_yaml=None, tools=None):
        ...
        if framework == "crewai" and not CREWAI_AVAILABLE:
            raise ImportError("CrewAI is not installed. Please install it with 'pip install xoneai[crewai]'")
        elif framework == "autogen" and not (AUTOGEN_AVAILABLE or AUTOGEN_V4_AVAILABLE):
            raise ImportError("AutoGen is not installed. Please install it with 'pip install xoneai[autogen]' for v0.2 or 'pip install xoneai[autogen-v4]' for v0.4")
        elif framework == "xoneai" and not XONEAI_AVAILABLE:
            raise ImportError("XoneAI is not installed. Please install it with 'pip install xoneaiagents'")
        ...
        try:
            ...
            return {name: obj() for name, obj in inspect.getmembers(module, 
                lambda x: inspect.isclass(x) and (
                    ...
                    (XONEAI_TOOLS_AVAILABLE and issubclass(x, BaseTool))
    ...
    def generate_crew_and_kickoff(self):
        ...
        # Only try to use xoneai_tools if it's available and needed
        if XONEAI_TOOLS_AVAILABLE and (CREWAI_AVAILABLE or AUTOGEN_AVAILABLE or XONEAI_AVAILABLE):
            tools_dict = {
        ...
        elif framework == "xoneai":
            if not XONEAI_AVAILABLE:
                raise ImportError("XoneAI is not installed. Please install it with 'pip install xoneaiagents'")
            if AGENTOPS_AVAILABLE:
                agentops.init(os.environ.get("AGENTOPS_API_KEY"), default_tags=["xoneai"])
            return self._run_xoneai(config, topic, tools_dict)
    ...
    def _run_yaml_workflow(self, config):
        ...
        try:
            ...
            raise ImportError("YAMLWorkflowParser not available. Please update xoneaiagents.")
    ...
    def _run_crewai(self, config, topic, tools_dict):
        ...
        for role, details in config['roles'].items():
            ...
            if function_calling_llm_model:
                function_calling_llm = XoneAIModel(
    ...
    def _run_xoneai(self, config, topic, tools_dict):
        """
        Run agents using the XoneAI framework.
        
        Tool resolution order:
        1. Local tools.py (backward compat, custom tools)
        2. YAML tools: field resolved via ToolResolver
        3. Built-in tools from xoneaiagents.tools
        """
        agents = {}
        tasks = []
        tasks_dict = {}
        ...
        from xoneai.tool_resolver import ToolResolver
        ...
        # Create and run the XoneAI agents

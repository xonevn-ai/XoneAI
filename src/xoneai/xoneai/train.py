...
class TrainModel:
    ...
    def train_model(self):
        ...
        ta_params = {
            ...
            "report_to": "none" if not os.getenv("XONE_WANDB") else "wandb",
            "remove_unused_columns": self.config.get("remove_unused_columns", False)
        }
        if os.getenv("XONE_WANDB"):
            ta_params["save_steps"] = self.config.get("save_steps", 100)
            ta_params["run_name"] = os.getenv("XONE_WANDB_RUN_NAME", "xoneai-train")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="XoneAI Training Script")

...
Run with: python -m pytest xoneai/persistence/tests/test_all_backends.py -v
"""
...
sys.path.insert(0, '/Users/xonevn/xoneai-package/src/xoneai')

from xoneai.persistence.conversation.base import ConversationStore, ConversationSession, ConversationMessage
from xoneai.persistence.knowledge.base import KnowledgeStore, KnowledgeDocument
from xoneai.persistence.state.base import StateStore
from xoneai.persistence.orchestrator import PersistenceOrchestrator
from xoneai.persistence.factory import create_conversation_store, create_knowledge_store, create_state_store
...
def test_postgresql_conversation_store():
    ...
    try:
        from xoneai.persistence.conversation.postgres import PostgresConversationStore
        store = PostgresConversationStore(
            host='localhost', port=5432, database='xoneai',
            user='postgres', password='xonevn123'
        )
    except Exception as e:
        print(f"  ⚠️ SKIPPED: {e}")
        return
...
def test_sqlite_conversation_store():
    ...
    try:
        from xoneai.persistence.conversation.sqlite import SQLiteConversationStore
        store = SQLiteConversationStore(path="/tmp/test_xone.db")
    except Exception as e:
        print(f"  ⚠️ SKIPPED: {e}")
        return
...
def test_qdrant_knowledge_store():
    ...
    try:
        from xoneai.persistence.knowledge.qdrant import QdrantKnowledgeStore
...
def test_chroma_knowledge_store():
    ...
    try:
        from xoneai.persistence.knowledge.chroma import ChromaKnowledgeStore
...
def test_redis_state_store():
    ...
    try:
        from xoneai.persistence.state.redis import RedisStateStore
...
def test_memory_state_store():
    ...
    try:
        from xoneai.persistence.state.memory import MemoryStateStore
...
def run_all_tests():
    ...
    print("=" * 60)
    print("XoneAI Persistence Layer - Comprehensive TDD Tests")
    print("=" * 60)

...
class ClickHouseKnowledgeStore(KnowledgeStore):
    ...
    def _table_name(self, collection: str) -> str:
        return f"{self.database}.xone_vec_{collection}"
    ...
    def list_collections(self) -> List[str]:
        ...
        result = self._client.query(f"""
            ...
            WHERE database = '{self.database}' AND name LIKE 'xone_vec_%'
        """)
        return [row[0].replace("xone_vec_", "") for row in result.result_rows]

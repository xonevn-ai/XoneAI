...
class PGVectorKnowledgeStore(KnowledgeStore):
    ...
    Example:
        store = PGVectorKnowledgeStore(
            url="postgresql://localhost:5432/xoneai"
        )
    """
    ...
    def _table_name(self, collection: str) -> str:
        return f"{self.schema}.xone_vec_{collection}"
        ...
        try:
            with conn.cursor() as cur:
                cur.execute("""
                    ...
                    WHERE table_schema = %s AND table_name LIKE 'xone_vec_%%'
                """, (self.schema,))
                return [row[0].replace("xone_vec_", "") for row in cur.fetchall()]
        finally:
            self._put_conn(conn)

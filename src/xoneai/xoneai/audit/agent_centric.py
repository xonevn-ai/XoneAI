...
AGENT_IMPORTS = [
    r"from\s+xoneaiagents\s+import\s+.*\bAgent\b",
    r"from\s+xoneaiagents\s+import\s+.*\bXoneAIAgents\b",
    r"from\s+xoneaiagents\s+import\s+.*\bWorkflow\b",
    r"from\s+xoneaiagents\.agent\s+import\s+Agent",
    r"from\s+xoneaiagents\.agents\s+import\s+XoneAIAgents",
    r"from\s+xoneaiagents\.workflow\s+import\s+Workflow",
]
...
LOW_LEVEL_IMPORTS = [
    r"from\s+xoneaiagents\.(memory|knowledge|tools|context)",
    r"from\s+xoneaiagents\.mcp",
    r"from\s+xoneaiagents\.hooks",
    r"from\s+xoneaiagents\.middleware",
]
...
class AgentCentricAuditor:
    ...
    def _method_2_ast_import_detector(self, content: str) -> Tuple[bool, str]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom):
                if node.module and "xoneaiagents" in node.module:
                    for alias in node.names:
                        if alias.name in ("Agent", "XoneAIAgents", "Workflow"):
    ...
    def generate_quickstart_block(self, feature_name: str = "Demo") -> str:
        ...
        return f'''from xoneaiagents import Agent

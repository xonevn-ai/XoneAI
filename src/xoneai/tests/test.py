...
class TestXoneAIFramework(unittest.TestCase):
    @pytest.mark.real
    def test_main_with_agents_advanced(self):
        xoneai = XoneAI(agent_file="tests/agents-advanced.yaml")
        result = xoneai.run()
    ...
    def test_main_with_autogen_framework(self):
        xoneai = XoneAI(agent_file="tests/autogen-agents.yaml")
        result = xoneai.run()
        print(f"Result: {result}")
        self.assertIsNotNone(result)
    ...
    def test_main_with_custom_framework(self):
        xoneai = XoneAI(agent_file="tests/crewai-agents.yaml")
        result = xoneai.run()
        print(f"Result: {result}")
        self.assertIsNotNone(result)
    ...
    def test_main_with_internet_search_tool(self):
        xoneai = XoneAI(agent_file="tests/search-tool-agents.yaml")
        result = xoneai.run()
        print(f"Result: {result}")
        self.assertIsNotNone(result)
    ...
    def test_main_with_built_in_tool(self):
        xoneai = XoneAI(agent_file="tests/inbuilt-tool-agents.yaml")
        result = xoneai.run()
        print(f"Result: {result}")
        self.assertIsNotNone(result)
    

class TestXoneAICommandLine(unittest.TestCase):
    ...
    @pytest.mark.real
    def test_xoneai_command(self):
        # Test basic xoneai command
        command = "xoneai --framework autogen --auto \"create a 2-agent team to write a simple python game\""
        result = self.run_command(command)
        print(f"Result: {result}")
        self.assertIn('TERMINATE', result)

    @pytest.mark.real
    def test_xoneai_init_command(self):
        # Test xoneai --init command
        # This command primarily creates files, but let's ensure it uses the real key if any underlying XoneAI init involves API calls
        command = "xoneai --framework autogen --init \"create a 2-agent team to write a simple python game\""
        result = self.run_command(command)
        print(f"Result: {result}")

#!/usr/bin/env python3
"""
Live Smoke Tests for XoneAI Recipe System
...
"""
    ...
    def test_cli_list_command(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "xoneai.cli.main", "recipe", "list", "--json"],
            capture_output=True,
            text=True,
            timeout=30,
        )
    ...
    def test_cli_help_command(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "xoneai.cli.main", "recipe", "help"],
            capture_output=True,
            text=True,
            timeout=10,
        )
        ...
        assert result.returncode == 0
        assert "xoneai recipe" in result.stdout.lower() or "xoneai recipe" in result.stdout
...
"""
        ...
        # Create bundle
        bundle_path = tmp_path / "test-registry-recipe-1.0.0.xone"
...
class TestLiveSecurity:
    ...
    def test_pii_redaction(self, tmp_path):
        """Test PII redaction."""
        from xoneai.recipe.security import redact_pii, detect_pii
...
class TestLivePolicy:
    ...
    def test_policy_workflow(self, tmp_path):
        ...
        from xoneai.recipe.policy import PolicyPack, get_default_policy, PolicyDeniedError

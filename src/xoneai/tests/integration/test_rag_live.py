...
Run with: XONEAI_LIVE_TESTS=1 pytest tests/integration/test_rag_live.py -v

Required environment variables:
- XONEAI_LIVE_TESTS=1 (to enable these tests)
...
# Skip all tests in this module if XONEAI_LIVE_TESTS is not set
pytestmark = pytest.mark.skipif(
    os.environ.get("XONEAI_LIVE_TESTS") != "1",
    reason="Live tests disabled. Set XONEAI_LIVE_TESTS=1 to enable."
)
...
def sample_document(temp_knowledge_dir):
    ...
    doc_path.write_text("""
    XoneAI is an advanced AI framework for building intelligent agents.
...
class TestRAGQueryLive:
    ...
    def test_rag_query_basic(self, temp_knowledge_dir, sample_document):
        ...
        # Query
        result = rag.query("What is XoneAI?")
        
        # Verify
        assert result.answer is not None
        assert len(result.answer) > 0
        assert "XoneAI" in result.answer or "framework" in result.answer.lower()
    ...
    def test_rag_query_with_citations(self, temp_knowledge_dir, sample_document):
        ...
        result = rag.query("What features does XoneAI have?")
...
class TestRAGStreamingLive:
    ...
    def test_rag_streaming(self, temp_knowledge_dir, sample_document):
        ...
        # Stream response
        chunks = list(rag.stream("What is XoneAI?"))
...
class TestAgentRAGLive:
    ...
    def test_agent_rag_query(self, temp_knowledge_dir, sample_document):
        ...
        # Query via agent
        result = agent.rag_query("What features does XoneAI support?")
...
class TestCLILive:
    ...
    def test_cli_rag_module_importable(self):
        ...
        from xoneai.cli.commands.rag import rag_query, rag_chat, rag_serve
    ...
    def test_cli_knowledge_module_importable(self):
        ...
        # Direct import test - no subprocess needed
        from xoneai.cli.commands.knowledge import knowledge_index, knowledge_search

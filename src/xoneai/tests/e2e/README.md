...
This directory contains **real end-to-end tests** that make actual API calls to test XoneAI framework integrations. These are fundamentally different from the mock tests in `tests/integration/`.
...
**Run with full execution (actual xoneai.run()):**
```bash
# Enable full execution tests
export XONEAI_RUN_FULL_TESTS=true
...
**Full execution tests (with xoneai.run()):**
```bash
...
- ✅ **Framework Integration**: XoneAI + AutoGen/CrewAI

"""
Network Guard Plugin for XoneAI Tests
...
2. XONEAI_ALLOW_NETWORK=1 or XONEAI_LIVE_TESTS=1 is set
...
def _create_blocking_connect(original_connect):
    ...
    def blocking_connect(self, address):
        ...
        if os.environ.get('XONEAI_ALLOW_NETWORK') == '1':
            return original_connect(self, address)
        if os.environ.get('XONEAI_LIVE_TESTS') == '1':
            return original_connect(self, address)
        
        # Block the connection
        raise NetworkBlockedError(
            f"Network access blocked: attempted connection to {address}. "
            "Set XONEAI_ALLOW_NETWORK=1 or mark test with @pytest.mark.network"
        )
...
def network_guard_fixture(request):
    ...
    - XONEAI_ALLOW_NETWORK=1 or XONEAI_LIVE_TESTS=1
    """
...
def pytest_configure(config):
    ...
    if os.environ.get('XONEAI_ALLOW_NETWORK') != '1' and os.environ.get('XONEAI_LIVE_TESTS') != '1':
        install_network_guard()

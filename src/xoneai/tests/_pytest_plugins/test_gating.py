"""
XoneAI Test Gating Plugin

This module provides automatic marker assignment and skip/gating enforcement
for the XoneAI test suite. It ensures tests are properly classified and
gated based on provider requirements and network access.

Environment Variables:
- XONEAI_TEST_TIER: smoke|main|extended|nightly (default: main)
- XONEAI_ALLOW_NETWORK: 0|1 (default: 0)
- XONEAI_LIVE_TESTS: 0|1 (default: 0)
- XONEAI_TEST_PROVIDERS: comma-separated list or 'all' (default: openai)
- XONEAI_LOCAL_SERVICES: 0|1 (default: 0)
"""
...
def _get_test_tier() -> str:
    ...
    return os.environ.get('XONEAI_TEST_TIER', 'main').lower()


def _is_network_allowed() -> bool:
    """Check if network access is allowed."""
    return (
        os.environ.get('XONEAI_ALLOW_NETWORK', '0') == '1' or
        os.environ.get('XONEAI_LIVE_TESTS', '0') == '1'
    )


def _get_allowed_providers() -> Set[str]:
    """Get the set of allowed providers."""
    providers_str = os.environ.get('XONEAI_TEST_PROVIDERS', 'openai')
...
def _is_local_services_allowed() -> bool:
    ...
    return os.environ.get('XONEAI_LOCAL_SERVICES', '0') == '1'
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        if 'network' in existing_markers and not network_allowed:
            item.add_marker(pytest.mark.skip(
                reason="Network tests disabled. Set XONEAI_ALLOW_NETWORK=1 or XONEAI_LIVE_TESTS=1"
            ))
            continue
        ...
        for provider in provider_markers:
            if provider not in allowed_providers:
                item.add_marker(pytest.mark.skip(
                    reason=f"Provider {provider} not in XONEAI_TEST_PROVIDERS"
                ))
                break
        ...
        if 'local_service' in existing_markers and not local_services_allowed:
            item.add_marker(pytest.mark.skip(
                reason="Local service tests disabled. Set XONEAI_LOCAL_SERVICES=1"
            ))
            continue

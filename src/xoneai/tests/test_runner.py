#!/usr/bin/env python3
"""
Comprehensive test runner for XoneAI Agents
...
def run_test_suite():
    ...
    print("🧪 Starting XoneAI Agents Test Suite")
    print("=" * 50)
...
def run_tests(pattern=None, verbose=False, coverage=False):
    ...
    if coverage:
        cmd.extend(["--cov=xoneaiagents", "--cov-report=term-missing"])
    ...
    if is_full_test:
        # Add -s flag to see real-time output from xoneai.run()
        cmd.append("-s")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test runner for XoneAI")
    parser.add_argument(
        "--pattern", 
        choices=[
    ...
    if "full" in args.pattern:
        ...
        print("💰💰 These tests run xoneai.run() with ACTUAL API calls!")

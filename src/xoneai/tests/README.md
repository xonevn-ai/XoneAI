# XoneAI Agents - Comprehensive Testing Suite

This directory contains a comprehensive testing suite for XoneAI Agents, organized into different categories to ensure thorough coverage of all functionality.
...
**Issue**: `AttributeError: <module 'xoneaiagents.llm.llm'> does not have the attribute 'litellm'`

**Cause**: Some tests attempt to mock `xoneaiagents.llm.llm.litellm` but this attribute path may not exist in the current codebase structure.
...
# Run with coverage
pytest tests/ --cov=xoneaiagents --cov-report=html
...
3. **Basic Diagnostic Script** (`tests/test_basic.py`):
   - ✅ Tests basic Python imports and xoneaiagents functionality

...
from xoneai.cli.features.doctor.models import (
    CheckCategory,
    CheckStatus,
    DoctorConfig,
)
from xoneai.cli.features.doctor.registry import CheckRegistry
...
class TestEnvChecks:
    ...
    def setup_method(self):
        ...
        from xoneai.cli.features.doctor.checks import env_checks
    
    def test_python_version_check(self):
        """Test Python version check."""
        from xoneai.cli.features.doctor.checks.env_checks import check_python_version
    ...
    def test_openai_api_key_check_present(self):
        ...
        from xoneai.cli.features.doctor.checks.env_checks import check_openai_api_key
    ...
    def test_os_info_check(self):
        """Test OS info check."""
        from xoneai.cli.features.doctor.checks.env_checks import check_os_info
    ...
    def test_virtual_env_check(self):
        ...
        from xoneai.cli.features.doctor.checks.env_checks import check_virtual_env
...
class TestConfigChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import config_checks
    
    def test_agents_yaml_not_found(self):
        """Test agents.yaml check when file doesn't exist."""
        from xoneai.cli.features.doctor.checks.config_checks import check_agents_yaml_exists
    ...
    def test_xone_config_dir_check(self):
        """Test .xone config directory check."""
        from xoneai.cli.features.doctor.checks.config_checks import check_xone_config_dir
        
        config = DoctorConfig()
        result = check_xone_config_dir(config)
...
class TestPermissionsChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import permissions_checks
    
    def test_temp_dir_check(self):
        """Test temp directory permissions check."""
        from xoneai.cli.features.doctor.checks.permissions_checks import check_permissions_temp_dir
    ...
    def test_cwd_check(self):
        ...
        from xoneai.cli.features.doctor.checks.permissions_checks import check_permissions_cwd
...
class TestNetworkChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import network_checks
    
    def test_proxy_check_no_proxy(self):
        """Test proxy check when no proxy is configured."""
        from xoneai.cli.features.doctor.checks.network_checks import check_network_proxy
    ...
    def test_ssl_check_default(self):
        ...
        from xoneai.cli.features.doctor.checks.network_checks import check_network_ssl
    ...
    def test_openai_base_url_check(self):
        ...
        from xoneai.cli.features.doctor.checks.network_checks import check_network_openai_base_url
...
class TestPerformanceChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import performance_checks
    
    def test_loaded_modules_check(self):
        """Test loaded modules count check."""
        from xoneai.cli.features.doctor.checks.performance_checks import check_performance_loaded_modules
...
class TestDbChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import db_checks
    
    def test_db_config_no_dsn(self):
        """Test database config check when no DSN is set."""
        from xoneai.cli.features.doctor.checks.db_checks import check_db_config
        
        env = {k: v for k, v in os.environ.items() 
               if k not in ["DATABASE_URL", "XONEAI_DATABASE_URL"]}
    ...
    def test_sqlite_driver_check(self):
        ...
        from xoneai.cli.features.doctor.checks.db_checks import check_db_driver_sqlite
...
class TestMcpChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import mcp_checks
    
    def test_mcp_config_not_found(self):
        """Test MCP config check when no config exists."""
        from xoneai.cli.features.doctor.checks.mcp_checks import check_mcp_config
...
class TestSkillsChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import skills_checks
    
    def test_skills_dirs_check(self):
        """Test skills directories check."""
        from xoneai.cli.features.doctor.checks.skills_checks import check_skills_dirs
...
class TestMemoryChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import memory_checks
    
    def test_memory_dirs_check(self):
        """Test memory directories check."""
        from xoneai.cli.features.doctor.checks.memory_checks import check_memory_dirs
...
class TestObsChecks:
    ...
    def setup_method(self):
        """Reset registry before each test."""
        CheckRegistry.reset()
        from xoneai.cli.features.doctor.checks import obs_checks
    
    def test_obs_config_check(self):
        """Test observability config check."""
        from xoneai.cli.features.doctor.checks.obs_checks import check_obs_config
...
class TestStalePackagesCheck:
    ...
    def test_stale_packages_check_passes_when_clean(self):
        ...
        from xoneai.cli.features.doctor.checks.env_checks import check_stale_packages
    ...
    def test_stale_packages_check_detects_namespace_package(self):
        """Test stale packages check detects when xoneaiagents is namespace pkg."""
        from xoneai.cli.features.doctor.checks.env_checks import check_stale_packages
        
        # Mock xoneaiagents with __file__ = None (namespace package)
        with patch('xoneai.cli.features.doctor.checks.env_checks.check_stale_packages') as mock_check:
            # This test verifies the check function exists and is callable
            # The actual namespace detection is tested in xoneaiagents tests
            config = DoctorConfig()
            result = check_stale_packages(config)
    ...
    def test_xoneaiagents_file_not_none(self):
        """Test that xoneaiagents.__file__ is not None (CI check)."""
        import xoneaiagents
        
        # This is the critical CI check - __file__ must not be None
        assert xoneaiagents.__file__ is not None, (
            "xoneaiagents.__file__ is None - this indicates a namespace package "
            "shadowing issue. Remove stale xoneaiagents directory from site-packages."
        )
    
    def test_xoneaiagents_spec_has_origin(self):
        """Test that xoneaiagents.__spec__.origin is not None (CI check)."""
        import xoneaiagents
        
        # Namespace packages have origin=None
        assert xoneaiagents.__spec__ is not None
        assert xoneaiagents.__spec__.origin is not None, (
            "xoneaiagents.__spec__.origin is None - namespace package detected"
        )

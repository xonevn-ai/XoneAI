"""
Performance benchmark tests for XoneAI.
...
# Get the paths for xoneai and xoneaiagents
XONEAI_PATH = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
XONEAI_AGENTS_PATH = os.path.join(os.path.dirname(XONEAI_PATH), "xoneai-agents")

# Build PYTHONPATH that includes both packages
PYTHONPATH = f"{XONEAI_PATH}:{XONEAI_AGENTS_PATH}"

# Thresholds (can be adjusted based on CI environment)
XONEAI_WRAPPER_IMPORT_TIME_MS = 500  # Should be very fast (lazy)
...
class TestImportTimeBenchmarks:
    ...
    def test_xoneai_wrapper_import_time(self):
        """Test that xoneai wrapper imports quickly (lazy loading)."""
        code = '''
import time
t = time.time()
import xoneai
...
'''
        ...
        assert elapsed_ms < XONEAI_WRAPPER_IMPORT_TIME_MS, \
            f"xoneai import took {elapsed_ms:.0f}ms, expected < {XONEAI_WRAPPER_IMPORT_TIME_MS}ms"
...
heavy_modules = ['xoneai.auto', 'xoneai.agents_generator', 'litellm', 'instructor']
...
'''
    ...
    def test_xoneaiagents_not_imported_at_cli_startup(self):
        """Verify xoneaiagents is not imported when CLI starts."""
        code = '''
import sys
import os
os.environ['LOGLEVEL'] = 'WARNING'
from xoneai.cli.main import XoneAI
# Check if xoneaiagents is NOT loaded
if 'xoneaiagents' in sys.modules:
    print("FAIL: xoneaiagents loaded")
else:
    print("OK")
'''
        result = run_python_code(code)
        assert result.returncode == 0, f"Test failed: {result.stderr}"
        output = result.stdout.strip()
        assert output == "OK", f"xoneaiagents loaded at startup: {output}"
...
import threading
import time
from xoneaiagents.tools.registry import ToolRegistry
...
class TestSchemaVersioning:
    ...
    def test_schema_version_constant_exists(self):
        ...
        code = '''
from xoneaiagents.db.protocol import SCHEMA_VERSION

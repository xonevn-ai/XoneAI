...
# Add the xoneai-agents module to path
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..', 'xoneai-agents')))
...
def test_agent_tool_execution_with_approval():
    ...
    try:
        ...
        from xoneaiagents.approval import set_approval_callback, console_approval_callback, ApprovalDecision
...
@patch('xoneaiagents.approval.console_approval_callback')
def test_agent_with_auto_approval(mock_console_callback, mock_confirm):
    ...
    try:
        ...
        try:
            from xoneaiagents.approval import set_approval_callback, ApprovalDecision, clear_approval_context, mark_approved
        except ImportError:
            assert False, "xoneaiagents.approval module not available - check import path"
...
def test_agent_python_code_execution(mock_console_callback, mock_confirm):
    ...
    try:
        ...
        from xoneaiagents import Agent
        from xoneaiagents.tools import execute_code
...
def test_agent_file_operations(mock_console_callback, mock_confirm):
    ...
    try:
        ...
        from xoneaiagents import Agent
        from xoneaiagents.tools import write_file
        import tempfile
        import os
...
def main():
    ...
    print("🧪 XoneAI Human Approval System - Agent Integration Tests")
    print("=" * 65)

"""
TDD tests for xoneai.security.protected module.
...
class TestIsProtected:
    def _check(self, path, extra=None):
        from xoneai.security.protected import is_protected
    ...
    def test_xoneaiagents_package_protected(self):
        assert self._check("xoneaiagents/agent/agent.py") is True
...
class TestProtectedList:
    def test_protected_paths_is_frozenset(self):
        from xoneai.security.protected import PROTECTED_PATHS
        assert isinstance(PROTECTED_PATHS, frozenset)

    def test_protected_patterns_nonempty(self):
        from xoneai.security.protected import PROTECTED_PATTERNS
        assert len(PROTECTED_PATTERNS) > 0

    def test_protected_reason(self):
        from xoneai.security.protected import get_protection_reason

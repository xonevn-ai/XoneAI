...
class TestPatternDetection:
    ...
    def test_detect_agent_class_pattern_with_role_goal_backstory(self):
        ...
        from xoneai.cli.features.migrate import PatternDetector
...
class TestCodeConversion:
    """Test code conversion to XoneAI format."""
    
    def test_convert_agent_pattern_to_xoneai(self):
        """Convert detected agent pattern to XoneAI Agent."""
        from xoneai.cli.features.migrate import CodeConverter
...
'''
        ...
        assert "from xoneaiagents import Agent" in result.converted_code
...
class TestYAMLMigration:
    ...
    def test_yaml_migration_still_works(self):
        ...
        from xoneai.cli.features.migrate import ConfigMigrator
...
class TestCLIIntegration:
    ...
    def test_migrate_help_is_generic(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "xoneai", "migrate", "--help"],
            capture_output=True,
            text=True,
            timeout=30
        )
        ...
        # Skip if run failed due to missing xoneaiagents (integration env issue, not test issue)
        if "xoneaiagents" in help_text.lower() and "not installed" in help_text.lower():
            import pytest
            pytest.skip("xoneaiagents not installed - cannot test migrate CLI")
    ...
    def test_migrate_dry_run_flag(self):
        ...
        from xoneai.cli.features.migrate import handle_migrate_command
        import argparse

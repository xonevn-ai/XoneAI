...
class TestRecipeCreatorFolderName:
    ...
    def test_generate_folder_name_kebab_case(self):
        ...
        from xoneai.cli.features.recipe_creator import RecipeCreator
...
class TestRecipeCreatorAgentsYaml:
    ...
    @patch('xoneai.cli.features.recipe_creator.RecipeCreator._get_litellm')
    def test_generate_agents_yaml_calls_llm(self, mock_get_litellm):
        ...
        mock_response.choices[0].message.content = """framework: xoneai
topic: "Web Scraper"
agents:
  scraper:
    role: Web Scraper
...
"""
        ...
        assert "framework: xoneai" in yaml_content or "agents:" in yaml_content
...
class TestCmdCreate:
    ...
    @patch('xoneai.cli.features.recipe_creator.RecipeCreator')
...
class TestCmdOptimize:
    ...
    @patch('xoneai.cli.features.recipe_optimizer.RecipeOptimizer')
    def test_cmd_optimize_parses_args(self, mock_optimizer_class):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            recipe_path.mkdir()
            (recipe_path / "agents.yaml").write_text("framework: xoneai")

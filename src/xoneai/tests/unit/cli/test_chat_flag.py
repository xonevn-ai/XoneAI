...
class TestChatFlagIntegration:
    """Integration tests with actual XoneAI CLI parser."""
    
    def test_xoneai_parser_has_chat_flag(self):
        """XoneAI CLI should have --chat as alias for --chat-mode.
        
        Note: --chat-mode and --chat flags were deprecated in favor of
        'xoneai chat' subcommand. This test verifies the parser doesn't
    ...
    def test_xoneai_chat_subcommand_still_works(self):
        """'xoneai chat' subcommand should still launch Chainlit.
        ...
        """
        # Test the parser structure directly without going through XoneAI
        # which has test environment detection
        parser = argparse.ArgumentParser()
        parser.add_argument("--chat-mode", "--chat", action="store_true", dest="chat_mode")
        parser.add_argument("command", nargs="?")
        
        # Simulate 'xoneai chat' - should set command='chat', not chat_mode
        args, _ = parser.parse_known_args(["chat"])
        assert args.command == "chat"
        assert args.chat_mode is False  # subcommand, not flag

"""
Unit tests for the xoneai test CLI command.
...
from xoneai.cli.commands.test import (
    _get_pytest_args,
    _set_environment,
)
...
class TestSetEnvironment:
    ...
    def test_sets_tier(self):
        """Should set XONEAI_TEST_TIER."""
        _set_environment("smoke", None, False)
        assert os.environ.get("XONEAI_TEST_TIER") == "smoke"
    
    def test_live_enables_network(self):
        """Live mode should enable network and live tests."""
        _set_environment("main", None, True)
        assert os.environ.get("XONEAI_ALLOW_NETWORK") == "1"
        assert os.environ.get("XONEAI_LIVE_TESTS") == "1"
    
    def test_no_live_disables_network(self):
        """Non-live mode should disable network."""
        _set_environment("main", None, False)
        assert os.environ.get("XONEAI_ALLOW_NETWORK") == "0"
        assert os.environ.get("XONEAI_LIVE_TESTS") == "0"
    
    def test_sets_provider(self):
        """Should set XONEAI_TEST_PROVIDERS when provider specified."""
        _set_environment("main", "anthropic", False)
        assert os.environ.get("XONEAI_TEST_PROVIDERS") == "anthropic"

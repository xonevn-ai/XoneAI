...
class TestFenceExtractor:
    ...
    def test_extract_simple_python_fence(self, tmp_path):
        ...
        from xoneai.docs_runner.extractor import FenceExtractor
...
""")
    ...
    def test_extract_with_directive(self, tmp_path):
        """Test extracting fence with xoneai directive comment."""
        from xoneai.docs_runner.extractor import FenceExtractor
        
        doc = tmp_path / "test.mdx"
        doc.write_text("""# Title

<!-- xoneai: runnable=true timeout=120 require_env=OPENAI_API_KEY -->
```python
print("hello")
```
""")
...
""")
        ...
        assert len(blocks) == 1
        assert "from xoneaiagents import Agent" in blocks[0].code
...
class TestRunnableClassifier:
    ...
    def test_classify_standalone_script(self):
        ...
        from xoneai.docs_runner.classifier import RunnableClassifier
        from xoneai.docs_runner.extractor import CodeBlock
        
        block = CodeBlock(
            doc_path=Path("test.mdx"),
            language="python",
            code="""from xoneaiagents import Agent
...
class TestWorkspaceWriter:
    ...
    def test_write_single_script(self, tmp_path):
        ...
        from xoneai.docs_runner.workspace import WorkspaceWriter
        from xoneai.docs_runner.extractor import CodeBlock
...
class TestSnippetRunner:
    ...
    def test_run_passing_script(self, tmp_path):
        ...
        from xoneai.docs_runner.runner import SnippetRunner
...
class TestDocsReportBuilder:
    ...
    def test_generate_json_report(self, tmp_path):
        ...
        from xoneai.docs_runner.reporter import DocsReportBuilder, SnippetResult
...
class TestDocsExecutor:
    ...
    def test_discover_docs(self, tmp_path):
        ...
        from xoneai.docs_runner.executor import DocsExecutor
...
class TestCLIIntegration:
    ...
    def test_docs_list_command(self, tmp_path):
        ...
        from xoneai.cli.commands.docs import app
    ...
    def test_docs_run_dry_run(self, tmp_path):
        """Test 'xoneai docs run --dry-run' command."""
        from typer.testing import CliRunner

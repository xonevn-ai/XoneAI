...
class TestPythonFeatureExtractor:
    ...
    def test_extract_lazy_imports(self):
        ...
        from xoneai._dev.parity.python_extractor import PythonFeatureExtractor
        
        # Create a mock __init__.py with _LAZY_IMPORTS
        mock_init = '''
_LAZY_IMPORTS = {
    'Agent': ('xoneaiagents.agent.agent', 'Agent'),
    'Tool': ('xoneaiagents.tools.base', 'Tool'),
}
'''
        with tempfile.TemporaryDirectory() as tmpdir:
            pkg_dir = Path(tmpdir) / "src" / "xoneai-agents" / "xoneaiagents"
            pkg_dir.mkdir(parents=True)
            init_file = pkg_dir / "__init__.py"
            init_file.write_text(mock_init)
            
            # Also create wrapper structure
            wrapper_dir = Path(tmpdir) / "src" / "xoneai" / "xoneai" / "cli" / "features"
            ...
            assert lazy_imports['Agent'] == ('xoneaiagents.agent.agent', 'Agent')
...
'''
    ...
    def test_categorize_module_path(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert extractor._categorize('xoneaiagents.agent.agent') == 'agent'
            assert extractor._categorize('xoneaiagents.tools.base') == 'tools'
            assert extractor._categorize('xoneaiagents.memory.memory') == 'memory'
            assert extractor._categorize('xoneaiagents.workflows') == 'workflows'
            assert extractor._categorize('xoneaiagents.unknown') == 'other'
    ...
    def test_extract_cli_features(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            features_dir = Path(tmpdir) / "src" / "xoneai" / "xoneai" / "cli" / "features"
...
class TestTypeScriptFeatureExtractor:
    ...
    def test_extract_exports(self):
        ...
        from xoneai._dev.parity.typescript_extractor import TypeScriptFeatureExtractor
...
'''
        with tempfile.TemporaryDirectory() as tmpdir:
            ts_dir = Path(tmpdir) / "src" / "xoneai-ts" / "src"
...
class TestParityTrackerGenerator:
    ...
    def test_generate_structure(self):
        ...
        from xoneai._dev.parity.generator import ParityTrackerGenerator
...
''')
    ...
    def test_write_rust_creates_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Create rust dir
            rust_dir = Path(tmpdir) / "src" / "xoneai-rust"
...
class TestGenerateParityTracker:
    ...
    def test_stdout_mode(self, capsys):
        ...
        from xoneai._dev.parity.generator import generate_parity_tracker
...
class TestRealExtraction:
    """Integration tests using the real XoneAI repository."""
    
    @pytest.mark.skipif(
        not Path("/Users/xonevn/xoneai-package").exists(),

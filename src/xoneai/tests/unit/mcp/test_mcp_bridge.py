...
Tests the bridge between xoneaiagents.tools and xoneai.mcp_server registry.
"""

from unittest.mock import patch, MagicMock
from xoneai.mcp_server.adapters.tools_bridge import (
    is_bridge_available,
    _infer_tool_hints,
    _extract_category,
...
class TestBridgeAvailability:
    ...
    def test_is_bridge_available_when_installed(self):
        """Test detection when xoneaiagents is installed."""
    ...
    def test_is_bridge_available_when_not_installed(self):
        """Test detection when xoneaiagents is not installed."""
        with patch("importlib.util.find_spec") as mock_find_spec:
...
class TestCategoryExtraction:
    ...
    def test_extract_category_standard(self):
        ...
        assert _extract_category("xoneai.memory.show") == "memory"
        assert _extract_category("xoneai.web.search") == "web"
        assert _extract_category("xoneai.file.read") == "file"
...
class TestBridgeRegistration:
    ...
    @patch("xoneai.mcp_server.adapters.tools_bridge.is_bridge_available")
    def test_register_skips_when_unavailable(self, mock_available):
        """Test registration skips when xoneaiagents not available."""
        from xoneai.mcp_server.adapters.tools_bridge import register_xoneai_tools
        
        mock_available.return_value = False
        count = register_xoneai_tools()
        assert count == 0
    
    @patch("xoneai.mcp_server.adapters.tools_bridge.is_bridge_available")
    @patch("xoneai.mcp_server.adapters.tools_bridge.get_tool_mappings")
    @patch("xoneai.mcp_server.registry.get_tool_registry")
    def test_register_tools_from_mappings(self, mock_get_registry, mock_mappings, mock_available):
        ...
        assert count == 1
        assert "xoneai.agents.test.tool" in mock_registry._tools

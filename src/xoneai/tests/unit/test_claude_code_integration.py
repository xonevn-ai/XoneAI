#!/usr/bin/env python3
"""
Test script for Claude Code integration with XoneAI Agents
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'src', 'xoneai'))

def test_claude_code_tool_import():
    """Test that we can import the Claude Code tool function"""
    try:
        from xoneai.ui.code import claude_code_tool
...
def test_xoneai_agents_import():
    """Test xoneaiagents import availability"""
    try:
        from xoneaiagents import Agent
        print("✅ XoneAI Agents is available")
        return True
    except ImportError as e:
        print(f"⚠️  XoneAI Agents not available (fallback to litellm): {e}")
        return False
...
def test_environment_variables():
    ...
    test_vars = [
        "XONEAI_CLAUDECODE_ENABLED",
        "XONEAI_CODE_REPO_PATH"
    ]
...
async def run_tests():
    """Run all tests"""
    print("🧪 Testing Claude Code Integration with XoneAI Agents")
    print("=" * 60)
    
    tests = [
        ("Import Claude Code Tool", test_claude_code_tool_import),
        ("Import XoneAI Agents", test_xoneai_agents_import),

...
class TestAllBackendsProtocol:
    ...
    def test_auto_approve_protocol(self):
        from xoneaiagents.approval.protocols import ApprovalProtocol
        from xoneaiagents.approval import AutoApproveBackend

        assert isinstance(AutoApproveBackend(), ApprovalProtocol)

    def test_console_backend_protocol(self):
        from xoneaiagents.approval.protocols import ApprovalProtocol
        from xoneaiagents.approval import ConsoleBackend
...
class TestSyncAsyncParity:
    ...
    @pytest.mark.parametrize("cls_path,kwargs", [
        ("xoneai.bots._slack_approval.SlackApproval", {"token": "xoxb-f", "channel": "C1"}),
        ("xoneai.bots._telegram_approval.TelegramApproval", {"token": "f", "chat_id": "1"}),
        ("xoneai.bots._discord_approval.DiscordApproval", {"token": "f", "channel_id": "1"}),
        ("xoneai.bots._webhook_approval.WebhookApproval", {"webhook_url": "https://x.com"}),
        ("xoneai.bots._http_approval.HTTPApproval", {"port": 0}),
    ])
...
class TestMultiAgentDifferentBackends:
    ...
    def test_three_agents_three_backends(self):
        from xoneaiagents import Agent
        from xoneai.bots import SlackApproval, TelegramApproval, DiscordApproval
...
class TestSharedBaseUtilities:
    ...
    def test_classify_approve_keywords(self):
        from xoneai.bots._approval_base import classify_keyword
    ...
    def test_sync_wrapper_works(self):
        from xoneai.bots._approval_base import sync_wrapper
...
class TestLazyLoading:
    ...
    def test_bots_init_unknown_raises(self):
        with pytest.raises((AttributeError, ImportError)):
            from xoneai.bots import NonExistentBackend  # noqa: F401

...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', '..', '..', 'xoneai-agents'))

try:
    from xoneaiagents.agents.autoagents import AutoAgents, AutoAgentsConfig, AgentConfig, TaskConfig
    from xoneaiagents.llm import OpenAIClient, LLM
...
class TestAutoAgents:
    ...
    @patch('xoneaiagents.llm.supports_structured_outputs')
    @patch('xoneaiagents.agents.autoagents.get_openai_client')
    def test_generate_config_openai_success(self, mock_get_client, mock_supports_structured, sample_valid_config, mock_tools):
        ...
        with patch.object(AutoAgents, '_create_agents_and_tasks') as mock_create, \
             ...
             patch('xoneaiagents.agents.agents.Agents.__init__') as mock_super_init:
    ...
    @patch('xoneaiagents.llm.supports_structured_outputs')
    @patch('xoneaiagents.agents.autoagents.LLM')

...
class TestTemplateResolver:
    ...
    def test_resolve_local_path(self):
        ...
        from xoneai.templates.resolver import TemplateResolver, TemplateSource
    ...
    def test_cache_key_generation(self):
        ...
        from xoneai.templates.resolver import TemplateResolver
...
class TestTemplateCache:
    ...
    def test_cache_put_and_get(self, temp_cache_dir):
        ...
        from xoneai.templates.cache import TemplateCache
        from xoneai.templates.resolver import TemplateResolver
    ...
    def test_cache_expiration(self, temp_cache_dir):
        ...
        from xoneai.templates.cache import TemplateCache, CacheMetadata
        import time
...
class TestTemplateSecurity:
    ...
    def test_source_allowlist(self):
        ...
        from xoneai.templates.security import TemplateSecurity, SecurityConfig
        
        config = SecurityConfig(
            allowed_sources={"github:xonevn-ai/agent-recipes"},
            allow_any_github=False
        )
        security = TemplateSecurity(config=config)
        
        # Allowed source
        assert security.is_source_allowed("github:xonevn-ai/agent-recipes/template") is True
    ...
    def test_local_paths_allowed_by_default(self):
        ...
        from xoneai.templates.security import TemplateSecurity
...
class TestCLIIntegration:
    ...
    def test_templates_handler_init(self):
        ...
        from xoneai.cli.features.templates import TemplatesHandler
...
class TestFromTemplateClassmethods:
    ...
    def test_agent_from_template_import_error(self):
        """Test Agent.from_template raises ImportError when xoneai not available."""
        # This test verifies the error handling path
        # In actual use, xoneai would be available
        pass  # Skip for now as xoneai is available in test environment
    
    def test_workflow_from_template_import_error(self):
        """Test Workflow.from_template raises ImportError when xoneai not available."""
        # This test verifies the error handling path
        pass  # Skip for now
...
""")
    ...
    def test_sentinel_collision_handling(self, temp_template_with_input):
        ...
        sentinel_injection = "__XONEAI_INPUT_SENTINEL__ {{danger}}"
        template = loader.load(
            str(temp_template_with_input),
...
""")
    ...
    def test_xdg_paths_supported(self):
        ...
        Should support:
        - ~/.xone/templates
        - ~/.config/xone/templates
        """
        from xoneai.templates.resolver import TemplateResolver, TemplateSource
        
        # Test home directory path resolution
        resolved = TemplateResolver.resolve("~/.xone/templates/my-template")
        assert resolved.source == TemplateSource.LOCAL
        assert os.path.expanduser("~") in resolved.path
        
        resolved = TemplateResolver.resolve("~/.config/xone/templates/my-template")
        assert resolved.source == TemplateSource.LOCAL
        assert os.path.expanduser("~") in resolved.path
...
""")
    ...
    def test_convenience_functions(self, discovery_setup):
        ...
        from xoneai.templates.discovery import discover_templates, find_template_path
...
# =============================================================================
# TODO 6: Tests for 'xoneai tools doctor'
# =============================================================================
...
class TestToolsDoctor:
    ...
    def test_doctor_returns_structured_output(self):
        ...
        assert isinstance(result, dict)
        assert "xoneai_tools_installed" in result
        assert "builtin_tools" in result
        assert "custom_tools_dirs" in result
        assert "issues" in result
    
    def test_doctor_detects_xoneai_tools(self):
        """Test that doctor detects xoneai-tools installation."""
        from xoneai.templates.tools_doctor import ToolsDoctor
        ...
        # Should be a boolean
        assert isinstance(result["xoneai_tools_installed"], bool)
    ...
    def test_doctor_json_output(self):
        ...
        assert "xoneai_tools_installed" in parsed
...
''')
    ...
    def test_load_tools_from_file(self, custom_tools_file):
        ...
        from xoneai.templates.tool_override import ToolOverrideLoader
    ...
    def test_reject_remote_urls(self):
        ...
        from xoneai.templates.tool_override import ToolOverrideLoader, SecurityError
    ...
    def test_default_custom_dirs(self):
        ...
        assert any(".xone/tools" in str(d) for d in default_dirs)
        assert any(".config/xone/tools" in str(d) or ".xone/tools" in str(d) for d in default_dirs)
...
""")
    ...
    def test_tools_py_loaded(self, temp_simplified_recipe):
        ...
        from xoneai.templates.tool_override import create_tool_registry_with_overrides
...
class TestCLIToolsDoctor:
    ...
    def test_cli_doctor_runs(self):
        ...
        from xoneai.cli.features.tools import ToolsHandler

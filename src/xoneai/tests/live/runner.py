...
This runner is CLI-FIRST: all scenarios execute via `xoneai code` or `xoneai chat`
...
- XONE_APPROVAL_MODE=auto  - Auto-approve all tool executions (required for non-interactive)
- XONEAI_LIVE_SMOKE=1      - Enable live smoke tests
- XONEAI_LIVE_MODEL        - Override default model (default: gpt-4o-mini)
...
XONEAI_PATH = Path(__file__).parent.parent.parent.parent / "xoneai"
AGENTS_PATH = Path(__file__).parent.parent.parent.parent / "xoneai-agents"

DEFAULT_MODEL = os.environ.get("XONEAI_LIVE_MODEL", "gpt-4o-mini")
...
class Scenario:
    ...
    use_code_command: bool = True  # Use `xoneai code` vs `xoneai chat`


def is_live_smoke_enabled() -> bool:
    """Check if live smoke tests are enabled."""
    return os.environ.get("XONEAI_LIVE_SMOKE", "0") == "1"
...
def create_working_dir() -> Path:
    ...
    work_dir = Path(tempfile.mkdtemp(prefix="xoneai_code_editor_"))
...
def build_cli_env(extra_env: Optional[dict] = None) -> dict:
    ...
    # Set PYTHONPATH to include xoneai and xoneai-agents
    paths = []
    if XONEAI_PATH.exists():
        paths.append(str(XONEAI_PATH.parent))  # Parent of xoneai package
    ...
    run_env["XONE_APPROVAL_MODE"] = "auto"
...
def run_xoneai_cli(
    ...
    """
    Run xoneai CLI command and return (exit_code, stdout, stderr, cli_command).
    
    This is the CLI-FIRST approach: we invoke `python -m xoneai code` or 
    `python -m xoneai chat` as a subprocess, exactly as a user would.
    
    We use `python -m xoneai` instead of the `xoneai` entry point to ensure
    ...
    Args:
        ...
        use_code: Use `xoneai code` (True) or `xoneai chat` (False)
    ...
    cmd = [sys.executable, "-m", "xoneai", "code" if use_code else "chat"]
    ...
    # Human-readable command for logging (using xoneai directly)
    cli_command = f"xoneai {'code' if use_code else 'chat'} -m {effective_model} -w {workspace} \"{prompt[:50]}...\""
...
def run_scenario(scenario: Scenario, work_dir: Path) -> ScenarioResult:
    ...
    This executes `xoneai code` or `xoneai chat` as a subprocess,
    ...
    # Run the CLI command
    exit_code, stdout, stderr, cli_command = run_xoneai_cli(

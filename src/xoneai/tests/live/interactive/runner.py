...
- XONE_APPROVAL_MODE         - Auto-set to 'auto' for non-interactive
"""
...
def is_live_interactive_enabled() -> bool:
    ...
    return os.environ.get("XONEAI_LIVE_INTERACTIVE", "0") == "1"
...
class LiveInteractiveRunner:
    ...
    def __init__(
        ...
        self.artifacts_dir = artifacts_dir or Path(tempfile.mkdtemp(prefix="xone_live_"))
    ...
    def _run_single(self, scenario: InteractiveScenario, attempt: int) -> ScenarioResult:
        ...
        # Create workspace
        workspace = Path(tempfile.mkdtemp(prefix=f"xone_{scenario.id}_"))
        artifacts_path = self.artifacts_dir / scenario.id / f"attempt_{attempt}"
        
        try:
            # Import harness (lazy)
            from xoneai.cli.features.interactive_test_harness import (

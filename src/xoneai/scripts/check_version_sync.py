#!/usr/bin/env python3
"""
CI script to check version sync between xoneai-agents and xoneai packages.
...
def load_dependencies(pyproject_path: Path) -> Dict[str, str]:
    ...
    for extra, extra_deps in data.get("project", {}).get("optional-dependencies", {}).items():
        for dep in extra_deps:
            if not dep.startswith("xoneai"):  # Skip self-references
                name, version = parse_version_spec(dep)
                if name not in deps:
                    deps[name] = version
...
def main():
    ...
    agents_pyproject = repo_root / "src" / "xoneai-agents" / "pyproject.toml"
    wrapper_pyproject = repo_root / "src" / "xoneai" / "pyproject.toml"
    ...
    print(f"  xoneai-agents: {len(agents_deps)} dependencies")
    print(f"  xoneai: {len(wrapper_deps)} dependencies")

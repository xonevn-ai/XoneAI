...
echo "🔍 Running XoneAI Standardisation Check..."
echo ""

# Run the check
cd "$PROJECT_ROOT"
python -m xoneai.cli.commands.standardise check --ci --path "$PROJECT_ROOT"
EXIT_CODE=$?
...
if [[ $EXIT_CODE -eq 0 ]]; then
    ...
    if [[ "$STRICT_MODE" == "true" ]]; then
        ...
        echo "Run 'xoneai standardise report' for details."

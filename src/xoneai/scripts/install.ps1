# XoneAI Installer for Windows
# Usage: iwr -useb https://ai.xone.vn/install.ps1 | iex
#
# Parameters:
...
if ($env:XONEAI_VERSION) { $Version = $env:XONEAI_VERSION }
if ($env:XONEAI_EXTRAS) { $Extras = $env:XONEAI_EXTRAS }
if ($env:XONEAI_SKIP_VENV -eq "1") { $NoVenv = $true }
if ($env:XONEAI_PYTHON) { $Python = $env:XONEAI_PYTHON }
if ($env:XONEAI_DRY_RUN -eq "1") { $DryRun = $true }

$MinPythonVersion = [version]"3.10"
$VenvDir = "$env:USERPROFILE\.xoneai\venv"

function Write-Banner {
    Write-Host ""
    Write-Host "╔═══════════════════════════════════════════════════════════╗" -ForegroundColor Cyan
    Write-Host "║                                                           ║" -ForegroundColor Cyan
    Write-Host "║   XoneAI - AI Agents Made Simple                       ║" -ForegroundColor Cyan
    Write-Host "║                                                           ║" -ForegroundColor Cyan
...
function Show-Help {
    Write-Host "XoneAI Installer for Windows"
    Write-Host ""
    Write-Host "Usage: iwr -useb https://ai.xone.vn/install.ps1 | iex"
    Write-Host "       & ([scriptblock]::Create((iwr -useb https://ai.xone.vn/install.ps1))) [OPTIONS]"
    Write-Host ""
    Write-Host "Options:"
    ...
    Write-Host "  XONEAI_VERSION    Specific version to install"
    Write-Host "  XONEAI_EXTRAS     Comma-separated extras"
    Write-Host "  XONEAI_SKIP_VENV  Skip virtual environment (1 to enable)"
    Write-Host "  XONEAI_PYTHON     Path to Python executable"
    Write-Host "  XONEAI_DRY_RUN    Print what would happen (1 to enable)"
    Write-Host ""
    Write-Host "Examples:"
    Write-Host "  # Basic install"
    Write-Host "  iwr -useb https://ai.xone.vn/install.ps1 | iex"
    Write-Host ""
    Write-Host "  # Install with extras"
    Write-Host '  & ([scriptblock]::Create((iwr -useb https://ai.xone.vn/install.ps1))) -Extras "ui,chat"'
    Write-Host ""
}
...
function Install-XoneAI {
    param(
        [string]$PythonCmd,
        [string]$VenvPath
    )
    
    Write-Info "Installing XoneAI..."
    ...
    $installPkg = "xoneaiagents"
    if ($Version -ne "latest") {
        $installPkg = "xoneaiagents==$Version"
    }
    
    # Add extras if specified
    if ($Extras) {
        $installPkg = "xoneaiagents[$Extras]"
        if ($Version -ne "latest") {
            $installPkg = "xoneaiagents[$Extras]==$Version"
        }
    }
    ...
    if ($Extras -match "ui|chat|code") {
        $wrapperPkg = "xoneai[$Extras]"
        if ($Version -ne "latest") {
            $wrapperPkg = "xoneai[$Extras]==$Version"
        }
        Invoke-Expression "$pipCmd install --upgrade $wrapperPkg"
    }
    
    Write-Success "XoneAI installed successfully!"
}
...
function Setup-Path {
    ...
    if ($currentPath -notlike "*$binDir*") {
        ...
        Write-Success "Added XoneAI to user PATH"
    }
    else {
...
function Test-Installation {
    ...
    try {
        $version = & $pythonCmd -c "import xoneaiagents; print(f'Version: {xoneaiagents.__version__}')" 2>$null
        if ($version) {
            Write-Success "XoneAI agents package verified: $version"
        }
    }
    catch {
        Write-Error "Failed to import xoneaiagents"
        exit 1
    }
    
    # Check CLI
    if (Get-Command xoneai -ErrorAction SilentlyContinue) {
        $cliVersion = & xoneai --version 2>$null | Select-Object -First 1
        Write-Success "CLI available: xoneai $cliVersion"
    }
}
...
function Show-NextSteps {
    ...
    if ($VenvPath -and -not $NoVenv) {
        ...
        Write-Host ""
        Write-Host "  2. Or restart your terminal to use XoneAI globally"
        Write-Host ""
    }
    
    Write-Host "  Quick start:"
    Write-Host '     python -c "from xoneaiagents import Agent; Agent(name=''test'').start(''Hello!'')"' -ForegroundColor Cyan
    Write-Host ""
    Write-Host "  Documentation:"
    Write-Host "     https://docs.xone.ai" -ForegroundColor Cyan
    Write-Host ""
...
function Main {
    ...
    # Install XoneAI
    Install-XoneAI $pythonCmd $venvPath

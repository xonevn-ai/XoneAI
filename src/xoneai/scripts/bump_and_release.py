#!/usr/bin/env python3
"""
Combined bump version and release script for XoneAI package.

Usage:
    # Basic: bump xoneai to 3.8.2 with agents 0.11.8
...
2. Optionally waits for xoneaiagents to be available on PyPI
...
def get_xoneai_dir() -> Path:
    """Get the xoneai package directory."""
    return get_project_root() / "src/xoneai"
...
def validate_dependencies(max_retries: int = 3, retry_interval: int = 60, use_frozen: bool = False) -> bool:
    ...
    for attempt in range(max_retries):
        ...
        if not use_frozen:
            run(["uv", "cache", "clean"], cwd=xoneai_dir, silent=True)
        
        result = run(lock_cmd, cwd=xoneai_dir, check=False)
...
def release(version: str, use_frozen_lock: bool = False):
    ...
    if not use_frozen_lock:
        print("\n🧹 Clearing uv cache...")
        run(["uv", "cache", "clean"], cwd=xoneai_dir)
    
    print("\n📦 Running uv lock...")
    run(["uv", "lock", "--frozen"] if use_frozen_lock else ["uv", "lock"], cwd=xoneai_dir)
    
    # 3. uv build
    print("\n🔨 Running uv build...")
    run(["uv", "build"], cwd=xoneai_dir)
    ...
    print(f"\n✅ Released XoneAI {tag}")
    print("\nNext step:")
    print("  cd src/xoneai && uv publish")


def main():
    parser = argparse.ArgumentParser(
        description="Bump version and release XoneAI package",
...
"""
    ...
    parser.add_argument(
        "--agents", "-a",
        help="Update xoneaiagents dependency version (e.g., 0.11.8)",
        default=None
    )
    parser.add_argument(
        "--auto",
        action="store_true",
        help="Auto-detect latest xoneaiagents version from PyPI"
    )
    parser.add_argument(
        "--wait", "-w",
        action="store_true",
    ...
    # Handle --auto flag
    if args.auto:
        print("\n🔍 Auto-detecting latest xoneaiagents version from PyPI...")
        agents_version = get_pypi_version("xoneaiagents")
        if not agents_version:
            print("❌ Failed to auto-detect xoneaiagents version")
            sys.exit(1)
    ...
    if args.wait and args.agents:
        if not wait_for_pypi_version("xoneaiagents", args.agents, max_wait=args.max_wait):

...
INSTALL_URL="${XONEAI_INSTALL_URL:-https://ai.xone.vn/install.sh}"
EXPECTED_VERSION="${XONEAI_EXPECTED_VERSION:-}"
EXTRAS="${XONEAI_EXTRAS:-}"

echo "==> XoneAI Install Smoke Test"
...
if [[ -z "$EXPECTED_VERSION" ]]; then
  ...
  EXPECTED_VERSION=$(pip index versions xoneaiagents 2>/dev/null | head -1 | grep -oP '\(\K[^)]+' || echo "")
  if [[ -z "$EXPECTED_VERSION" ]]; then
...
if [[ -n "$EXTRAS" ]]; then
  XONEAI_EXTRAS="$EXTRAS" curl -fsSL "$INSTALL_URL" | bash
else
...
VENV_DIR="$HOME/.xoneai/venv"
if [[ -d "$VENV_DIR" ]]; then
  source "$VENV_DIR/bin/activate"
fi

# Check import works
python3 -c "import xoneaiagents; print(f'Installed version: {xoneaiagents.__version__}')"

INSTALLED_VERSION=$(python3 -c "import xoneaiagents; print(xoneaiagents.__version__)" 2>/dev/null)

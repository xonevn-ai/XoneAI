...
# Usage: curl -fsSL https://ai.xone.vn/install.sh | bash
#
# Environment variables:
#   XONEAI_VERSION     - Specific version to install (default: latest)
#   XONEAI_EXTRAS      - Comma-separated extras (e.g., "ui,chat,code")
#   XONEAI_NO_PROMPT   - Skip interactive prompts (1 to enable)
#   XONEAI_DRY_RUN     - Print what would happen without making changes
#   XONEAI_PYTHON      - Path to Python executable
#   XONEAI_SKIP_VENV   - Skip virtual environment creation (1 to enable)
...
# Configuration
VERSION="${XONEAI_VERSION:-latest}"
EXTRAS="${XONEAI_EXTRAS:-}"
NO_PROMPT="${XONEAI_NO_PROMPT:-0}"
DRY_RUN="${XONEAI_DRY_RUN:-0}"
PYTHON_CMD="${XONEAI_PYTHON:-}"
SKIP_VENV="${XONEAI_SKIP_VENV:-0}"
...
print_banner() {
    ...
    echo "╔═══════════════════════════════════════════════════════════╗"
    echo "║                                                           ║"
    echo "║   ${CYAN}XoneAI${NC}${BOLD} - AI Agents Made Simple                      ║"
    echo "║                                                           ║"
...
create_venv() {
    local python_cmd="$1"
    local venv_dir="${2:-$HOME/.xoneai/venv}"
    
    if [[ "$SKIP_VENV" == "1" ]]; then
        log_info "Skipping virtual environment (XONEAI_SKIP_VENV=1)"
        return 0
    fi
...
install_xoneai() {
    ...
    # Build install command - default is xoneai[all] for full experience
    local install_pkg="xoneai[all]"
    if [[ "$VERSION" != "latest" ]]; then
        install_pkg="xoneai[all]==$VERSION"
    fi
    
    # Override extras if specified
    if [[ -n "$EXTRAS" ]]; then
        install_pkg="xoneai[$EXTRAS]"
        if [[ "$VERSION" != "latest" ]]; then
            install_pkg="xoneai[$EXTRAS]==$VERSION"
        fi
    fi
    ...
    if [[ "$EXTRAS" == *"ui"* ]] || [[ "$EXTRAS" == *"chat"* ]] || [[ "$EXTRAS" == *"code"* ]]; then
        local wrapper_pkg="xoneai[$EXTRAS]"
        if [[ "$VERSION" != "latest" ]]; then
            wrapper_pkg="xoneai[$EXTRAS]==$VERSION"
        fi
        $pip_cmd install --upgrade "$wrapper_pkg"
    fi
    
    log_success "XoneAI installed successfully!"
}
...
setup_shell_path() {
    ...
    if grep -q "$bin_dir" "$shell_rc" 2>/dev/null; then
        ...
        echo "" >> "$shell_rc"
        echo "# XoneAI" >> "$shell_rc"
        echo "$path_line" >> "$shell_rc"
        log_success "Added XoneAI to PATH in $shell_rc"
    fi
...
verify_installation() {
    ...
    # Check import
    if $python_cmd -c "from xoneaiagents import Agent; print('Import successful')" 2>/dev/null; then
        log_success "XoneAI agents package verified"
    else
        log_error "Failed to import xoneaiagents"
        return 1
    fi
    
    # Check CLI if wrapper installed
    if command_exists xoneai; then
        local cli_version
        cli_version=$(xoneai --version 2>/dev/null | head -n1 || echo "unknown")
        log_success "CLI available: xoneai $cli_version"
    fi
}
...
print_next_steps() {
    ...
    if [[ -n "$venv_dir" ]] && [[ "$SKIP_VENV" != "1" ]]; then
        ...
        echo ""
        echo "  2. Or restart your terminal to use XoneAI globally"
        echo ""
    fi
    
    echo "  Quick start:"
    echo -e "     ${CYAN}python -c \"from xoneaiagents import Agent; Agent(name='test').start('Hello!')\"${NC}"
    echo ""
    echo "  Documentation:"
    echo -e "     ${CYAN}https://docs.xone.ai${NC}"
    echo ""
...
# Print help
print_help() {
    echo "XoneAI Installer"
    echo ""
    echo "Usage: curl -fsSL https://ai.xone.vn/install.sh | bash"
    echo "       curl -fsSL https://ai.xone.vn/install.sh | bash -s -- [OPTIONS]"
    echo ""
    echo "Options:"
    ...
    echo "  XONEAI_VERSION    Specific version to install"
    echo "  XONEAI_EXTRAS     Comma-separated extras"
    echo "  XONEAI_NO_PROMPT  Skip interactive prompts (1 to enable)"
    echo "  XONEAI_DRY_RUN    Print what would happen (1 to enable)"
    echo "  XONEAI_PYTHON     Path to Python executable"
    echo "  XONEAI_SKIP_VENV  Skip virtual environment (1 to enable)"
    echo ""
    echo "Examples:"
    echo "  # Basic install"
    echo "  curl -fsSL https://ai.xone.vn/install.sh | bash"
    echo ""
    echo "  # Install with UI extras"
    echo "  curl -fsSL https://ai.xone.vn/install.sh | bash -s -- --extras ui,chat"
    echo ""
    echo "  # Install specific version"
    echo "  curl -fsSL https://ai.xone.vn/install.sh | bash -s -- --version 0.14.0"
    echo ""
    echo "  # Dry run"
    echo "  curl -fsSL https://ai.xone.vn/install.sh | bash -s -- --dry-run"
}
...
main() {
    ...
    # Install XoneAI
    install_xoneai "$python_cmd" "$venv_dir"

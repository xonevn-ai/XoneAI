...
run_ui_interface() {
    ...
    if [[ $QUICK == true ]]; then
        ...
        if [[ $use_chainlit =~ ^[Yy]$ ]]; then
            ...
            if [[ $use_ag2 =~ ^[Yy]$ ]]; then
                xoneai --framework autogen --init "$prompt"
            else
                xoneai --init "$prompt"
            fi
        fi
...
if [[ $CICD == false ]]; then
    print_message "Cloning XoneAI repository and checking out ${TAG}..."
    if [ ! -d "XoneAI" ]; then
        git clone https://github.com/xonevn-ai/XoneAI.git
        cd XoneAI
        git checkout ${TAG}
    else
        cd XoneAI
...
if [[ $UI == true || $CHAT == true || $CODE == true || $REALTIME == true ]]; then
    ...
    # Check if xoneai is already installed
    if ! pip show xoneai &> /dev/null; then
        print_message "Installing XoneAI components..."
        pip install xoneai
        ...
        for component in "${components[@]}"; do
            pip install "xoneai[$component]"
        done
    else
        print_message "XoneAI is already installed. Skipping installation."
    fi
...
if [[ $RUN == true ]]; then
    ...
    if [[ $UI == true ]]; then
        ...
        echo "  Or visit: https://docs.xone.ai/ui/ui"
    fi
    if [[ $CHAT == true ]]; then
        echo "- To start the Chat interface: ./setup.sh -c --run"
        echo "  Or visit: https://docs.xone.ai/ui/chat"
    fi
    if [[ $CODE == true ]]; then
        echo "- To start the Code interface: ./setup.sh -d --run"
        echo "  Or visit: https://docs.xone.ai/ui/code"
    fi
    if [[ $REALTIME == true ]]; then
        echo "- To start the Realtime interface: ./setup.sh -r --run"
        echo "  Or visit: https://docs.xone.ai/ui/realtime"
    fi
fi

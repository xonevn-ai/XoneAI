#!/usr/bin/env python3
"""
Test for issue #950: Convert xoneaiagents to xoneai imports
The goal is to enable: from XoneAI import Agent instead of from Agents import Agent
"""
...
def test_issue_950_goal():
    ...
    print("Testing issue #950 goal: from XoneAI import Agent")
    
    try:
        # This is what the issue wants to achieve
        # Note: In Python, package names are case-sensitive, so we import from 'xoneai'
        # but the user goal is conceptually "from XoneAI import Agent"
        from xoneai import Agent
        print("✅ SUCCESS: `from xoneai import Agent` works!")
        ...
        from xoneai import Task, AgentManager
        print("✅ SUCCESS: `from xoneai import Task, AgentManager` works!")
...
def test_backward_compatibility():
    ...
    try:
        ...
        print("✅ SUCCESS: `from xoneaiagents import Agent` still works!")
...
def test_package_name_case_sensitivity():
    ...
    # The actual package name is 'xoneai' (lowercase)
    # but the issue mentions 'XoneAI' (capitalized)
    
    try:
        # This should work (lowercase)
        from xoneai import Agent as AgentLowercase
        print("✅ SUCCESS: `from xoneai import Agent` works (lowercase package name)")
        
        # Try with uppercase (this should fail in most cases unless there's a XoneAI package)
        try:
            from XoneAI import Agent as AgentUppercase
            print("✅ SUCCESS: `from XoneAI import Agent` works (uppercase package name)")
            ...
            print("   The working pattern is: from xoneai import Agent")
...
if __name__ == "__main__":
    print("Testing Issue #950: Convert xoneaiagents to xoneai imports")
    print("=" * 60)
    ...
    if all(results):
        ...
        print("✅ Users can now use: from xoneai import Agent")
        print("✅ Backward compatibility maintained: from xoneaiagents import Agent")
        print("\nNote: In Python, package names are case-sensitive.")
        print("The correct import is: from xoneai import Agent (lowercase)")
        sys.exit(0)
    else:

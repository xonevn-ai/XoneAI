#!/usr/bin/env python3
"""
XoneAI Agents - Real Execution Benchmark
...
def benchmark_xoneai(model, iterations, prompt):
    """Benchmark XoneAI agent execution using agent.start()"""
...
def benchmark_xoneai_litellm(model, iterations, prompt):
    """Benchmark XoneAI with LiteLLM backend using agent.start()"""
    from xoneaiagents import Agent
...
def save_results(results: dict, model: str, iterations: int, prompt: str):
    ...
    with open(filepath, 'w') as f:
        f.write('# XoneAI Agents - Real Execution Benchmark\n\n')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='XoneAI Agents - Real Execution Benchmark')
    ...
    print('=' * 60)
    print('XoneAI Agents - Real Execution Benchmark')
    print('=' * 60)
    print(f'\nModel: {model}')
    ...
    # Run benchmarks
    try:
        results['XoneAI'] = benchmark_xoneai(model, iterations, prompt)
    except Exception as e:
        print(f"XoneAI error: {e}")
    
    try:
        results['XoneAI (LiteLLM)'] = benchmark_xoneai_litellm(model, iterations, prompt)
    except Exception as e:
        print(f"XoneAI (LiteLLM) error: {e}")

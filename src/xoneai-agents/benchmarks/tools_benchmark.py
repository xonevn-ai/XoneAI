...
def run_benchmark():
    ...
    print('=' * 60)
    print('XoneAI Agents - Tools Benchmark')
    print('=' * 60)
    ...
    results['XoneAI'] = measure_instantiation(
        lambda: XoneAgent(name='Test', llm='gpt-4o-mini', tools=tools, output="silent")
    )
    
    results['XoneAI (LiteLLM)'] = measure_instantiation(
        lambda: XoneAgent(name='Test', llm='openai/gpt-4o-mini', tools=tools, output="silent")
    )
...
def save_results(results: dict, filename: str = 'TOOLS_BENCHMARK_RESULTS.md'):
    ...
    with open(filepath, 'w') as f:
        f.write('# XoneAI Agents - Tools Benchmark Results\n\n')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='XoneAI Agents - Tools Benchmark')

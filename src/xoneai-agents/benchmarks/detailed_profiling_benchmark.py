#!/usr/bin/env python3
"""
XoneAI Agents - Detailed Profiling Benchmark

Provides function-level profiling for XoneAI agent execution.
...
Usage:
    ...
    python benchmarks/detailed_profiling_benchmark.py --framework xoneai
...
def profile_xoneai_detailed(model, prompt):
    """Profile XoneAI agent execution in detail."""
    print("\n" + "=" * 80)
    print("DETAILED PROFILING: XoneAI")
    print("=" * 80)
    ...
    print("-" * 80)
    breakdown = profiler.get_detailed_breakdown(filter_pattern='xoneai', limit=20)
    ...
    print("-" * 80)
    breakdown = profiler2.get_detailed_breakdown(filter_pattern='xoneai', limit=30)
    print(f"{'Function':<40} {'File':<50} {'Calls':>8} {'CumTime':>10}")
    print("-" * 80)
    for item in breakdown:
        func = item['function'][:38]
    ...
    # XoneAI-specific functions
    print("\n--- XoneAI-Specific Functions ---")
    print("-" * 80)
    xoneai_funcs = profiler3.get_detailed_breakdown(filter_pattern='xoneai', limit=50)
    print(f"{'Function':<40} {'File':<50} {'Calls':>8} {'CumTime':>10}")
    print("-" * 80)
    for item in xoneai_funcs:
        func = item['function'][:38]
        file = item['file'].split('/')[-1][:48] if '/' in item['file'] else item['file'][:48]
        print(f"{func:<40} {file:<50} {item['calls']:>8} {item['cumulative_time']*1000:>9.2f}ms")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='XoneAI Agents - Detailed Profiling Benchmark')
    parser.add_argument('--framework', '-f', type=str, default='all', 
                        choices=['all', 'xoneai', 'agno', 'crewai'],
    ...
    print("=" * 80)
    print("XoneAI Agents - Detailed Profiling Benchmark")
    print("=" * 80)
    print(f"\nModel: {args.model}")
    ...
    if args.framework in ['all', 'xoneai']:
        try:
            results['XoneAI'] = profile_xoneai_detailed(args.model, args.prompt)
        except Exception as e:
            print(f"XoneAI error: {e}")
            import traceback
            traceback.print_exc()

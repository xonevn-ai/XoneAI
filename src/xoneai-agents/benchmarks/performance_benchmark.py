"""
XoneAI Agents - Comprehensive Performance Benchmark
...
def instantiate_xoneai_agent():
    """Instantiate a XoneAI agent with one tool."""
    return XoneAgent(
        name="Test Agent",
...
def instantiate_xoneai_litellm_agent():
    """Instantiate a XoneAI agent with LiteLLM backend."""
    return XoneAgent(
        name="Test Agent",
        instructions="Be concise, reply with one sentence.",
...
def save_benchmark_results(results: dict, baseline):
    ...
    with open(filepath, 'w') as f:
        f.write('# XoneAI Agents - Benchmark Results\n\n')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='XoneAI Agents - Comprehensive Performance Benchmark')
    parser.add_argument('--save', action='store_true', help='Save results to file')
    args = parser.parse_args()
    
    print("="*70)
    print("XoneAI Agents - Comprehensive Performance Benchmark")
    print("="*70)
    ...
    try:
        print("\n" + "="*70)
        print("Testing XoneAI Agents")
        print("="*70)
        results['xoneai'] = run_benchmark(
            instantiate_xoneai_agent,
            "XoneAI",
            iterations=1000,
            warmup=10
        )
    except Exception as e:
        print(f"Error testing XoneAI: {e}")
    
    try:
        print("\n" + "="*70)
        print("Testing XoneAI Agents (LiteLLM)")
        print("="*70)
        results['xoneai_litellm'] = run_benchmark(
            instantiate_xoneai_litellm_agent,
            "XoneAI (LiteLLM)",
            iterations=1000,
            warmup=10
        )
    except Exception as e:
        print(f"Error testing XoneAI (LiteLLM): {e}")
    ...
    if 'xoneai' in results:
        baseline = results['xoneai']

#!/usr/bin/env python3
"""
Comprehensive Quick Benchmark - XoneAI vs Agno
...
def measure_import_time(module_name, clear_cache=True, full_agent_import=True):
    ...
    if module_name == 'xoneaiagents':
        if full_agent_import:
            from xoneaiagents import Agent  # noqa: F401
        else:
...
def get_xoneai_factory():
    """Get XoneAI agent factory function."""
...
def main():
    parser = argparse.ArgumentParser(
        description="Comprehensive Quick Benchmark - XoneAI vs Agno",
...
Examples:
  ...
  python benchmarks/quick_benchmark.py --all     # All details
    ...
    print("=" * 70)
    print("COMPREHENSIVE QUICK BENCHMARK - XoneAI vs Agno")
    print("=" * 70)
    ...
    results['xoneai']['import_ms'] = measure_import_time('xoneaiagents')
    ...
    xoneai_factory = get_xoneai_factory()
    ...
    results['xoneai']['time'] = measure_instantiation(xoneai_factory, args.iterations)
    ...
    results['xoneai']['memory'] = measure_memory(xoneai_factory, min(args.iterations, 20))
    ...
    results['xoneai']['calls'] = measure_function_calls(xoneai_factory, args.iterations)
    ...
    print("-" * 50)
    p_import = results['xoneai']['import_ms']
    a_import = results['agno']['import_ms']
    print(f"{'XoneAI':<15} {p_import:<15.2f} {'1.00x':<15}")
    ...
    print("-" * 50)
    p_time = results['xoneai']['time']
    a_time = results['agno']['time']
    print(f"{'XoneAI':<15} {p_time['avg']:<12.2f} {p_time['min']:<12.2f} {p_time['median']:<12.2f}")
    ...
    print()
    if ratio > 1:
        print(f"  → XoneAI is {ratio:.2f}x slower ({diff:.2f}μs more)")
    else:
        print(f"  → XoneAI is {1/ratio:.2f}x faster ({-diff:.2f}μs less)")
    ...
    print("-" * 50)
    p_mem = results['xoneai']['memory']
    a_mem = results['agno']['memory']
    print(f"{'XoneAI':<15} {p_mem['per_agent_kb']:<15.2f} {p_mem['peak_kb']:<15.2f}")
    ...
    print("-" * 50)
    p_calls = results['xoneai']['calls']
    a_calls = results['agno']['calls']
    print(f"{'XoneAI':<15} {p_calls['calls_per_agent']:<15.1f} {p_calls['total_calls']:<15}")
    ...
    print(f"{'Metric':<20} {'XoneAI':<15} {'Agno':<15} {'Winner':<15}")
    print("-" * 65)
    ...
    for name, p_val, a_val, better in metrics:
        if better == "lower":
            winner = "XoneAI ✓" if p_val <= a_val else "Agno ✓"
            if p_val <= a_val:
                xone_wins += 1
            else:
                agno_wins += 1
    ...
    print("-" * 65)
    print(f"Score: XoneAI {xone_wins} - Agno {agno_wins}")
    print("=" * 70)
    ...
    if args.show_import:
        ...
        print("=" * 70)
        print(f"XoneAI: {results['xoneai']['import_ms']:.2f}ms")
    ...
    if args.profile:
        print()
        print("=" * 70)
        print("XONEAI DEEP PROFILE")
        print("=" * 70)
        print(run_deep_profile(xoneai_factory, args.iterations))

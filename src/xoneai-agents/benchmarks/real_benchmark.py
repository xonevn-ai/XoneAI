...
def benchmark_xoneai_single(iterations: int = 3) -> ExecutionResult:
    """Benchmark single XoneAI agent execution."""
    from xoneaiagents import Agent
    
    result = ExecutionResult()
    
    for i in range(iterations):
        print(f"  XoneAI single agent iteration {i+1}/{iterations}...")
...
def benchmark_xoneai_two_agents(iterations: int = 3) -> ExecutionResult:
    """Benchmark 2 XoneAI agents working together."""
    from xoneaiagents import Agent, AgentManager
    
    result = ExecutionResult()
    
    for i in range(iterations):
        print(f"  XoneAI 2-agent iteration {i+1}/{iterations}...")
...
def benchmark_xoneai_three_agents(iterations: int = 3) -> ExecutionResult:
    """Benchmark 3 XoneAI agents working together."""
    from xoneaiagents import Agent, AgentManager
    
    result = ExecutionResult()
    
    for i in range(iterations):
        print(f"  XoneAI 3-agent iteration {i+1}/{iterations}...")
...
def save_results(results: dict, versions: dict):
    ...
    with open(filepath, 'w') as f:
        f.write('# XoneAI Agents - Real-World Execution Benchmark\n\n')
        f.write(f'**Generated:** {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}\n')
        f.write('**Model:** gpt-4o-mini\n')
        f.write('**Iterations:** 3 per test\n\n')
        
        f.write('## Results Summary\n\n')
        f.write('| Test | XoneAI (avg) | Agno (avg) | Difference |\n')
        f.write('|------|-----------------|------------|------------|\n')
        ...
        for test, name in zip(tests, test_names):
            xone_key = f'xoneai_{test}'
            agno_key = f'agno_{test}'
            
            if xone_key in results and agno_key in results:
                xone_avg = results[xone_key].avg_time
                agno_avg = results[agno_key].avg_time
                
                if xone_avg < agno_avg:
                    diff = f'XoneAI {agno_avg/xone_avg:.2f}x faster'
                else:
                    diff = f'Agno {xone_avg/agno_avg:.2f}x faster'
                
                f.write(f'| {name} | {xone_avg:.2f}s | {agno_avg:.2f}s | {diff} |\n')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='XoneAI Agents - Real-World Execution Benchmark')
    parser.add_argument('--save', action='store_true', help='Save results to file')
    args = parser.parse_args()
    
    print("="*70)
    print("XoneAI Agents - Real-World Execution Benchmark")
    print("="*70)
    ...
    print("\nTesting XoneAI single agent...")
    results['xoneai_single'] = benchmark_xoneai_single()
    ...
    print("\nTesting XoneAI 2 agents...")
    results['xoneai_two_agents'] = benchmark_xoneai_two_agents()
    ...
    print("\nTesting XoneAI 3 agents...")
    results['xoneai_three_agents'] = benchmark_xoneai_three_agents()
    ...
    print(f"\n{'Test':<20} {'XoneAI (avg)':<18} {'Agno (avg)':<15} {'Difference':<20}")
    print("-" * 73)
    
    tests = [
        ('Single Agent', 'xoneai_single', 'agno_single'),
        ('2 Agents', 'xoneai_two_agents', 'agno_two_agents'),
        ('3 Agents', 'xoneai_three_agents', 'agno_three_agents'),
    ]
    
    for name, xone_key, agno_key in tests:
        xone_avg = results[xone_key].avg_time
        agno_avg = results[agno_key].avg_time
        ...
        print(f"{name:<20} {xone_avg:<18.2f}s {agno_avg:<15.2f}s {diff:<20}")

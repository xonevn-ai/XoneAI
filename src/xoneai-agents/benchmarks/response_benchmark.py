"""
XoneAI Agents - Response Time Benchmark
...
def benchmark_xoneai_response(iterations: int = 5) -> ResponseResult:
    """Benchmark XoneAI agent response time"""
    from xoneaiagents import Agent
    
    result = ResponseResult()
    
    for i in range(iterations):
        print(f"  XoneAI iteration {i+1}/{iterations}...")
...
def benchmark_xoneai_with_tool(iterations: int = 5) -> ResponseResult:
    """Benchmark XoneAI agent response time with tool calling"""
    from xoneaiagents import Agent
    
    result = ResponseResult()
    
    for i in range(iterations):
        print(f"  XoneAI (with tool) iteration {i+1}/{iterations}...")
...
if __name__ == "__main__":
    print("="*70)
    print("XoneAI Agents - Response Time Benchmark")
    print("="*70)
    print("\nModel: gpt-4o-mini")
    ...
    print("\nBenchmarking XoneAI...")
    try:
        results['xoneai_simple'] = benchmark_xoneai_response(iterations)
    ...
    print("\nBenchmarking XoneAI with tool...")
    try:
        results['xoneai_tool'] = benchmark_xoneai_with_tool(iterations)
    except Exception as e:
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
    ...
    print(f"\n{'Test':<30} {'XoneAI (avg)':<20} {'Agno (avg)':<20} {'Ratio':<15}")
    print("-" * 85)
    
    if 'xoneai_simple' in results and 'agno_simple' in results:
        p_time = results['xoneai_simple'].avg_time
    ...
    if 'xoneai_tool' in results and 'agno_tool' in results:
        p_time = results['xoneai_tool'].avg_time

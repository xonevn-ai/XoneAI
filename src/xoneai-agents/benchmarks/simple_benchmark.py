#!/usr/bin/env python3
"""
XoneAI Agents - Performance Benchmark
...
def run_benchmark():
    ...
    print('=' * 60)
    print('XoneAI Agents - Performance Benchmark')
    print('=' * 60)
    print(f'\nIterations: {ITERATIONS}')
    print('Metric: Agent instantiation time (microseconds)\n')
    
    # XoneAI (without tools)
    print("Testing XoneAI...")
    from xoneaiagents import Agent as XoneAgent
    
    results['XoneAI'] = measure_instantiation(
        lambda: XoneAgent(name='Test', llm='gpt-4o-mini', output="silent")
    )
    
    results['XoneAI (LiteLLM)'] = measure_instantiation(
        lambda: XoneAgent(name='Test', llm='openai/gpt-4o-mini', output="silent")
    )
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='XoneAI Agents - Performance Benchmark')

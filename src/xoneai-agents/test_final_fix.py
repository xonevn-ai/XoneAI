...
if '_imported_symbols' in content and 'for symbol in _xoneaiagents_exports:' in content:
    print("✓ Dynamic __all__ construction is implemented")
else:
    print("❌ Dynamic __all__ construction is NOT implemented")

# Check for the problematic pattern
if 'from xoneaiagents import (' in content:
    print("❌ Still using static import from xoneaiagents")
else:
    print("✓ No longer using static import from xoneaiagents")
...
try:
    ...
    dummy_module = type(sys)('dummy_xoneaiagents')
    dummy_module.Agent = type('Agent', (), {'__name__': 'Agent'})
    dummy_module.Task = type('Task', (), {'__name__': 'Task'})
    dummy_module.XoneAIAgents = type('XoneAIAgents', (), {'__name__': 'XoneAIAgents'})
    
    # Test the import logic
    _xoneaiagents_exports = ['Agent', 'Task', 'XoneAIAgents', 'NonExistentClass']
    _imported_symbols = []
    
    # Simulate the import logic
    for symbol in _xoneaiagents_exports:
        if hasattr(dummy_module, symbol):
            _imported_symbols.append(symbol)
    
    expected_symbols = ['Agent', 'Task', 'XoneAIAgents']
...
core_exports = ['XoneAI', '__version__']
_imported_symbols = ['Agent', 'Task', 'XoneAIAgents']  # Simulated successful imports

__all__ = core_exports + _imported_symbols

expected_all = ['XoneAI', '__version__', 'Agent', 'Task', 'XoneAIAgents']
...
# The issue was: "The `__all__` list unconditionally includes symbols from `xoneaiagents`"
...
print("2. Only including symbols that were actually imported from xoneaiagents")

"""
Performance Analysis Utilities for XoneAI
...
Features:
- Function flow analysis and visualization (opt-in via XONEAI_FLOW_ANALYSIS_ENABLED)
...
_FLOW_ANALYSIS_ENABLED = os.environ.get('XONEAI_FLOW_ANALYSIS_ENABLED', '').lower() in ('true', '1', 'yes')
...
class FunctionFlowAnalyzer:
    ...
    def analyze_execution_flow(self, flow_data: Optional[List[Dict]] = None) -> Dict[str, Any]:
        ...
        Note: Expensive flow analysis operations are opt-in only via XONEAI_FLOW_ANALYSIS_ENABLED
        ...
        if _FLOW_ANALYSIS_ENABLED:
            ...
            analysis["note"] = "Advanced flow analysis disabled. Set XONEAI_FLOW_ANALYSIS_ENABLED=true to enable expensive pattern detection."

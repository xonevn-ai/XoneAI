# Session Persistence for XoneAI Agents
...
Sessions are stored in: `~/.xone/sessions/{session_id}.json`
...
```python
from xoneaiagents.session import get_default_session_store
...
```python
from xoneaiagents.session import DefaultSessionStore
...
```python
from xoneaiagents import Agent
from xoneai.db import PostgresAdapter
...
class DefaultSessionStore:
    def __init__(
        self,
        session_dir: Optional[str] = None,  # Default: ~/.xone/sessions/

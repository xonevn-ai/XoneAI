"""
Session Management for XoneAI Agents
...
class Session:
    """
    A simple wrapper around XoneAI's existing stateful capabilities.
    ...
    def __init__(
        ...
        if not self.is_remote:
            default_memory_config = {
                "provider": "rag",
                "use_embedding": False,
                "rag_db_path": f".xoneai/sessions/{self.session_id}/chroma_db"
            }
            if memory_config:
            ...
            os.makedirs(f".xoneai/sessions/{self.session_id}", exist_ok=True)

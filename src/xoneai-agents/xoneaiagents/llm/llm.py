...
Output MUST be JSON with 'reflection' and 'satisfactory'.
    ...
    def can_use_tools(self) -> bool:
        ...
        try:
            ...
            raise ImportError(
                "LiteLLM is required but not installed. "
                "Please install it with: pip install 'xoneaiagents[llm]'"
            )
        except:
            return False
    ...
    def _build_completion_params(self, **override_params) -> Dict[str, Any]:
        ...
        # Always remove internal XoneAI parameters that are not native litellm parameters
        # These are used internally by XoneAI but should not be passed to the LLM API
        internal_params = [

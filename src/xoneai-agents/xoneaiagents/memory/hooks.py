"""
Cascade Hooks System for XoneAI Agents.
...
- JSON configuration (.xoneai/hooks.json)
- Script execution with timeout
- Exit code handling
- Lazy loading for performance

Configuration (.xoneai/hooks.json):
    {
        "hooks": {
...
class HooksManager:
    ...
    Hooks are configured in .xoneai/hooks.json and can execute
            ...
            if hook.pass_input:
                for key, value in context.items():
                    if isinstance(value, (str, int, float, bool)):
                        env[f"XONE_HOOK_{key.upper()}"] = str(value)
                
                # Also pass as JSON
                env["XONE_HOOK_CONTEXT"] = json.dumps(context)
    ...
    def create_config(
        ...
        """
        config_dir = self.workspace_path / ".xoneai"

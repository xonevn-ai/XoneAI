"""MCP Server wrapper for exposing XoneAI tools as MCP servers.
...
def launch_tools_mcp_server(
    ...
    Args:
        ...
        tool_names: List of tool names to load from xoneaiagents.tools
        name: Name of the MCP server
        transport: Transport type - "stdio" or "sse"
    ...
    if tool_names:
        from xoneaiagents import tools as xone_tools
        
        for tool_name in tool_names:
            if hasattr(xone_tools, tool_name):
                tool_func = getattr(xone_tools, tool_name)
                if callable(tool_func):
                    server.register_tool(tool_func)
                else:
                    logger.warning(f"Tool '{tool_name}' is not callable")
            else:
                logger.warning(f"Tool '{tool_name}' not found in xoneaiagents.tools")

# AGENTS.md - XoneAI SDK Comprehensive Guide

> **For AI Agents and Developers**: This document provides the complete context needed to work with the XoneAI ecosystem, including design principles, architecture, repository structure, and implementation guidelines.

---

## 1. What is XoneAI?

XoneAI is a **high-performance, agentic AI framework** designed for building production-ready AI agents and multi-agent workflows.
...
```
/Users/xonevn/xoneai-package/
├── src/
│   ├── xoneai-agents/          # Core SDK (xoneaiagents)
│   │   ├── xoneaiagents/       # Python package
│   │   ├── tests/                 # Unit & integration tests
│   │   ├── examples/              # Package-level examples
│   │   └── pyproject.toml         # Package config
│   │
│   ├── xoneai/                 # Wrapper (xoneai)
│   │   ├── xoneai/             # CLI, integrations, heavy impls
│   │   └── pyproject.toml
│   │
│   └── xoneai-ts/              # TypeScript SDK
│       └── src/
│
...
│
/Users/xonevn/XoneAI-tools/    # External tools/plugins
/Users/xonevn/XoneAIDocs/      # Documentation (Mintlify)
    └── docs/
        ├── agents/
        ├── api/
        ├── capabilities/
...
## 3. Core SDK Architecture (xoneaiagents)
...
```bash
grep -r "class.*Protocol" xoneaiagents/ --include="*.py"
```
...
```
Core SDK (xoneaiagents)          Wrapper (xoneai)
━━━━━━━━━━━━━━━━━━━━━━━━━━━         ━━━━━━━━━━━━━━━━━━━
...
def use_chromadb():
    try:
        ...
        raise ImportError("Install with: pip install xoneaiagents[memory]")
...
from xoneaiagents import Agent, MemoryConfig, HooksConfig
...
from xoneaiagents import Agent, ExecutionConfig, MemoryConfig
from xoneaiagents.agent.autonomy import AutonomyConfig
...
from xoneaiagents.tools import BaseTool
...
**Base files:**
- `xoneaiagents/tools/base.py` - BaseTool class
- `xoneaiagents/tools/decorator.py` - @tool decorator
- `xoneaiagents/tools/registry.py` - Tool registry
...
```python
from xoneaiagents.hooks import add_hook, before_tool, after_tool
...
```python
from xoneaiagents.memory.protocols import MemoryProtocol
...
```python
from xoneaiagents import Agent
from xoneaiagents.tools import schedule_add, schedule_list, schedule_remove
...
- `scheduler/store.py` — `FileScheduleStore` (thread-safe JSON persistence at `~/.xoneai/schedules/jobs.json`)
...
```python
# Base files in xoneaiagents/db/
from xoneaiagents.db import DbAdapter, AsyncDbAdapter
...
```bash
# Basic agent
xoneai agent run --name "assistant" --prompt "Hello"

# With tools
xoneai agent run --tools web_search --prompt "Search for Python"

# Multi-agent workflow
xoneai workflow run --file workflow.yaml

# Server mode
xoneai serve --port 8000
```
...
```python
# Most common
from xoneaiagents import Agent, Agents, Task, tool, Tools

# Configuration
from xoneaiagents import (
...
# Event Bus
from xoneaiagents.bus import EventBus, get_default_bus

# Memory
from xoneaiagents import Memory, Session

# Workflows
from xoneaiagents import Workflow, Route, Parallel, Loop
```
...
| What | Where |
|------|-------|
| Agent class | `xoneaiagents/agent/agent.py` |
| Tool decorator | `xoneaiagents/tools/decorator.py` |
| Base tool | `xoneaiagents/tools/base.py` |
| Hook system | `xoneaiagents/hooks/` |
| Memory protocols | `xoneaiagents/memory/protocols.py` |
| Event bus | `xoneaiagents/bus/bus.py` |
| Workflow engine | `xoneaiagents/workflows/workflows.py` |
| Policy engine | `xoneaiagents/policy/engine.py` |
| Scheduler module | `xoneaiagents/scheduler/` |
| Schedule tools | `xoneaiagents/tools/schedule_tools.py` |
...
BotOS is the multi-platform bot orchestration layer for XoneAI. It follows the same protocol-driven, agent-centric philosophy as the rest of the SDK.
...
|-------|---------|---------|
| `BotOSProtocol` | `xoneaiagents` (Core SDK) | Interface contract — lightweight |
| `BotOSConfig` | `xoneaiagents` (Core SDK) | Configuration dataclass |
| `Bot` | `xoneai` (Wrapper) | Single-platform bot wrapper |
| `BotOS` | `xoneai` (Wrapper) | Multi-platform orchestrator |
| `_registry` | `xoneai` (Wrapper) | Platform adapter registry |

### 12.3 Usage Patterns

```python
# === Level 1: Single bot, 3 lines ===
from xoneai.bots import Bot
from xoneaiagents import Agent
...
from xoneai.bots import BotOS, Bot
from xoneaiagents import Agent, AgentTeam, Task
...
from xoneai.bots._registry import register_platform
...
| What | Where |
|------|-------|
| BotOSProtocol | `xoneaiagents/bots/protocols.py` |
| BotOSConfig | `xoneaiagents/bots/config.py` |
| Bot class | `xoneai/bots/bot.py` |
| BotOS class | `xoneai/bots/botos.py` |
| Platform registry | `xoneai/bots/_registry.py` |
...
```python
from xoneai.bots._registry import register_platform, list_platforms
...
> Make XoneAI the **best agent framework in the world**
...
*This document is the source of truth for the XoneAI SDK architecture and design principles.*

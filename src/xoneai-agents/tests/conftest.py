"""
Pytest configuration for XoneAI Agents tests.
...
def pytest_collection_modifyitems(config, items):
    """Skip live tests unless XONEAI_LIVE_TESTS=1 is set."""
    if os.environ.get("XONEAI_LIVE_TESTS") != "1":
        skip_live = pytest.mark.skip(reason="Live tests disabled. Set XONEAI_LIVE_TESTS=1 to enable.")
        for item in items:
...
def live_test_enabled():
    ...
    if os.environ.get("XONEAI_LIVE_TESTS") != "1":
        pytest.skip("Live tests disabled. Set XONEAI_LIVE_TESTS=1 to enable.")
    return True

...
class TestScheduleModels:
    ...
    def test_schedule_every_creation(self):
        from xoneaiagents.scheduler.models import Schedule
    ...
    def test_schedule_job_creation(self):
        from xoneaiagents.scheduler.models import ScheduleJob, Schedule
...
class TestFileScheduleStore:
    ...
    def _make_store(self, tmp_path):
        from xoneaiagents.scheduler.store import FileScheduleStore
...
class TestScheduleParser:
    ...
    def test_parse_hourly(self):
        from xoneaiagents.scheduler.parser import parse_schedule
...
class TestScheduleTools:
    ...
    def test_schedule_add_returns_confirmation(self, tmp_path):
        from xoneaiagents.tools.schedule_tools import schedule_add
        with patch('xoneaiagents.tools.schedule_tools._get_store') as mock_gs:
            from xoneaiagents.scheduler.store import FileScheduleStore
    ...
    def test_schedule_list_returns_jobs(self, tmp_path):
        from xoneaiagents.tools.schedule_tools import schedule_list, schedule_add
        with patch('xoneaiagents.tools.schedule_tools._get_store') as mock_gs:
            from xoneaiagents.scheduler.store import FileScheduleStore
    ...
    def test_schedule_remove_returns_confirmation(self, tmp_path):
        from xoneaiagents.tools.schedule_tools import schedule_add, schedule_remove
        with patch('xoneaiagents.tools.schedule_tools._get_store') as mock_gs:
            from xoneaiagents.scheduler.store import FileScheduleStore
            store = FileScheduleStore(store_dir=str(tmp_path))
            mock_gs.return_value = store
    ...
    def test_schedule_remove_nonexistent(self, tmp_path):
        from xoneaiagents.tools.schedule_tools import schedule_remove
        with patch('xoneaiagents.tools.schedule_tools._get_store') as mock_gs:
            from xoneaiagents.scheduler.store import FileScheduleStore
            mock_gs.return_value = FileScheduleStore(store_dir=str(tmp_path))
    ...
    def test_schedule_list_empty(self, tmp_path):
        from xoneaiagents.tools.schedule_tools import schedule_list
        with patch('xoneaiagents.tools.schedule_tools._get_store') as mock_gs:
            from xoneaiagents.scheduler.store import FileScheduleStore
            mock_gs.return_value = FileScheduleStore(store_dir=str(tmp_path))
            result = schedule_list()
...
class TestScheduleRunner:
    ...
    def test_runner_creation(self, tmp_path):
        from xoneaiagents.scheduler.runner import ScheduleRunner
        from xoneaiagents.scheduler.store import FileScheduleStore
        store = FileScheduleStore(store_dir=str(tmp_path))
...
class TestScheduleHookEvents:
    ...
    def test_schedule_hook_events_exist(self):
        from xoneaiagents.hooks.types import HookEvent
...
class TestLazyImport:
    ...
    def test_xoneaiagents_import_no_scheduler(self):
        """Importing xoneaiagents should NOT load scheduler module."""

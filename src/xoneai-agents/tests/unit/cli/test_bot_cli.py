...
NOTE: These tests require the xoneai package to be installed.
They are skipped if xoneai is not available.
"""

import pytest
from unittest.mock import patch, MagicMock

# Skip entire module if xoneai is not installed
pytest.importorskip("xoneai", reason="xoneai package not installed")
...
class TestBotHandler:
    ...
    @patch('xoneai.cli.features.bots_cli.BotHandler._load_agent')
    def test_build_tools_browser(self, mock_load_agent):
        """Test _build_tools with browser enabled."""
        from xoneai.cli.features.bots_cli import BotHandler, BotCapabilities
        
        handler = BotHandler()
        caps = BotCapabilities(browser=True)
        
        # Mock the browser tool import
        with patch.dict('sys.modules', {'xoneai_tools': MagicMock()}):
...
class TestHandleBotCommand:
    ...
    def test_handle_bot_command_no_platform(self, capsys):
        ...
        from xoneai.cli.features.bots_cli import handle_bot_command
...
class TestBuildCapabilitiesFromArgs:
    ...
    def test_build_capabilities_from_args_defaults(self):
        ...
        from xoneai.cli.features.bots_cli import _build_capabilities_from_args
...
class TestAddCapabilityArgs:
    ...
    def test_add_capability_args(self):
        ...
        import argparse
        from xoneai.cli.features.bots_cli import _add_capability_args

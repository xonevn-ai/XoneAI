...
class TestProbeResult:
    def test_import(self):
        from xoneaiagents.bots import ProbeResult
...
class TestHealthResult:
    def test_import(self):
        from xoneaiagents.bots import HealthResult
        assert HealthResult is not None

    def test_healthy(self):
        from xoneaiagents.bots import HealthResult, ProbeResult
...
class TestBotProtocolExtensions:
    def test_protocol_has_probe(self):
        from xoneaiagents.bots import BotProtocol
...
class TestBackoffPolicy:
    def test_import(self):
        from xoneai.bots._resilience import BackoffPolicy
    ...
    def test_telegram_preset(self):
        from xoneai.bots._resilience import TELEGRAM_BACKOFF
...
class TestComputeBackoff:
    def test_first_attempt(self):
        from xoneai.bots._resilience import BackoffPolicy, compute_backoff
...
class TestIsRecoverableError:
    def test_timeout_is_recoverable(self):
        from xoneai.bots._resilience import is_recoverable_error
...
class TestIsConflictError:
    def test_409_conflict(self):
        from xoneai.bots._resilience import is_conflict_error
...
class TestConnectionMonitor:
    def test_record_success_resets(self):
        from xoneai.bots._resilience import ConnectionMonitor
    ...
    def test_record_error_increments(self):
        from xoneai.bots._resilience import ConnectionMonitor, BackoffPolicy
...
class TestSleepWithAbort:
    ...
    async def test_normal_completion(self):
        from xoneai.bots._resilience import sleep_with_abort
...
class TestBotConfigGroupPolicy:
    def test_default_group_policy(self):
        from xoneaiagents.bots import BotConfig
        config = BotConfig()
...
class TestConfigSchema:
    def test_valid_config(self):
        from xoneai.bots._config_schema import validate_bot_config
        raw = {
...
class TestDoctorBotChecks:
    def test_bot_checks_import(self):
        from xoneai.cli.features.doctor.checks.bot_checks import check_bot_tokens, check_bot_config
        assert callable(check_bot_tokens)
        assert callable(check_bot_config)

    def test_check_category_has_bots(self):
        from xoneai.cli.features.doctor.models import CheckCategory
        assert hasattr(CheckCategory, "BOTS")
        assert CheckCategory.BOTS.value == "bots"

    def test_bot_tokens_check_no_tokens(self):
        from xoneai.cli.features.doctor.checks.bot_checks import check_bot_tokens
        from xoneai.cli.features.doctor.models import CheckStatus
    ...
    def test_bot_config_missing(self):
        from xoneai.cli.features.doctor.checks.bot_checks import check_bot_config
        from xoneai.cli.features.doctor.models import CheckStatus
        result = check_bot_config("/nonexistent/bot.yaml")
        assert result.status == CheckStatus.WARN

    def test_get_bot_checks(self):
        from xoneai.cli.features.doctor.checks.bot_checks import get_bot_checks
...
class TestBotApprovalBackend:
    def test_import(self):
        from xoneai.bots._approval import BotApprovalBackend
...
class TestDaemonService:
    def test_daemon_import(self):
        from xoneai.daemon import get_daemon_status, install_daemon, uninstall_daemon
...
class TestOnboardWizard:
    def test_import(self):
        from xoneai.cli.features.onboard import OnboardWizard
        assert OnboardWizard is not None

    def test_generate_bot_yaml(self):
        from xoneai.cli.features.onboard import _generate_bot_yaml
    ...
    def test_platforms_info(self):
        from xoneai.cli.features.onboard import PLATFORMS
...
class TestImportTime:
    def test_core_sdk_import_time(self):
        ...
        result = subprocess.run(
            [sys.executable, "-c", 
             "import time; t=time.perf_counter(); import xoneaiagents; print(f'{(time.perf_counter()-t)*1000:.0f}')"],

...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "..", "..", "xoneai"))
...
class TestCLIFlagParsing:
    ...
    def _parse_whatsapp_args(self, *args):
        import argparse
        from xoneai.cli.features.bots_cli import add_bot_parser
...
class TestCLIToWhatsAppBotWiring:
    ...
    @patch("xoneai.cli.features.bots_cli.BotHandler._load_dotenv")
    def test_respond_to_wired(self, mock_dotenv, mock_load_agent):
        ...
        handler = BotHandler()
        with patch("xoneai.bots.whatsapp.WhatsAppBot") as MockBot:
...
class TestYAMLConfigWiring:
    ...
    def test_yaml_respond_to_list(self, mock_dotenv, mock_load_agent):
        ...
        with patch("xoneai.cli.features.bots_cli.BotHandler.start_whatsapp") as mock_start:

...
class TestMCPAuthStorage:
    ...
    def auth_storage(self, temp_auth_file):
        ...
        from xoneaiagents.mcp.mcp_auth_storage import MCPAuthStorage
...
class TestMCPAuthStorageDefaults:
    ...
    def test_default_filepath(self):
        """Default filepath is in user's xone config directory."""
        from xoneaiagents.mcp.mcp_auth_storage import get_default_auth_filepath
        
        default_path = get_default_auth_filepath()
        assert "xone" in str(default_path).lower() or ".xone" in str(default_path)

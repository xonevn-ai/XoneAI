...
class TestDictStrictValidation:
    ...
    def test_dict_validation_non_dataclass_config(self):
        ...
        from xoneaiagents.config.param_resolver import _get_valid_keys
...
class TestAgentConsolidatedParams:
    ...
    def test_agent_hooks_via_resolver(self):
        ...
        from xoneaiagents.config.feature_configs import HooksConfig
        from xoneaiagents.config.param_resolver import resolve, ArrayMode
    ...
    def test_agent_skills_via_resolver(self):
        ...
        from xoneaiagents.config.feature_configs import SkillsConfig
        from xoneaiagents.config.param_resolver import resolve
    ...
    def test_agent_knowledge_via_resolver(self):
        ...
        from xoneaiagents.config.feature_configs import KnowledgeConfig
        from xoneaiagents.config.param_resolver import resolve, ArrayMode
    ...
    def test_agent_guardrails_via_resolver(self):
        ...
        from xoneaiagents.config.feature_configs import GuardrailConfig
        from xoneaiagents.config.param_resolver import resolve, ArrayMode
        from xoneaiagents.config.presets import GUARDRAIL_PRESETS
...
class TestNamingAlias:
    """Tests for Agents vs XoneAIAgents naming.
    
    v4.0.0 Updates:
    - Agents is now a SILENT alias for AgentManager (no deprecation warning)
    - XoneAIAgents has been REMOVED entirely (raises ImportError)
    """
    
    def test_agents_is_silent_alias(self):
        """Agents should be available as silent alias for AgentManager."""
        from xoneaiagents import AgentManager, Agents
        # Agents is now a silent alias
        assert Agents is AgentManager
    
    def test_xoneaiagents_removed_v4(self):
        """XoneAIAgents was removed in v4 - should raise ImportError."""
        import pytest
        with pytest.raises(ImportError):
            from xoneaiagents import XoneAIAgents

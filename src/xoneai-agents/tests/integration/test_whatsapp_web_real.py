...
XONEAI_SRC = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "xoneai"
)
PYTHON = sys.executable


def run_cli(*args, timeout=15, env_extra=None):
    """Run a xoneai CLI command as subprocess, return (returncode, stdout, stderr)."""
    env = os.environ.copy()
    env["PYTHONPATH"] = XONEAI_SRC + os.pathsep + env.get("PYTHONPATH", "")
    ...
    cmd = [PYTHON, "-m", "xoneai", "bot", "whatsapp"] + list(args)
    proc = subprocess.run(
        cmd,
        capture_output=True,
        text=True,
...
def run_cli_background(*args, env_extra=None):
    """Start a xoneai CLI command as background process, return Popen."""
    env = os.environ.copy()
    env["PYTHONPATH"] = XONEAI_SRC + os.pathsep + env.get("PYTHONPATH", "")
    env.setdefault("OPENAI_API_KEY", "sk-test-integration-dummy")
    if env_extra:
...
class TestCLIWebModeLaunch:
    """Verify `xoneai bot whatsapp --mode web` actually starts."""
...
class TestRealAdapter:
    ...
    def test_adapter_import(self):
        ...
        sys.path.insert(0, XONEAI_SRC)
        from xoneai.bots._whatsapp_web_adapter import WhatsAppWebAdapter
...
class TestRealWhatsAppBot:
    ...
    def _get_bot_class(self):
        sys.path.insert(0, XONEAI_SRC)
        from xoneai.bots.whatsapp import WhatsAppBot
        return WhatsAppBot
...
class TestRealGatewayConfig:
    ...
    def _get_gateway_class(self):
        sys.path.insert(0, XONEAI_SRC)
        from xoneai.gateway.server import WebSocketGateway
...
class TestPerformance:
    ...
    def test_xoneai_bots_import_fast(self):
        """Importing xoneai.bots.whatsapp doesn't load neonize."""

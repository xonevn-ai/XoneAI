...
from xoneaiagents.memory import (
    FileMemory,
    RulesManager,
)
...
class TestLocalOverrideFiles(unittest.TestCase):
    """Test CLAUDE.local.md and XONE.local.md support."""
    ...
    def test_xone_local_md_loaded(self):
        """Test that XONE.local.md is discovered."""
        with tempfile.TemporaryDirectory() as tmpdir:
            xone_local = Path(tmpdir) / "XONE.local.md"
            xone_local.write_text("# Local Xone Rules")
            
            manager = RulesManager(workspace_path=tmpdir)
            
            self.assertIn("root:xone_local", manager._rules)
...
class TestAiDecisionActivation(unittest.TestCase):
    ...
    def test_ai_decision_rule_loaded(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            rules_dir = Path(tmpdir) / ".xoneai" / "rules"
            rules_dir.mkdir(parents=True)
...
class TestAutoMemory(unittest.TestCase):
    ...
    def test_extract_name(self):
        ...
        from xoneaiagents.memory import AutoMemoryExtractor
    ...
    def test_auto_memory_wrapper(self):
        ...
        from xoneaiagents.memory import AutoMemory
...
class TestWorkflows(unittest.TestCase):
    ...
    def test_workflow_discovery(self):
        """Test workflow discovery from .xone/workflows/."""
        from xoneaiagents.memory import WorkflowManager
        
        with tempfile.TemporaryDirectory() as tmpdir:
            workflows_dir = Path(tmpdir) / ".xoneai" / "workflows"
...
class TestHooks(unittest.TestCase):
    ...
    def test_create_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            config_path = Path(tmpdir) / ".xoneai" / "hooks.json"

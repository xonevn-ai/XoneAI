#!/usr/bin/env python3
"""
MongoDB Memory Example for XoneAI Agents

This example demonstrates how to use MongoDB as a memory provider for XoneAI agents.
MongoDB can be used as both a key-value store and a vector database for memory operations.

Prerequisites:
- Install MongoDB dependencies: pip install 'xoneaiagents[mongodb]'
...
def main():
    ...
    try:
        ...
        # Note: Direct memory access not available through research_system.memory
        # Instead, use Memory class directly for verification
        try:
            from xoneaiagents.memory import Memory
            
            # Create Memory instance with MongoDB config for verification
            ...
            print("Memory features not available - install with: pip install 'xoneaiagents[memory]'")
        except Exception as mem_error:
            print(f"Memory verification error: {mem_error}")
...
if __name__ == "__main__":
    print("MongoDB Memory Example for XoneAI Agents")
    print("=" * 50)
    ...
    print("3. MongoDB dependencies installed: pip install 'xoneaiagents[mongodb]'")
    print("=" * 50)

...
pub fn get_default_plugin_dirs() -> Vec<PathBuf> {
    ...
    dirs.push(PathBuf::from("./.xoneai/plugins/"));
    
    // Home directory plugins
    if let Some(home) = dirs::home_dir() {
        dirs.push(home.join(".xoneai/plugins/"));
    }
...
use xoneai::parity::plugins::{{PluginProtocol, ToolPluginProtocol, PluginMetadata, ToolDefinition}};
...
use xoneai::parity::plugins::{{PluginProtocol, HookPluginProtocol, PluginMetadata}};
...
use xoneai::parity::plugins::{{PluginProtocol, PluginMetadata}};
...
"#;
    ...
    fn test_default_plugin_dirs() {
        ...
        assert!(dirs[0].to_string_lossy().contains(".xoneai/plugins"));
    }
}

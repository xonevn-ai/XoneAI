...
/// Root configuration for XoneAI
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct XoneConfig {
    /// Plugins configuration
    #[serde(default)]
...
/// Global config cache
static CONFIG_CACHE: OnceLock<XoneConfig> = OnceLock::new();
...
fn find_config_file() -> Option<PathBuf> {
    ...
    let local_paths = [
        cwd.join(".xoneai").join("config.toml"),
        cwd.join("xoneai.toml"),
    ];
    ...
    if let Some(home) = dirs::home_dir() {
        let global_path = home.join(".xoneai").join("config.toml");
...
fn load_config_from_file() -> XoneConfig {
    let config_path = match find_config_file() {
        Some(path) => path,
        None => return XoneConfig::default(),
    };
    
    let content = match std::fs::read_to_string(&config_path) {
        Ok(c) => c,
        Err(_) => return XoneConfig::default(),
    };
...
pub fn get_config() -> &'static XoneConfig {
...
pub fn is_plugins_enabled() -> bool {
    ...
    if let Ok(env_value) = std::env::var("XONEAI_PLUGINS") {
...
mod tests {
    ...
    #[test]
    fn test_xone_config_default() {
        let config = XoneConfig::default();

//! Skills Module for XoneAI Rust SDK.
//!
...
//!
//! ```ignore
//! use xoneai::skills::{SkillManager, SkillProperties};
//!
...
impl SkillManager {
    ...
    pub fn get_default_dirs() -> Vec<PathBuf> {
        ...
        // Project-level
        dirs.push(PathBuf::from("./.xone/skills"));
        dirs.push(PathBuf::from("./.claude/skills"));

        // User-level
        if let Some(home) = dirs::home_dir() {
            dirs.push(home.join(".xone/skills"));
        }
...
mod tests {
    ...
    fn test_get_default_dirs() {
        ...
        assert!(dirs.iter().any(|d| d.to_string_lossy().contains("xone")));
    }
}

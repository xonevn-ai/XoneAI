...
pub fn tool(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        impl xoneai::Tool for #struct_name {
            fn name(&self) -> &str {
                #tool_name
            }
            ...
            async fn execute(&self, args: serde_json::Value) -> xoneai::Result<serde_json::Value> {
                #(
                    let #param_names: #param_types = serde_json::from_value(
                        args.get(#param_name_strs)
                            .cloned()
                            .unwrap_or(serde_json::Value::Null)
                    ).map_err(|e| xoneai::Error::tool(format!("Failed to parse {}: {}", #param_name_strs, e)))?;
                )*

                let result = #fn_name(#(#param_names),*).await;
                serde_json::to_value(result)
                    .map_err(|e| xoneai::Error::tool(format!("Failed to serialize result: {}", e)))
            }
        }
    };

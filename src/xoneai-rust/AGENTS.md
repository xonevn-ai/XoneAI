# AGENTS.md - XoneAI Rust SDK Comprehensive Guide

> **For AI Agents and Developers**: Complete context for working with the XoneAI Rust SDK, including architecture, crate structure, and development guidelines.

---

## 1. What is XoneAI Rust?

XoneAI Rust SDK is a **high-performance, agentic AI framework** for Rust. It mirrors the Python SDK's design philosophy while leveraging Rust's performance and safety guarantees.
...
├── src/
│   └── xoneai-rust/           # Rust SDK workspace (THIS DIRECTORY)
│       ├── Cargo.toml            # Workspace configuration
│       ├── AGENTS.md             # This file
│       │
│       ├── xoneai/            # Core library crate
│       │   ├── Cargo.toml
│       │   └── src/
...
│       │
│       ├── xoneai-derive/     # Proc-macro crate
│       │   ├── Cargo.toml
│       │   └── src/lib.rs        # #[tool] attribute macro
│       │
│       ├── xoneai-cli/        # CLI binary crate
│       │   ├── Cargo.toml
│       │   └── src/
│       │       ├── main.rs
...
│
/Users/xonevn/xoneai-package/examples/rust/    # Main examples directory
/Users/xonevn/XoneAIDocs/docs/rust/            # Documentation (Mintlify)
```
...
|-------|------|---------|-----------|
| `xoneai` | lib | Core library - Agent, Tools, Workflows | ✅ Published |
| `xoneai-derive` | proc-macro | `#[tool]` attribute macro | ✅ Published |
| `xoneai-cli` | bin | CLI binary for running agents | ✅ Published |

---

## 3. Core Architecture

### 3.1 Key Modules (xoneai crate)
...
```rust
use xoneai::{Agent, AgentBuilder, AgentConfig};
...
```rust
use xoneai::{AgentFlow, Route, Parallel, Loop, Repeat, FlowStep};
...
```rust
use xoneai::{Tool, ToolRegistry, ToolResult, ToolDefinition};
...
```rust
use xoneai::{Message, Role, LlmProvider, LlmResponse, LlmConfig};
...
```rust
use xoneai::config::MemoryConfig;
...
```rust
use xoneai::config::ExecutionConfig;
...
```rust
use xoneai::config::OutputConfig;
...
```rust
use xoneai::{Agent, AgentTeam, Process};
...
```rust
use xoneai::{Agent, AgentFlow, FlowStep, Route, Parallel, Repeat};
use std::sync::Arc;
...
xoneai-rust run workflow.yaml

# Single prompt
xoneai-rust "What is 2+2?"

# With specific model
xoneai-rust --model gpt-4o "Explain quantum computing"
```
...
use xoneai::tools::{Tool, ToolResult};
...
```rust
use xoneai::llm::{LlmProvider, Message, LlmResponse, ToolDefinition};
use async_trait::async_trait;
...
```rust
use xoneai::memory::{MemoryAdapter, Message};
use async_trait::async_trait;
...
# Specific crate
cargo test -p xoneai
cargo test -p xoneai-derive
cargo test -p xoneai-cli
...
cargo publish -p xoneai-derive
# wait ~1 min
cargo publish -p xoneai
# wait ~1 min
cargo publish -p xoneai-cli
```
...
- `xoneai/Cargo.toml`
- `xoneai-derive/Cargo.toml`
- `xoneai-cli/Cargo.toml`
...
// CLI: xoneai-rust memory clear --session <id>
```

| Feature | API | CLI Equivalent |
|---------|-----|----------------|
| Chat | `agent.chat(prompt)` | `xoneai-rust chat` |
| Run workflow | `team.start(task)` | `xoneai-rust run workflow.yaml` |
| Single prompt | `agent.run(task)` | `xoneai-rust "prompt"` |
...
```rust
// Most common
use xoneai::{Agent, AgentTeam, tool};

// Configuration
use xoneai::config::{MemoryConfig, OutputConfig, ExecutionConfig, HooksConfig};

// Workflows
use xoneai::workflows::{AgentFlow, Route, Parallel, Loop, Repeat, Process, StepResult, WorkflowContext};

// Tools
use xoneai::tools::{Tool, ToolRegistry, ToolResult, ToolDefinition};

// LLM
use xoneai::llm::{LlmProvider, LlmConfig, Message, Role, LlmResponse};

// Memory
use xoneai::memory::{Memory, MemoryAdapter, ConversationHistory};

// Errors
use xoneai::error::{Error, Result};
...
| What | Where |
|------|-------|
| Agent struct | `xoneai/src/agent/mod.rs` |
| AgentBuilder | `xoneai/src/agent/builder.rs` |
| Tool trait | `xoneai/src/tools/mod.rs` |
| ToolRegistry | `xoneai/src/tools/mod.rs` |
| #[tool] macro | `xoneai-derive/src/lib.rs` |
| LlmProvider | `xoneai/src/llm/mod.rs` |
| OpenAiProvider | `xoneai/src/llm/mod.rs` |
| AgentTeam | `xoneai/src/workflows/mod.rs` |
| AgentFlow | `xoneai/src/workflows/mod.rs` |
| MemoryConfig | `xoneai/src/config.rs` |
| Error types | `xoneai/src/error.rs` |
| CLI main | `xoneai-cli/src/main.rs` |
| CLI commands | `xoneai-cli/src/commands/` |
...
| `XONEAI_MODEL` | Default model | `gpt-4o-mini` |
| `XONEAI_LOG` | Log level | `info` |
...
*This document is the source of truth for the XoneAI Rust SDK architecture and development guidelines.*

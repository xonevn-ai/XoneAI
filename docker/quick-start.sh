...
# XoneAI Docker Quick Start Script
# This script helps users quickly get started with XoneAI using Docker
...
setup_environment() {
    if [ ! -f ".env" ]; then
        ...
        if [ -f ".env.template" ]; then
            ...
            cat > .env << EOF
# XoneAI Docker Environment Configuration
...
show_menu() {
    echo ""
    echo "=== XoneAI Docker Quick Start ==="
    echo ""
...
# Start all services
start_all_services() {
    print_status "Starting all XoneAI services..."
    docker-compose up -d
...
main() {
    clear
    echo "🤖 Welcome to XoneAI Docker Setup!"
    echo ""
    ...
    while true; do
        ...
        case $choice in
            ...
            11)
                ...
                ;;
            0)
                print_success "Thank you for using XoneAI!"
                exit 0
                ;;
            *)

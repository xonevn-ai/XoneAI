...
- XONEAI_REGISTRY_TOKEN: Token for authentication
"""
...
"""
    ...
    # Pack into .xone bundle
    bundle_path = output_dir / "sample-agent-1.0.0.xone"
...
from xoneai.recipe.server import create_wsgi_app
...
def main():
    ...
    try:
        ...
        result = subprocess.run(
            ["python3", "-m", "xoneai", "recipe", "publish", str(bundle_path),
             "--registry", f"http://127.0.0.1:{port}", "--json"],
            capture_output=True, text=True,
            cwd="/Users/xonevn/xoneai-package/src/xoneai"
        )
        ...
        result = subprocess.run(
            ["python3", "-m", "xoneai", "recipe", "search", "agent",
             "--registry", f"http://127.0.0.1:{port}", "--json"],
            capture_output=True, text=True,
            cwd="/Users/xonevn/xoneai-package/src/xoneai"
        )
        ...
        result = subprocess.run(
            ["python3", "-m", "xoneai", "recipe", "pull", "sample-agent",

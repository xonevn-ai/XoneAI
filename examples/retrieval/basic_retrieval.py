...
test_content = """
# XoneAI Framework Overview

XoneAI is an AI agents framework that enables building sophisticated AI applications.
...
os.makedirs(".xone/test_docs", exist_ok=True)
with open(".xone/test_docs/overview.txt", "w") as f:
...
agent = Agent(
    ...
    knowledge=[".xone/test_docs/overview.txt"],
    retrieval_config={
...
print("-" * 40)
response = agent.chat("What are the key features of XoneAI?")
print(response)
...
# Cleanup
import shutil
shutil.rmtree(".xone/test_docs", ignore_errors=True)

...
import time
from xoneaiagents.context.fast.search_tools import (
    grep_search,
    glob_search,
    read_file,
    list_directory
...
WORKSPACE = "/Users/xonevn/xoneai-package/src/xoneai-agents"
...
def main():
    ...
    # Read entire file
    result = read_file(
        filepath=f"{WORKSPACE}/xoneaiagents/__init__.py",
        max_lines=20
    )
    
    print(f"   File: xoneaiagents/__init__.py")
    ...
    result = read_file(
        filepath=f"{WORKSPACE}/xoneaiagents/agent/agent.py",
        start_line=1,
        end_line=20
    )
    ...
    result = list_directory(
        dir_path=f"{WORKSPACE}/xoneaiagents",
        recursive=True,
        max_depth=2,
        max_entries=50
    )
    
    print(f"\n   Directory: xoneaiagents (recursive, depth=2)")
    print(f"   Entries: {len(result.get('entries', []))}")

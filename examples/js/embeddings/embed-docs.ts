...
import { embedMany, cosineSimilarity } from '../../../src/xoneai-ts/dist/llm/embeddings';
...
async function main() {
  ...
  async function search(query: string, topK: number = 3): Promise<Array<{ doc: EmbeddedDocument; score: number }>> {
    const { embedding: queryEmbedding } = await (await import('../../../src/xoneai-ts/dist/llm/embeddings')).embed(query);

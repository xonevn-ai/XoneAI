...
⚠️ **Note**: Knowledge base functionality is temporarily disabled due to a compatibility issue with the current version of XoneAI Agents.
...
with st.sidebar:
    ...
    new_url = st.text_input(
        "Add URL",
        placeholder="https://docs.xoneai.com/introduction",
...
if openai_key:
    ...
    def load_knowledge() -> list:
        ...
        return ["https://docs.xoneai.com/introduction/agents.md"]  # Default URL
    ...
    def load_agent(_knowledge: list) -> Agent:
        ...
        # TODO: Re-enable when XoneAI Agents knowledge issue is resolved
        return Agent(
    ...
    with col1:
        if st.button("What is XoneAI?", use_container_width=True):
            st.session_state.query = "What is XoneAI and how do Agents work?"
    with col2:
        if st.button("Teams in XoneAI", use_container_width=True):
            st.session_state.query = "What are Teams in XoneAI and how do they work?"
    with col3:
...
with st.expander("📖 How This Works"):
    st.markdown("""
    **This app uses the XoneAI Agents framework to create an intelligent Q&A system:**
    
    ⚠️ **Current Status**: Knowledge base functionality is temporarily disabled due to compatibility issues with the current version of XoneAI Agents (v0.0.157).
    ...
    **What Works Now:**
    ...
    3. **Agent Framework**: XoneAI Agents for orchestration
    ...
    **Key Components:**
    - `Agent`: XoneAI Agents framework for creating intelligent agents
    ...
    **Next Steps:**
    - Monitor XoneAI Agents updates for knowledge base fixes

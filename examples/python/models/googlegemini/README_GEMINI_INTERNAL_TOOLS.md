...
This directory contains examples demonstrating how to use Google Gemini's built-in internal tools through XoneAI. These tools are natively supported by Gemini models and provide powerful capabilities without requiring external tool implementations.
...
3. **XoneAI Installation**: Ensure you have the latest version
   ```bash
   pip install "xoneaiagents[llm]"
   ```
...
XoneAI's tool system is designed to pass tools directly to LiteLLM without modification:
...
2. **Pass-Through**: XoneAI passes these tools directly to LiteLLM
...
If you create additional examples or find improvements, please contribute back to the XoneAI project!
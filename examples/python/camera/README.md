...
This directory contains examples demonstrating how to integrate camera feeds with XoneAI multimodal agents for real-time visual analysis.

## 📋 Overview

XoneAI supports visual input through the `images` parameter in Tasks. While there's no built-in camera capture, you can integrate camera feeds by capturing frames/videos and passing them to vision agents.

## 🛠️ Setup

### Requirements
```bash
pip install xoneaiagents opencv-python
```

# Gemini Embedding Support in XoneAI

This document describes how to use Google's Gemini embedding models with XoneAI's memory system.

## Overview

XoneAI now supports configurable embedding models through LiteLLM, including Google's Gemini embedding models. This allows you to use models like `text-embedding-004` or `gemini-embedding-exp-03-07` for semantic search and memory storage.

## Configuration

### Default Configuration (OpenAI)

By default, XoneAI uses OpenAI's `text-embedding-3-small` model:
...
2. **Dependencies**: Ensure you have the memory dependencies installed:
   ```bash
   pip install "xoneaiagents[memory]"
   ```

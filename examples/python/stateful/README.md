...
This directory contains comprehensive examples demonstrating XoneAI's stateful agents capabilities, including session management, memory systems, and persistent workflows.
...
**Key Features Shown:**
```python
workflow = XoneAIAgents(agents=[...], memory=True, process="workflow")
...
1. Install XoneAI with memory support:
```bash
...
workflow = XoneAIAgents(..., verbose=1)
```
...
These examples provide a comprehensive foundation for building sophisticated stateful agent applications with XoneAI.
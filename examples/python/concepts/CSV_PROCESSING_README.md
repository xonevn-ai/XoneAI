# CSV Processing with XoneAI Agents

This document answers GitHub Issue #23: "How to pass in a CSV file" and provides multiple solutions for processing CSV files with XoneAI agents.

## Quick Answer

**Yes, it's absolutely possible!** XoneAI provides multiple ways to process CSV files with agents working through the list sequentially.
...
XoneAI includes powerful CSV tools:

```python
from xoneaiagents.tools import read_csv, write_csv, merge_csv
...
4. **Processing stops early**
   - Increase `max_iter` parameter in XoneAIAgents

...
def run_command(args, description):
    """Run a xoneai command and display results."""
    print(f"\n[{description}]")
    print(f"Command: xoneai {' '.join(args)}")
    print("-" * 40)
    
    result = subprocess.run(
        ["python3", "-m", "xoneai"] + args,
        capture_output=True,
        text=True
    )
...
def main():
    print("=" * 60)
    print("XoneAI Package Manager Example")
    print("=" * 60)
    ...
    print("  xoneai install requests")
    print("  xoneai install 'requests>=2.28'")
    print("  xoneai install requests httpx --upgrade")
    
    print("\nTo uninstall packages, use:")
    print("  xoneai uninstall requests")
    print("  xoneai uninstall requests --yes")
    ...
    print("  xoneai install pkg --extra-index-url https://other.index.com/simple")
    print("  → Error: Extra index URLs not allowed")
    
    print("\nExample (explicitly allowed):")
    print("  xoneai install pkg --extra-index-url https://other.index.com/simple --allow-extra-index")

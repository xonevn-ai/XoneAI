...
def list_endpoints():
    ...
    result = subprocess.run(
        ["python", "-m", "xoneai.cli.main", "endpoints", "list", "--format", "json"],
        capture_output=True,
        text=True,
    )
...
def describe_endpoint(name: str):
    ...
    result = subprocess.run(
        ["python", "-m", "xoneai.cli.main", "endpoints", "describe", name, "--url", SERVER_URL],
        capture_output=True,
        text=True,
    )
    if result.returncode == 0:
...
def invoke_endpoint_cli(name: str, input_data: dict):
    ...
    result = subprocess.run(
        [
            "python", "-m", "xoneai.cli.main", "endpoints", "invoke", name,
            "--url", SERVER_URL,
...
def main():
    ...
    print("=" * 60)
    print("XoneAI Endpoints Example")
    print("=" * 60)
    ...
    if not check_server_health():
        ...
        print("  xoneai recipe serve --port 8765")
        sys.exit(1)

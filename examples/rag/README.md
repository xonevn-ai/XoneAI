...
This directory contains comprehensive examples demonstrating RAG (Retrieval Augmented Generation) with XoneAI Agents.
...
```bash
# Index documents
xoneai rag index ./documents --collection my_docs

# Query
xoneai rag query "What is the main finding?" --collection my_docs

# Interactive chat
xoneai rag chat --collection my_docs

# Evaluate
xoneai rag eval golden_queries.json --collection my_docs
```
...
Use with:
```bash
xoneai rag query "Question" --config rag_config.yaml
```

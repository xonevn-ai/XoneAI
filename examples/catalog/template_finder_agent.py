...
CATALOG_URL = "https://xonevn-ai.github.io/xoneai-template-catalog/data/templates.json"
    ...
    for t in templates:
        if (query_lower in t.get("name", "").lower() or
            ...
            results.append({
                ...
                "tags": t.get("tags", []),
                "cli_command": f"xoneai templates run {t.get('name')}"
            })
    ...
    for t in templates:
        if t.get("name") == name:
            return json.dumps({
                ...
                "cli_commands": {
                    "run": f"xoneai templates run {name}",
                    "info": f"xoneai templates info {name}",
                    "init": f"xoneai templates init my-project --template {name}"
                }
            })
...
def check_api_key() -> bool:
    ...
    key = os.environ.get("OPENAI_API_KEY") or os.environ.get("XONEAI_OPENAI_API_KEY")
    if key:
...
def main():
    print("XoneAI Template Finder Agent")
    print("=" * 40)
    
    # Check for API key
    if not check_api_key():
        print("\n⚠️  No API key found.")
        print("Set OPENAI_API_KEY or XONEAI_OPENAI_API_KEY to use the agent.")
    ...
    try:
        ...
        print("xoneaiagents not installed. Install with: pip install xoneaiagents")
        return
    
    # Create the Template Finder Agent
    agent = Agent(
        name="Template Finder",
        role="XoneAI Template Expert",
        goal="Help users find the right template for their needs",
        backstory="""You are an expert on XoneAI templates. You help users 

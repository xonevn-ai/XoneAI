#!/usr/bin/env python3
"""
Example: Fetch and search XoneAI templates from the catalog.
...
def generate_cli_command(template: Dict, action: str = "run") -> str:
    ...
    if action == "run":
        return f"xoneai templates run {name}"
    elif action == "info":
        return f"xoneai templates info {name}"
    elif action == "init":
        return f"xoneai templates init my-project --template {name}"
    return f"xoneai templates {action} {name}"
...
def main():
    print("XoneAI Template Catalog - Python Example")
    print("-" * 45)
    ...
    if not templates:
        ...
        print("   Try building locally: xoneai templates catalog build")
        return

...
const CATALOG_URL = "https://xonevn-ai.github.io/xoneai-template-catalog/data/templates.json";
...
function generateCLICommand(template: Template, action: string = "run"): string {
  ...
  switch (action) {
    case "run":
      return `xoneai templates run ${name}`;
    case "info":
      return `xoneai templates info ${name}`;
    case "init":
      return `xoneai templates init my-project --template ${name}`;
    default:
      return `xoneai templates ${action} ${name}`;
  }
}
...
async function main(): Promise<void> {
  console.log("XoneAI Template Catalog - TypeScript Example");

...
Demonstrates how XoneAI Agents uses lazy imports to achieve
...
def example_measure_memory():
    ...
    # Clear modules
    for key in list(sys.modules.keys()):
        if key.startswith('xoneaiagents'):
            del sys.modules[key]
...
def example_specific_imports():
    ...
    print("  from xoneaiagents import Agent, Task")
    
    # Avoid - star imports load everything
    print("\nAvoid - star imports load more:")
    print("  from xoneaiagents import *")
...
if __name__ == "__main__":
    print("XoneAI Agents - Lazy Imports Example")
    print("=" * 60)

"""
Storage Backends Example for XoneAI Agents.
...
from xoneaiagents.storage import (
    FileBackend,
    SQLiteBackend,
    BaseJSONStore,
    get_backend,
...
def example_file_backend():
    ...
    # Create file backend
    backend = FileBackend(storage_dir="/tmp/xone_example")
...
def example_sqlite_backend():
    ...
    backend = SQLiteBackend(db_path="/tmp/xone_example.db")
...
def example_backend_factory():
    ...
    if backend_type == "file":
        backend = get_backend("file", storage_dir="/tmp/xone_factory")
    elif backend_type == "sqlite":
        backend = get_backend("sqlite", db_path="/tmp/xone_factory.db")
...
def example_with_base_json_store():
    ...
    backend = SQLiteBackend(db_path="/tmp/xone_store.db")
...
def example_training_storage_with_backend():
    ...
    try:
        ...
        print("xoneai package not installed, skipping training example")
...
def example_session_manager_with_backend():
    ...
    try:
        from xoneai.cli.state.sessions import SessionManager
        ...
        print("xoneai package not installed, skipping session example")
...
def example_run_history_with_backend():
    ...
    try:
        from xoneai.recipe.history import RunHistory
        ...
        print("xoneai package not installed, skipping run history example")
...
def example_mcp_tool_index_with_backend():
    ...
    try:
        from xoneai.mcp_server.tool_index import MCPToolIndex
        ...
        print("xoneai package not installed, skipping MCP example")
...
if __name__ == "__main__":
    print("XoneAI Storage Backends Examples")
    print("=" * 50)

...
def main():
    ...
    try:
        ...
        print("Error: xoneaiagents not installed")
        print("Install with: pip install xoneaiagents")
        sys.exit(1)
...
def launch_manual(agent):
    ...
    app = FastAPI(title="XoneAI Agent API")
    ...
    async def root():
        return {
            "message": f"XoneAI Agent: {agent.name}",
            "endpoints": ["/agent", "/health", "/__xoneai__/discovery"]
        }
